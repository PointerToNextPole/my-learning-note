...
> Promise 的出现最重要的原因是：为了统一 JS 中的异步实现方案（在此之前都是使用回调，且使用方法千奇百怪：回调的个数、以及回调传参个数不确定）。另外，正是因为异步实现方案的统一，所以才会有 async / await 之类的出现
>
...
- `.prevent` ：和 `Event.preventDefault()` 作用一样，提交事件不再重载页面。比如 `submit` 事件触发后默认重新加载页面，而加上后则不会再重新加载
...
- `Proxy` 支持 13 种拦截操作，`defineProperty` 只支持 `[[Get]]` 和 `[[Set]]`
  ...
  > > 
  > > > 使用 Reflect 保证了 this 指向的是 “代理对象”，而非 “原始对象”
  > > > 
...
- 具备一定的惰性 lazy（数据不改变则不会执行）
  ...
  如果想要让 `watch` 变成 immediate （即：`watch` 创建时，数据改变相应的处理的回调立即执行一次），只需要在 watch 函数之后加上配置：`{ immediate: true }`
