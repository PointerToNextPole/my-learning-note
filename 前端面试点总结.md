...
- **多路复用** ：<font color=fuchsia>**HTTP/2 实现了多路复用**</font>，HTTP/2 仍然复用 TCP 连接，但是<font color=fuchsia>在一个 ( TCP ) 连接里</font>，<font color=fuchsia>客户端和服务器都可以同时发送多个请求或回应，而且不用按照顺序一一发送</font>（ 👀 可以乱序发送），<font color=fuchsia>这样就避免了 “队头堵塞” 的问题</font>。
