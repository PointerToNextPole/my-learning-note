...
>
> - `package.json` 中的 `sideEffect` 配置不当，导致 webpack 认为某些文件具有副作用，而不敢删除；或者反过来删除了全局 CSS i9出发图5
>
...
- ref 在 JavaScript / TypeScript 逻辑中，必须通过 `.value` 属性来访问和修改值。在 Vue 的模板（Template）中，Vue 会进行自动解包（Auto-unwrapping），不需要加 `.value` ，直接使用即可。
- reactive 无论是在 JS 逻辑中还是在模板中，都**不需要**任何后缀，直接像普通对象一样访问和修改属性即可。

###### 重新赋值的表现

- ref 可以对 `.value` 重新赋予一个全新的对象，不会丢失响应式。
- reactive 不能直接重新赋值整个对象，否则会导致原本的响应式连接断开。因为重新赋值会让变量指向一个非 Proxy 的普通对象。可以使用 `Object.assign(reactiveObj, { /* ...newObj */ })`

###### 解构时的表现

- 直接对 reactive 对象进行 ES6 解构或使用展开运算符，解构出来的属性会失去响应式。为了保持响应式，需要配合 `toRefs()` 或 `toRef()` 使用。
- ref 包含的本身就是单一的值或引用，通常不会直接解构 ref 对象本身。哪怕进行结构也不会失去响应式

###### 底层实现

- **ref**：
    
    - 对于基本数据类型：内部通过 RefImpl 类，使用对象的 getter 和 setter（即类属性访问器）来实现响应式拦截。
        
    - 对于引用数据类型：ref 内部会自动调用 reactive()，将其转化为 Proxy 对象，然后再挂载到 .value 上。
