...
- 该元素的 <font color=red>**最近可滚动祖先**</font> ：向上层寻找 `overflow` 属性为 `scroll` / `auto` / `hidden` 的祖先（只要不是 `visible`），如果往上找都找不到，即为视口 viewport 。
...
- **样式计算 ( style computing )**：确定每个元素的样式是什么，因为元素的样式不仅受到开发者书写的 CSS 样式影响，也会收到 User Agent StyleSheet 的影响
...
而 <font color=red>**“包含块”** ( containing block ) **属于视觉格式化模型的基本概念**</font>，包含块的概念是：<font color=fuchsia>一个元素在哪个区域里边排列</font>
