...
- `window.addEventListener("error")`
  ...
  > ⚠️ 相当值得注意的是：error 事件是不会冒泡的，通过冒泡无法监听，只能通过捕获监听；所以在使用 `EventTarget.addEventListener` 添加 error 事件时需要加上 `useCapture` 为 true，或者 `options` 对象中设置 capture 为 true。代码示例如下：
  >
  > ```js
...
- `defineProperty` <font color=fuchsia>只能通过递归劫持对象的属性</font>，而 `Proxy` 是 <font color=fuchsia>代理整个对象，且是惰性的，只会代理对象的第一层</font>
  ...
  > - 检测数组的时候可能触发了多个 `get/set`，那么如何防止触发多次呢？可以判断 `key` 是否是当前被代理的 `target` 自身属性
  >
