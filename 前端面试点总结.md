...
在 javascript 中我们推荐使用 `property` 属性 因为这个属性相对 `attribute` 更快，更简便。尤其是有些类型本该是布尔类型的attribute 特性。比如："checked", "disabled", "selected"。浏览器会自动将这些值转变成布尔值传给 property 属性。
...
2. **类选择器**，例如 `.example`
   ...
   **属性选择器**，例如 `[type="radio"]`
   ...
   > [!NOTE]
   > 
   > `html` 和 `:root` 基本上是等价的，不过，特指度不同，`html` 为 `001` ，`:root` 为 `010`
   > 
   > 学习自：[妙用CSS变量【渡一教育】](https://www.bilibili.com/video/BV1q94czJERH)
...
**`relative`** 和 `static` 类似，都会跟随 html 的排版流程移动（👀 这里是否可以说为受到文档流影响？）；`relative` 元素 相对于自身（本来的、不设置定位的）位置进行移动。但 `absolute` 不会

<font color=dodgerBlue>`relative` 最重要的功能是</font>：在它里面的 `absolute` 元素会根据 `relative` 的位置去定位（即：子绝父相）；而如果将 `absolute` 元素用 `static` 元素包裹，则完全不会按照父元素的位置去定位。

> [!warning]
> 
> 值得注意的是：上面 “子绝父相” 中的 <font color=red>“父子” 未必是指 DOM 树中绝对的父子关系，祖先和子孙也可以</font>。
>
> 另外，一直以来对于 absolute 定位的理解都是存在问题的：未必限制是 “子绝父相”，也可以是其他非 `static` 定位的祖先元素（即：所有不使用 `position` 以及显性使用 `position: static` 的元素）。可以看下 https://g.co/gemini/share/144d9692cf60 中 Gemini 的回答
>
...
**`fixed`** 是根据窗口定位。另外，即使 `fixed` 的元素被 `relative` 的父元素所包裹，但依然会根据 body 去定位；<font color=dodgerBlue>但是有一个特殊的地方，如果 `top` 、`bottom`、`left`、`right` <font size=4>**都**</font> 没有设置</font>，<font color=fuchsia>**则 `fixed` 的效果和 `absolute` 一样，相对位置是相较于 `relative` 父元素**</font>。

###### 修改 fixed 的基准参照对象

- 父元素设置 `transform` 属性，除了值为 `none` ，子属性将以这个设置了 `transform` 的父元素为参照对象，而不是浏览器窗口
...
- 父属性设置了 `contain` 属性中的 `paint` 以及 `paint` 相关的复合值（`strict`、`content` 等）
  ...
  > > TODO：[CSS新特性contain，控制页面的重绘与重排](https://juejin.cn/post/6958990366888607757)
  >
...
在 Vue 开发环境下，可以使用 `<Teleport to="body"><TargetElementOrComponent /></Teleport>` ，强制让一个元素归属于 body 下，而不会跑到其他地方去。

##### sticky

**`sticky`** （粘性布局）会像 `relative` 一样，默认会受到 HTML 排版的影响；但是一旦页面滚动，将会固定在设置的 `top` `bottom` `left` `right` 指定的位置。一般是 y 轴滚动，所以，可以只设置 `top` ，当 “sticky 块” y 轴的位置移动到 `top` 的位置，则会固定 ( fixed ) 下来。 👀 这也是教程视频中的示例。
...
>
> 另外，在 [MDN - position # 粘性定位](https://developer.mozilla.org/zh-CN/docs/Web/CSS/position#粘性定位) 中也有类似说明：
>
...
<font color=dodgerBlue>一个元素的粘性布局位置会受到如下两个东西的影响：</font>

- 该元素的 <font color=red>**包含块**</font> ( Containing Block ) ：一般情况下是它父元素的内容盒（可以简单理解为父元素）；虽然也有其他情况，详见 [[#如何确定包含块]]
- 该元素的 <font color=red>**最近可滚动祖先**</font> ：向上层寻找 `overflow` 属性为 `scroll` / `auto` / `hidden` 的祖先（只要不是 `visible`），<font color=dodgerBlue>如果往上找都找不到</font>，即为视口 viewport 。
...
学习自：[粘性定位【渡一教育】](https://www.bilibili.com/video/BV1gG411v7n5)
...
- **`display: none`** ：<font color=fuchsia>渲染树 **不会包含该 渲染对象**</font>，因此<font color=FF0000>该元素不会在页面中占据位置</font>，也<font color=FF0000>**不会响应绑定的监听事件**</font>
  ...
  > 💡 **渲染对象** 是 **渲染树** 的节点。另外，不可见的 DOM 元素不会被插入渲染树；不可见的 DOM 中，也包含 meta 元素

- **`visibility: hidden`** ：<font color=fuchsia>元素在页面中仍占据空间</font>，但是 <font color=fuchsia>**不会响应绑定的监听事件**</font>

- **`opacity: 0`** ：将元素的透明度设置为 0，以此实现元素的隐藏。<font color=FF0000>元素在页面中仍然占据空间</font>，并且 <font color=FF0000>**能够响应元素绑定的监听事件**</font>

- **`position: absolute`**：通过使用绝对定位将元素从可视区域移除，以此来实现元素的隐藏
...
- **是否是继承属性**
  ...
  - `display: none` 是<font color=FF0000>非继承属性</font>，子孙节点会随着父节点从渲染树消失，<font color=red>修改子孙节点的属性也无法使自身显示</font>
...
**`place-content`** 复合属性是 `align-content` 和 `justify-content`的简写；`align-content` 在前，`justify-content` 在后
...
- `initial` 则是不继承，使用 CSS 属性的默认值
   >[!WARNING]
   > 
   > 值得注意的是：我开始的理解是错误的，即以为作用为 “将其设置为 UA StyleSheet ”，而这是完全错误的。这里摘抄下 [MDN - `initial`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/initial) 的内容：
   > 
   > > CSS 关键字 **`initial`** 将属性的初始（或默认）值应用于元素。不应将初始值与浏览器样式表指定的值混淆。
   > 
   > 此外，顺带问了 Gemini 具体解释， 下面摘抄了 [回答](https://gemini.google.com/share/1f6bc016071e) 中的重点部分
   > > “初始值” 是针对 <font color=fuchsia>CSS 属性本身的规范</font>，而 “UA StyleSheet” 是<font color=fuchsia>针对 HTML 元素的浏览器预设</font>。
   > > 
   > > 在 W3C 的 CSS 规范中，**每一个 CSS 属性都有一个官方指定的、雷打不动的“初始值”**。这个初始值只跟“属性”有关，跟它被应用在哪个 HTML 标签上**毫无关系**。
- `unset` 表示如果该属性默认可继承，则值为 `inherit` ，否则，值为 `initial` 。
...
##### span 与 span 之间有看不见的空白间隔是什么原因引起的
...
> [!TIP]
> 空白字符是一个 `#text` 的对象。另外，这里说的不完善，参考 [[JavaScript备忘录#Node.nextSibling#实践中的发现]] 中的内容
...
- **样式计算 ( style computing )**：确定每个元素的样式是什么，因为元素的样式不仅受到开发者书写的 CSS 样式影响，也会受到 User Agent StyleSheet 的影响
- **视觉格式化模型**：完成页面的布局。IFC、BFC、FFC、GFC 都属于这一块的范畴。

> 此外，根据 [使用chrome调试工具解决问题【渡一教育】](https://www.bilibili.com/video/BV18tM7ziEYv) 中的说法：“样式计算” 和 “视觉格式化模型” 分别对应 浏览器渲染管线 中渲染的的 第二步 ( style computing ) 和 第三步 ( layout，通过 “视觉格式化模型” 来确定元素的位置和尺寸)。这样的说法，在 [包含块【渡一教育】](https://www.bilibili.com/video/BV1XW421R7Su?t=157) 中也有说明（其中第一步是 parse，即解析 html 文档）

而 <font color=red>**“包含块”** ( containing block ) **属于视觉格式化模型的基本概念**</font>，包含块的概念是：<font color=fuchsia>一个元素在哪个区域里进行排列</font>

<font color=dodgerBlue>所以出现如下概念：</font>

下面这些属性的决定，<font color=red>**并不完全是根据父元素决定**</font>，<font color=dodgerBlue>更准确的说法</font> 是 <font color=fuchsia>**由包含块决定**</font>

> [!NOTE]
> 看了下，是空间相关的属性，也是盒子模型相关的属性
...
> [!NOTE]
> 此外，值得注意的是：除了 `height` 是相对于 “包含块” 的高度外，其他 `width`、`margin`、`padding` 都是相对于 “包含块” 的宽度

##### 如何确定包含块

1. 所有的 <font color=dodgerBlue>**浮动元素**</font> 和 <font color=dodgerBlue>**常规元素**</font>，它们的包含块是父元素的 **内容区域**（父元素的内容盒）
2. 一个绝对定位 ( `position: absolute` ) 的元素，它的包含块是离它最近的定位祖先的 padding 区域（即：填充盒 / padding box）
3. 一个固定定位 ( `position: fixed` ) 的元素，它的包含块要看它的祖先中是否改动了 transform。如果有，则是该祖先的填充盒 ( padding box ) 区域；如果没有，则为浏览器的视口

##### 渡一的解释

按照视频中所说：绝对定位和固定定位有什么区别？一个比较高级、更接近本质的回答是：这两种定位的包含块不一样
...
>
> > JavaScript 提升是指：<font color=red>解释器在执行代码之前</font>，<font color=fuchsia>**似乎** 将 函数、变量、**类** 或 <font size=4>**导入的声明**</font>（ ⚠️ 注意只有 静态导入）</font> 移动到其作用域的顶部的过程。
> >
> > > [!TIP]
> > > “hoisting 行为只会存在于 ‘静态导入’，‘动态导入’ 不存在” ，在询问 Gemini 后，得到了 [肯定的回答](https://g.co/gemini/share/0075e58721db)
> > > 
...
> [!TIP]
> 
> 这句话里的 “上下文” 可以按 **执行上下文** ( Execution Context ) 来理解：`var` 声明会被记录到当前执行上下文的 **变量环境** ( VariableEnvironment ) 中。
>
> - <font color=dodgerBlue>在函数体内</font>，当前执行上下文是函数执行上下文，所以 `var` 是函数作用域。
> - <font color=dodgerBlue>在普通 script 顶层</font>，当前执行上下文是 Script / 全局代码执行上下文，所以 `var` 会进入全局环境；在浏览器中，顶层 `var` 通常也会成为 `window` 的属性。
> - <font color=dodgerBlue>在 ES Module 顶层</font>，当前执行上下文是 Module 执行上下文，顶层 `var` 属于模块环境，不会成为全局对象的属性。
>
> 按代码入口理解，常见执行上下文包括：**Script / 全局代码执行上下文**、**Module 执行上下文**、**函数执行上下文**、**eval 执行上下文**。Worker、iframe、Service Worker 等属于宿主环境提供的独立全局环境 / Realm / 浏览上下文；Promise 回调运行时创建的是函数执行上下文；`with` 影响的是词法环境链；原型链负责属性查找，不属于执行上下文。

> [!TIP]
> 
> <font color=fuchsia size=4>**var 没有块级作用域，但是有函数作用域**</font>
>
> 学习自：[后盾人 - 第九章 这次把JS闭包给你讲得明明白白](https://www.bilibili.com/video/BV1YJ411R7ap)

<font color=dodgerBlue>**如果变量未经声明就被初始化了**</font>（👀 类似 python：直接使用一个变量，未使用 var 之类的定义关键字）， 那么<font color=red>**它就会自动被添加到全局上下文**</font>（💡 可以理解为：挂载到 `globalThis` 下）。如下示例：
...
> [!TIP]
> 
> 上面所说的是：使用 `var` 定义，和不使用 `var` 直接定义的区别中的一个：只要定义了，就是全局作用域
>
> 还有一个是：不使用 `var` 定义可以被 `delete` 掉（ 因为它被挂载到 `globalThis` 下了。另外，`delete` 操作返回 `true` ），而使用 `var` 定义，无法被 `delete` 掉（ `delete` 操作返回 `false` ）。
>
> 感觉可以理解为：
>
> ```js
...
> ```
>
> 无论 `nonVarDefVariable` 定义在哪里，都等价于 `delete globalThis.nonVarDefVariable` 
>
> ```js
> varDefVariable = 'bar'
...
> ```
>
> 另外，由上面 `delete foo` 为 `true` 可知，由于这里 `var foo = 'bar'` 的 foo 在函数作用域外无法访问，所以这里 `delete foo` 还是等价于 `delete globalThis.foo`
...
ES6 新增的 `let` 关键字跟 `var` 很相似，但 <font color=red>它的作用域是块级的，这也是 ES6 引入的新概念</font>。<font color=fuchsia>**块级作用域由最近的一对包含花括号 `{}` 界定**</font>。换句话说，<font color=fuchsia>if 块、while 块、function 块，甚至连单独的块也是 `let` 声明变量的作用域</font>

<font color=dodgerBlue>`let` 与 `var` 的另一个不同之处是</font>： `let` 在同一作用域内不能声明两次。<font color=fuchsia>**重复的 `var` 声明会被忽略**</font>，而重复的 `let` 声明会抛出 SyntaxError
...
> [!TIP]
> 
> 在 TDZ 状态中，`let` 声明的变量 **会被设置为 `<uninitialized>` 标志位**
...
>
> > 这种行为（ 👀 变量提升）仅适用于函数声明，而不适用于 将函数分配给变量的 <font color=fuchsia>函数表达式</font>，例如 `let say = function(name)` ...
> >
...
1. <font color=dodgerBlue>Key 都为自然数（正整数或 0）</font>，按照自然数的大小进行升序排序（⚠️ 这里所属的自然数：包含不带引号的自然数、科学计数法自然数，和带引号但是可以被成功转换为自然数的自然数字符串）。<font color=dodgerBlue>如果是其他类的 Number</font> —— 浮点数或者负数 —— 都会走到下一组类型（即：String ）里，像 `NaN` 或者 `Infinity` 这种也自然归到下一个类型里；另外，科学计数法的数字会被转换为直接表示的数字，作为 key，并按照这里的规则进行排序
   ...
   > [!NOTE]
   > 
   > 值得注意的是：[[#如何应对对象属性的有序性]] 中 `statusMap` 中的 `'00'` ，它不是正整数，因为是正整数的应该是 `'0'` ，而  `'00'` 和 `'0'` 显然不是一个东西。另外， `statusMap` 可以定义为下面这样，应该能减少不少困惑。
   >
   > ```js
   > const statusMap = {
   >   40: '报警关',
...
另外，个人感觉：对象属性的有序性应该是预编译时处理好的，如下示例：
...
在 <font color=dodgerBlue>**非严格模式**</font>（宽松模式） 下，`arugments` 会和对应参数的内容保持一致的，无论修改哪一个，另一个都会被修改。而这显然是不太合理的，<font color=lightSeaGreen>为了兼容性考虑，**在 ES5 添加的严格模式中，上面的特性将不会存在**</font>。同时，如果使用默认参数，哪怕不在严格模式下，`arugments` 和对应参数的机制也和严格模式一样。
...
>
> 👀 所以，对于 “ this 的绑定和 调用方式以及调用的位置 有关系” 不适用于 箭头函数，<font color=fuchsia>**它只和定义处相关**</font>；此外，箭头函数内部都没有 this，自然没法给它强制绑定一个 this
...
> ###### 一句话答案
>
> <font color=fuchsia>因为箭头函数没有 `[[Construct]]` 这个内部槽</font>（ 👀 internal slot ）
>
> ###### 进一步解释
>
...
>
>从上图中可以看到 `import` 导入进来的 `CountChild` 变量是一个对象（ 👀 更准确的说法是：组件配置对象），并且对象上面还有一些属性的方法：`render`、`setup` 。
>
...
根据 ECMAScript 规范，数组的索引必须严格小于 `2^32`。那如果超过了会怎样呢？

```js
const arr = [0];
arr[4294967296] = true
arr // 结果是 [ 0, '4294967296': true ]
```

这时它不再被当作数组元素，而是被当作一个普通的键值对。既然这样，我们当然也可以往数组里存放任意数据：

```js
 arr.name = "Brandon"
 arr // 结果是 [ 0, '4294967296': true, name: 'Brandon' ]
```

> 👀 经过上面两个操作， `Array.isArray(arr)` 结果依然为 “真”

摘自：[【第3586期】从规范看 JavaScript 数组：隐藏的对象属性](https://mp.weixin.qq.com/s/6gfm8c0Sxjs30uvQH_yHVw)

规范里规定的数组最大长度是 `2 ** 32 - 1`，也就是 32 位无符号整型 uint32 的最大值，大概 40 多亿：

<img src="https://s2.loli.net/2025/09/22/DOI7Qf8sRJXHqn6.png" alt="PNG_image.png" style="zoom:55%;" />

length 超过这个数字就一定会报错，但在实际的引擎实现里，由于受存储空间限制，数组长度的限制会更小，之所以现在给 length 设置 `2 ** 32 - 1` 没报错，是因为 JS 的数组可以有空 ( hole )，如果用 `fill()` 方法把它填满，就会发现根本存不下，会 OOM：

<img src="https://s2.loli.net/2025/09/22/1nyO2r6DvoEWkwu.png" style="zoom:50%" />  

假如用 `push()` 方法一个一个往里填呢？你就会发现元素数量到了 112813858，就抛了一个异常，抛异常比 `fill()` 的 OOM 是好多了：  

<img src="https://s2.loli.net/2025/09/22/IK8abrhVS3EXwOJ.jpg" alt="JPEG_image.jpg" style="zoom:55%;" />

> 👀 下面是在讲 JS 数组的扩容机制，感觉涉及到 V8 实现了，这里略。不过，还是要注意的是，其中说明了：数组的定义方法不同，初始化时给的容量不一样。
>
> <img src="https://s2.loli.net/2025/09/22/UvA5X6E7ZHjh81d.png" alt="PNG_image.png" style="zoom:50%;" />

摘自：[nodejs中数组允许的最大长度是多少？ - 紫云飞的回答 - 知乎](https://www.zhihu.com/question/650249886/answer/3448557422)
...
> [!warning]
> 
> 上面所说是：`setTimeout` 是在 “任务队列” 的尾部添加一个事件，不是 “执行栈” 的尾部。另外，这里要注意 `setTimeout(fn, 0)` 和 `process.nextTick` 的区别
...
异步函数（比如 setTimeout ）执行之后，浏览器会把异步函数（比如 `setTimeout(fn, 5000)` ）的回调函数 放到 <font color=FF0000 size=4>Event Table</font> 中。<font color=fuchsia>**Event Table 就是个注册站：调用栈让 Event Table 注册一个函数**，该函数会在5秒之后被调用</font>。<font color=FF0000>当指定的事情发生时，Event Table 会将这个函数移到 Event Queue</font>。<font color=FF0000 size=4>**Event Queue 其实就是个缓冲区域**，这里的函数等着被调用并移到调用栈</font>
...
摘自：[JavaScript 事件循环（译文JavaScript Event Loop）](https://segmentfault.com/a/1190000006811224) 另外，其中有代码流程示例，推荐阅读
...
3. <font color=red>失活页面的间隔会被强制调整为 1s</font>：哪怕设置的间隔远小于 1s，浏览器也会将间隔调整为 1s

###### 如何实现

1. <font color=red>回调中通过 `Performance.now()` **计算间隔偏差，并实时调整**</font>。受上述 1 和 2 的影响较小，主要受 3 的影响
...
> [!NOTE]
> 
> Promise 的出现最重要的原因是：为了统一 JS 中的异步实现方案（在此之前都是使用回调，且使用方法千奇百怪：回调的个数、以及回调传参个数不确定）。正是因为异步实现方案的统一，才会有 async / await 之类的出现
>
...
<font color=dodgerBlue>关于 ES6 的 Promise</font> ：ES6 中创建了构造函数 Promise ，它算是对 Promises/A+ 规范的实现（因为当时第三方库对 Promises/A+ 的遵守和实现已经相当普遍），通过构造函数 Promise 去创建 Promise 对象。构造函数 Promise 没有 `then` 方法，他创建的对象中拥有 `then` 方法。在这基础上，ES6 Promise 还加入了 `finally` 和 `catch` 方法（在 [Promise A+ 文档](https://promisesaplus.com/) 上确实完全找不到 `finally` 和 `catch` ）。
...
>
> - `[[PromiseState]]` —— 最初是 `"pending"`，然后在 `resolve` 被调用时变为 `"fulfilled"`，或者在 `reject` 被调用时变为 `"rejected"`。
> - `[[PromiseResult]]` —— 最初是 `undefined`，然后在 `resolve(value)` 被调用时变为 `value`，或者在 `reject(error)` 被调用时变为 `error`。
>
> Promise 对象的 `[[PromiseState]]` 和 `[[PromiseResult]]` 属性都是内部的，我们无法直接访问它们。
>
> 摘自：[现代 JS 教程 - Promise](https://zh.javascript.info/promise-basics) ，不过将 state 和 result 改成了 `[[PromiseState]]` 和 `[[PromiseResult]]`
> ***
>
...
- <font color=fuchsia>**抛出异常**</font> ： <font color=fuchsia>如果当前是 pending 就会变为 rejected</font>
...
> [!Warning]
> 
> 注意：一旦从 pending 状态变成为其他状态 ，就永远不能再更改状态了（变成 settled / resolved 状态），即：状态不可逆。
>
...
>
> 摘自：[MDN - US - `Promise.resolve()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/resolve)  
>

 > [!NOTE]
> 中文版翻译的很差。此外，根据这里的描述就已经可以实现一个 `Promise.resolve` 了；代码见 [[JS 函数手写实现#`Promise.resolve` 实现]]
> 
> 此外，这里有一个注意点：如果传入的 value 是一个 Promsie，那么这个 promise 会原封不动的 return 回来。即：
> 
> ```js
...
- **Promise 内部的错误不会影响到 Promise 外部的代码**，<font color=dodgerBlue>如果不使用 catch</font>，将<font color=red>无从获知 Promise 内部的错误</font>
...
- **无法取消 Promise**：一旦新建它就会立即执行，无法中途取消
  ...
  > [!TIP]
  >  
  > 知乎问题 [Promise 不能被取消，真的算是它的缺点吗？](https://www.zhihu.com/question/495412354) 其中有些内容相当值得参考
  >
  ...
  >
  > 下面内容，可能有点答非所问？不过，还是感觉内容开阔了视野，还是相当有启发性的：
  >
  > > HTTP 请求真的能取消，不然 `AbortController` 是干什么的。<font color=lightSeaGreen>虽然 HTTP 并没有定义 “取消” 这种东西，但你底下是 TCP/QUIC 啊，**TCP/QUIC 连接是能关闭的** 啊，而且还有完善的关闭事件的处理支持</font>
  > >
  > > 在页面加载时，你按 ESC 中止加载的瞬间浏览器就会断开和服务器的连接，<font color=red>此时服务器是能收到 FIN 信号的</font>，利用 `AbortController` 中止 `fetch` 请求也同理。
  > >
  > > <font color=dodgerBlue>像 asp.net core 等框架就 **实现了取消正在进行中的请求来避免资源的浪费**</font>：<font color=red>在服务器收到连接终止信号时会触发分配给当前请求的 `CancellationToken` 的取消信号，使得整个异步请求中所有的操作都能够被尽可能快地中止</font>。因此<font color=fuchsia>对于加载一半然后被取消的请求而言，是真的能做到请求被取消的时候 **立刻停止正在进行的数据库查询以及正在拼接的 HTML 和数据的**</font>。
  > >
...
<font color=fuchsia>async 函数返回一个全新的 Promise 对象</font>。另外，多次运行同一个 async 函数，它返回的 Promise 对象一定是不同的对象（不论返回的是原始类型的 `1` ，还是返回函数外层定义的 Promise）

> [!NOTE]
> 返回一个全新的 Promise ，这点相当容易遗忘；如果函数本身返回的是一个 Promise，那么就会出现 “状态吸收” 现象
...
> [!NOTE]
> 
> 在做题目 [[JS面试题#事件队列#事件队列第6题]] 时，发现还有些概念没完全搞清楚，下面做下补充。另外，async 函数会返回一个全新的 Promise

`async` 函数处理返回值，会像 `Promise.prototype.then` 一样，会对返回值的类型进行辨识。<font color=dodgerBlue>**根据返回值的类型**，引起  JS引擎 对返回值处理方式的不同</font>：

- <font color=fuchsia>**return 结果值为 非 `thenable` / 非 `promise`</font>（示例如下） ：<font color=fuchsia>不等待**</font>
...
- return 结果值为 `thenable` （示例如下）：等待 1 个 `then` 的时间（这个比较好理解）
...
- <font color=fuchsia size=4>**return 结果值为 `promise`**</font> （示例如下）：<font color=fuchsia size=4>等待 2个 `then` 的时间</font>
...
> [!NOTE]
> 这里的内容相当重要，比如 [[JS面试题#事件队列#事件队列第7题|事件队列第7题]] 运行结果的原理就是出自这里，而这题的结果有点颠覆我对 await 的认知。这里强烈建议去看下这道题
...
- `await` 命令后面的 `Promise` 对象，运行结果可能是 `rejected` ，所以<font color=fuchsia>最好把 `await` 命令放在 `try...catch` 代码块中</font>。
  ...
  > 个人感觉这种方法有点丑，还有两种方法：
  > 
  > - 使用 [await-to-js](https://github.com/scopsy/await-to-js) 。
  > - 对于一些简单的情况，比如调接口前将 `loading` 设置为 `true`，可以在 await 函数调用处紧跟 `catch` 将 `loading` 设置为 `false` ，代码示例：`await asyncFn().catch(() => { loading = false })`

- 多个 `await` 命令后面的异步操作，如果不存在继发关系，最好让它们同时触发 ( 👀 即：`Promise.all` / `Promise.allSettled` )
...
- `<script>` 标签中的运行代码，**整个 script 标签是个宏任务**
  > [!NOTE]
  > 说 `<script>` 本身是一个宏任务，是因为<font color=red>它是事件循环系统启动的入口点</font>。<font color=lightSeaGreen>它负责执行初始的全局代码，并在运行过程中产生和规划后续的宏任务与微任务，从而让整个 JavaScript 的异步架构得以有序运作</font>。
  > 
  > 摘自：[在 JS 中，为什么说一个 `\<script>\</script>ˋ 本身就是一个宏任务？](https://gemini.google.com/share/0ddb310dc4b2)
...
- <font color=fuchsia>事件触发的回调函数</font>：例如 <font color=red>DOM Events</font>、<font color=red>I/O</font>
  ...
  > [!NOTE]
  > 之前这里是有 rAF 的，但是感觉争议性较大；所以问了下 Claude，得到如下结果：既不是宏任务，也不是微任务
  >
...
- DOM 操作任务源 ( The DOM manipulation task source )：如元素以非阻塞方式插入文档
  > [!NOTE]
  > 该说法见 [whatwg - 8.1.7.4 Generic task sources](https://html.spec.whatwg.org/multipage/webappapis.html#generic-task-sources)
...
> **关于 fetch 为什么是宏任务**
>
> fetch 不是一个单纯的（返回一个） promise。关键在于，fetch 会发起一个网络请求，当请求被响应时，怎么通知主线程来处理回调? 在 XHR 中，是将回调放入对应于网络请求的任务队列中，fetch 是否也是这样呢？
>
...
>   
>   This [task source](https://html.spec.whatwg.org/multipage/webappapis.html#task-source) is used for features that <font color=lightSeaGreen>react to user interaction</font>, for example keyboard or mouse input.
...
>
>   This [task source](https://html.spec.whatwg.org/multipage/webappapis.html#task-source) is used to queue tasks involved in [navigation](https://html.spec.whatwg.org/multipage/browsing-the-web.html#navigate) and [history traversal](https://html.spec.whatwg.org/multipage/browsing-the-web.html#apply-the-traverse-history-step).
>   
> - The rendering task source
>
>   This [task source](https://html.spec.whatwg.org/multipage/webappapis.html#task-source) is used solely to [update the rendering](https://html.spec.whatwg.org/multipage/webappapis.html#update-the-rendering).
...
上图是对的，微任务 先于 宏任务 执行也是对的；因为 `<script>` 标签就是一个宏任务。要先执行上下文，这就是所谓的第一个宏任务；宏任务就是浏览器事件的 callback 。

可以理解为：每一个宏任务都可能会包含一个或多个微任务，所以，<font color=fuchsia>**在当前宏任务结束之前，要执行本次宏任务中的微任务**；接着再去执行下一个宏任务</font>。

> [!warning]
> 
> 上面高亮的句子，有点重要，重做了 [[JS面试题#第二题]] 后，发现自己有所遗忘。有两个包含多个微任务的 setTimeout，但一定是先执行完前一个 setTimeout 的所有微任务，才会执行后一个 setTimeout
...
高优先级任务的这三种 ( `Promise.then` 、`MutationObserver` 、`Object.observe` ) 也很好理解，MutationObserver 和 `Object.observe` 都是监听某个对象的变化的，变化是很瞬时的事情，肯定要马上响应，不然可能又变了，Promise 是组织异步流程的，异步结束调用 then 也是很高优先级的。
...
而 <font color=dodgerBlue>**Node.js 任务宏任务之间也是有优先级的**</font>，比如<font color=red>定时器 Timer 的逻辑就比 IO 的逻辑优先级高，因为涉及到时间，越早越准确</font>；而 close 资源的处理逻辑优先级就很低，因为不 close 最多多占点内存等资源，影响不大。于是就把<font color=dodgerBlue>**宏任务队列拆成了五个优先级**</font>：<font color=red>Timers、Pending、Poll、Check、Close</font>。
...
( Node ) 除了宏任务有优先级，微任务也划分了优先级，多了一个 `process.nextTick` 的高优先级微任务，在所有的普通微任务之前执行
...
2. 执行 microtask 检查点。简单说，<font color=lightSeaGreen>会执行 microtask 队列中的所有 microtask</font>，直到队列为空。如果 microtask 中又添加了新的 microtask，直接放进本队列末尾。
...
| 基本的请求能力                       | ✅     | ✅       |
| 基本的获取响应能力                     | ✅     | ✅       |
| <font color=red>监控请求进度</font> | ✅     | ❌       |
| <font color=red>监控响应进度</font> | ❌     | ✅       |
...
> 总结：
> 
> - 除了 “监控请求进度” （比如 “跟踪上传进度” ）这一功能无法做到外，fetch 几乎是全能的。
> - XHR 除了可以 “监控请求进度”（通过 `xhr.onprogress` 等事件实现，具体可见 [现代 JS 教程 - XMLHttpRequest # XMLHttpRequest 基础](https://zh.javascript.info/xmlhttprequest#xmlhttprequest-ji-chu) ） 和 取消请求（通过 `xhr.abort()` 实现）
...
Map 和一般 JS 对象最大的区别是：<font color=fuchsia>Map 的键可以是任意类型，且不会被转换</font>；而对象只能使用 string 和 symbol；同时，<font color=red>Map 一定是可迭代的，而对象不是</font>（一般对象没有实现可迭代协议）。
...
- 对 对象 或者 数组中的对象赋值为空，对象中的对象不会被内存回收
...
类似的：weakSet 只支持 `add`、`has`、`delete` 方法，不支持 `size` 和 `keys()`
...
2. <font color=FF0000>为</font> 步骤1 <font color=FF0000>新创建的对象添加属性 `__proto__`</font>，<font color=fuchsia>**将该属性链接至构造函数的原型对象 **</font>；
...
2. 使用指定的参数调用构造函数 `Foo`，并将 this 绑定到新创建的对象。
    `new Foo` 等同于 `new Foo()` ，也就是没有指定参数列表，不带任何参数调用的情况。
...
- `call` ：可以添加（多个，假设有 n 个）参数，第一个参数是借用函数的对象 `thisArg` ，后面的 n-1 个参数是被借用对象的函数的参数（ ⚠️ 除了 `thisArg` ，其他的参数是可选的）。立即执行
...
> [!NOTE]
> 
> 经过实验（如下）后发现：这种说法 （ `bind` 函数相当于：希望一个函数 “永久的”、“显式的” 绑定到一个对象上）似乎并不很准确，因为 `bind` 只是返回一个函数，需要手动接住。<font color=red>而如何接住是调用者的事情了</font>，可以用一个函数变量 `const fn = boundFn.bind(bindObj)` ，但这样 `fn`  并没有绑定到 `bindObj` 下。需要手动绑定  `bindObj.fn = boundFn.bind(bindObj)` 才行
>
> ```js
...
> [!NOTE]
> 
> 顺带一提：在 JS 某一个代码块中出现语法错误，不会有任何输出的，示例如下；因为语法错误是在预编译时就已经发现了，然后报出，完全不会运行；创建执行上下文都无法成功
> ```html
> <script>
>   console.log(123)
>   let a = 1
...
- `window.addEventListener("error")`
  ...
  > [!NOTE] 
  > 相当值得注意的是：<font color=fuchsia>`error` 事件是不会冒泡的</font>，即：通过冒泡无法监听，只能通过捕获监听；所以在使用 `EventTarget.addEventListener` 监听 error 事件时需要加上 `useCapture` 为 `true` ，或者 `options` 对象中设置 `capture` 为 `true` 。代码示例如下：
  >
  > ```js
...
> [!NOTE]
> 
> 就是前端监控上报的内容，上报可以通过 xhr ( 比如 fetch + fetch 自带的 `keepalive` 选项，和已经不推荐、兼容性差的 “同步XMLHttpRequest”)，`new image()` ，`Navigator.sendBeacon()` 等方式，也可以用一些现成的解决方案，比如：Sentry。 更多内容详见 [[Web相关#埋点 & 监控 能做什么]] 中的 “异常监控”
...
- **`encodeURIComponent`** 是 <font color=FF0000>**对 URI 的组成部分进行转义**</font>，所以一些特殊字符也会得到转义。
  ...
  > > [!NOTE]
  > > 
  > > [[#字符 & URI 转义方法#最佳实践|最佳实践]] 中也有类似的说法
  >
...
- <font color=red>CJS 模块输出的是 **单个值的拷贝**</font>，而 <font color=red>ESM 输出的是**（多个）值的引用**</font>
  ...
  > [!TIP]
  >
  > 这里 ESM 输出的 “值的引用”，更准确的说：这叫 <font color=fuchsia>“**live bindings**”</font> （ MDN 译为 “动态绑定”）。这也<font color=fuchsia>是 JS 中唯二的两个存在 “值的引用” 机制（即：使用同一块内存空间）的地方</font>（另一个见下面有介绍的非严格模式的 `arguments`。至于其他地方，哪怕是函数参数传递都是复制一份新的空间存储（对象的）地址（即：值传递），而不是引用传递、直接把原地址传过去）。也正是因为该机制，引入了 “消费者 / importee 可能会直接修改原数据” 的风险。所以，对于被导出的数据，务必将其定义为常量 ( constant )，而非变量；这样杜绝了被修改导致数据错乱的风险
  >
  > > [!warning]
  > > 另外，**相当值得注意的是** ：<font color=fuchsia>live bindings 在中文语境没有一个共识性的翻译</font>，所以渡一将其翻译为 “符号绑定”（参见 [带你对比分析一下，具名导入是在解构吗？【渡一教育】- 第 130秒开始]( https://www.bilibili.com/video/BV1M94y1V7jZ/?t=130) ），但是实际上 Google 搜索 “ JS 符号绑定” 是没有相关内容的；所以不要面试或者与人交谈不要说 “符号绑定” ，要说也是 “live bindings” 或者 MDN 的译法 “动态绑定”
  >
  > > [!note]
  > > 在 [严格和非严格模式下JS输出的结果分别是？【渡一教育】](https://www.bilibili.com/video/BV1cFL7z3EW8) 中有提及：存在另一个 live bindings 的情况，就是在非严格模式下的 `arguments`。代码示例如下：
  > >
  > > ```js
  > > function func(a) {
...
- <font color=red>CJS 模块是 **运行时加载**</font>，而 <font color=fuchsia>ESM 是 **编译时输出接口**</font>，<font color=red>使得对 JS 的模块进行静态分析 Tree Shaking 成为了可能</font>；
...
- 关于两个模块<font color=dodgerBlue>互相引用</font>的问题，<font color=red>**在 ESM 中，是支持加载 CJS 模块的**</font>。但反过来，<font color=red>CJS 并不能 require ES6 Module</font> ，在 Node 中，两种模块方案是分开处理的。
  ...
  > [!TIP]
  >
  > 在 Joyee Cheung 等 Node 开发者的努力下，`require(esm)` 特性，已经在可以在 **v20.19.0+**、 **v22.12.0+**、 **v23.0.0+** 默认启用，并且在 v25.4.0+ 版本稳定。
  > 
  > > The `require(esm)` feature is **enabled by default** in Node.js **v20.19.0+**, **v22.12.0+**, and **v23.0.0+**, and is **stable** in v25.4.0+. It is not supported in Node.js 18.x or earlier versions.
...
此时开始第一步 <font color=red>“**模块解析**”</font> ，浏览器会将相对路径转换为绝对路径，从而找到 `main.js` ，并开始下载。下载完成后，会进入 `main.js` 里面查看是否依赖其他模块，会去寻找所有
<font color=red>**顶层静态依赖**</font> （在循环、判断里面的依赖，以及动态依赖都不会找），并会将其全部提升（也就是 hoisting ）
...
同理，会下载 foo 和 bar 这两个模块，直到其下载完；以此类推，下载完入口及其子孙的所有依赖。

下载完之后就是 <font color=red>**模块执行**</font> 了，依然是从入口文件开始执行。此外，当模块执行完了，同时该模块存在默认导出，会将导出结果放到一个缓存表格中（这里存在一个 live bindings 的概念，即导入和导出使用的是同一块内存空间），当后面的模块再依赖则不会重复执行了。遇到动态导入，则同样执行 下载解析、执行 的过程

总结，一共两个步骤：解析模块 和 执行模块

学习自：[ESModule的工作原理【渡一教育】](https://www.bilibili.com/video/BV1dVGgzqE8M)


##### 阮一峰《 Node.js 如何处理 ES6 模块》笔记

###### Node.js 区分 ESM 和 CJS

Node.js 要求 ES6 模块采用 `.mjs` 后缀文件名。也就是说，只要脚本文件里面使用 `import` 或者`export` 命令，那么就必须采用 `.mjs` 后缀名。<font color=red>Node.js 遇到 `.mjs` 文件，就认为它是 ES6 模块，默认启用严格模式</font>，不必在每个模块文件顶部指定 `"use strict"` 。
...
<font color=dodgerBlue>如果这时还要使用 CommonJS 模块</font>，那么<font color=red>需要将 CommonJS 脚本的后缀名都改成 `.cjs`</font> 。<font color=dodgerBlue>如果没有 `type` 字段，或者 `type` 字段为 `commonjs`</font> ，则 <font color=red>`.js` 脚本会被解释成 CommonJS 模块</font>。

<font color=dodgerBlue>总结为一句话</font>：`.mjs` 文件总是以 ES6 模块加载，`.cjs` 文件总是以 CommonJS 模块加载，`.js` 文件的加载取决于 `package.json` 里面 `type` 字段的设置。


###### CommonJS 模块加载 ES6 模块

CommonJS 的 `require()` 命令不能加载 ES6 模块，会报错，只能使用 `import()` 加载。
...
ES6 模块的 `import` 命令可以加载 CommonJS 模块，但是<font color=red>只能整体加载</font>，不能只加载单一的输出项。
...
这是因为 ES6 模块需要支持静态代码分析，而 <font color=fuchsia>CommonJS 模块的输出接口是 `module.exports` ，**是一个对象**</font>，无法被静态分析，所以只能整体加载。

> [!NOTE]
> 
> 只能整体导入，通过解构获取了；不过，这显然失去了 Tree Shaking

###### 同时支持两种格式的模块

一个模块同时要支持 CommonJS 和 ES6 两种格式，也很容易。

<font color=dodgerBlue>如果原始模块是 ES6 格式</font>，那么需要给出一个整体输出接口，比如 `export default obj`，使得 CommonJS 可以用 `import()` 进行加载。

<font color=dodgerBlue>如果原始模块是 CommonJS 格式</font>，那么可以加一个包装层。
...
<font color=dodgerBlue>**另一种做法是**</font> 在`package.json`文件的`exports`字段，指明两种格式模块各自的加载入口。
...
摘自：[Node.js 如何处理 ES6 模块](https://www.ruanyifeng.com/blog/2020/08/how-nodejs-use-es6-module.html)
...
> <font color=dodgerBlue>When an argument is **passed by value**</font>, the value is copied into a local variable (a named argument and, in ECMAScript, a slot in the arguments object). <font color=dodgerBlue>When an argument is **passed by reference**</font>, the location of the value in memory is stored into a local variable, which means that changes to the local variable are reflected outside of the function.
...
> [!TIP]
> 
> For years, JavaScript has had exactly one meaningful directive, "use strict". It is standardized, enforced by runtimes, and <font color=red>**behaves the same in every environment**</font>. It represents a clear contract between the language, the engines, and developers.
> 
> 摘自：[Directives and the Platform Boundary](https://tanstack.com/blog/directives-and-the-platform-boundary)
...
3. 通过 `Object.getOwnPropertyDescriptor(Object, 'prototype')` 可以发现：通过 `class` 创建的 `Cls` 的 `prototype` 的属性描述符中，`configurable`、`writable`、`enumerable` 均为 `false` 。由于 `writable` 为 `false` ；所以，直接通过赋值修改 `prototype` 是没法成功的（ 即：`Cls.prorotype = newProto` 无法生效）。同时，普通函数的 `prototype` 属性则是 `writable: true`，所以 ES5 形式的构造函数 `function Fn() {}` 的 `Fn.prototype = newProto` 是成立的
   > [!NOTE]
   > 本点在原视频上没有，是从 [面试拷打系列：基础知识拷打 【渡一教育】](https://www.bilibili.com/video/BV1ox7f63Eoq) 处看到，并补充在这里的
4. 通过 class 创建的对象实例中，方法 和 访问器属性 都是不可枚举的。所以需要在定义时，通过 `Object.defineProperty` 定义，并将 `enumerable` 设置为 `false`
   > [!NOTE]
   > 访问器属性不可枚举的说法来自 [如何对class降级【渡一教育】](https://www.bilibili.com/video/BV1WYtqzYEwy)
   
   > [!TIP]
   > 访问器属性，具有类似 “响应式” 的特性，示例代码如下：
   > 
   > ```js
   > const cart = {
   >     items: [100, 200, 300],
   ...
   > ```
   > 
   > 此外，访问器属性具有 “惰性计算” 的特性，只有在被调用时才会运行。Vue 的<font color=red>**计算属性**</font>就是借鉴 “访问器属性” 的思想，<font color=red>`computed` 内部其实就是包装了一个 getter 函数</font>。
   > 
   > 学习自：[妙啊！Js的对象属性居然还能这么写](https://juejin.cn/post/7543300730116325403)

5. 通过 class 创建的对象实例中，方法是不能通过 `new` 来调用的
...
>
> > <font color=red>通过 `class` 创建的函数具有特殊的内部属性标记 `[[IsClassConstructor]]: true`</font> 。因此，它与手动创建并不完全相同。
> >
> >
...
const a = A() // 直接调用，会报错
const a = new A() // console.log(a) 为 "A {}"

console.log(a)
```

这里说明了一个问题：通过 `class` 定义的类必须要通过 `new` 调用；同时，必须返回一个对象，这点可以通过 [[JS 函数手写实现#new 运算符]] 的手写查看
###### 问题三
...
在  [[JS 函数手写实现#new 运算符]] 的手写最后一行写的很清楚：会检查 `fn.call(newObj, ...args)` 的返回值是否是对象，如果是，则返回这个返回值，否则返回 `newObj`

学习自：[面试拷打系列：基础知识拷打 【渡一教育】](https://www.bilibili.com/video/BV1ox7f63Eoq)
#### Proxy
...
- 在 **类** 中：
  ...
  > <font color=fuchsia><font size=4>**this 在 类 中的表现与在函数中类似**</font>，因为 <font size=4>**类 本质上也是函数**</font></font>，但<font color=dodgerBlue>也有一些区别和注意事项</font>。
  >
  > <font color=FF0000>在类的构造函数中，this 是一个常规对象</font>。类中所有<font color=red>非静态的方法</font>都会被添加到 this 的原型中
  >
  > <font color=dodgerBlue>**派生类中的 this**</font> ：不像基类的构造函数，<font color=red>派生类的构造函数没有初始的 this 绑定</font>。在<font color=fuchsia>构造函数中调用 `super()` 会生成一个 this 绑定</font>，并相当于执行如下代码，Base 为基类：
  >
  > ```js
  > this = new Base();
  > ```
  ...
  > ***
  >
  > 在 **静态方法** 中，this 表示 **类本身（构造函数）**，而并非 *类的实例*
  >
  > 可以参考：https://gemini.google.com/share/142394c9ae8b
...
> ***
> 26/2/22补充：上面的 Node 中的全局对象 `global` 是对的，还以为 `globalThis` 会更准确些，不过问了 Gemini [`global` 和 `globalThis` 两者的区别](https://gemini.google.com/share/2f6085513b4b)，得到了如下解释：
> > - **`global`**：这是 **Node.js 专属**的全局对象。它作为 Node.js API 的一部分存在，用于在 Node 宿主环境中挂载全局变量（例如 `process`, `Buffer`, `__dirname` 等）。<font color=red>如果在浏览器中直接调用 `global`，会报错 `ReferenceError: global is not defined`</font>。
> >
> > - **`globalThis`**：这是 **ECMAScript 2020 (ES11) 引入的官方标准**。它提供了一个标准的、跨环境的方式来访问全局的 `this` 对象。无论你的代码运行在浏览器、Node.js 还是 Web Worker 中，`globalThis` 都会自动指向当前环境的那个真正的全局对象。
> 
> 这里说 `global` ，感觉没什么问题。
...
- <font color=fuchsia size=4>**this 是在运行时被绑定的**</font>（ 👀 即是 动态的）
  > [!TIP]
  >   根据 [this 指向【渡一教育】 ](https://www.bilibili.com/video/BV1ha3kzDEVu) 中的说法：
  > 
  > > this 取决于执行上下文，所以，<font color=fuchsia>不执行没法确定 this 指向</font>。
  > > 
  > > 箭头函数没有 this，它的 this 是从执行上下文获取的，这样的行为和闭包没什么区别。而<font color=red>闭包是基于词法作用域的</font>（是一种静态作用域，依赖的是定义，而非运行），所以箭头函数的 this 是基于定义的位置，而不是运行。
##### this 的绑定规则
...
<font color=red>**JavaScript 所有的函数都可以使用 `call` 和 `apply` 方法**</font>（和 `prototype` 有关）。<font color=dodgerBlue>这两个函数的第一个参数都要求是一个对象，这个对象的作用是什么呢？</font> 就是 <font color=red>给 this 准备的</font>。在调用这个函数时，会将 this 绑定到这个传入的对象上。
...
有时，我们会调用一些 JavaScript 内置函数，或者一些第三方库中的函数。<font color=LightSeaGreen>这些函数会要求我们传入另外一个函数，自己并不会显示的调用这些函数，而是 JavaScript 内部或者第三方库内部会帮助我们执行</font>。这些函数中的 this 又是如何绑定的？示例如下：
...
2. 这个新对象会被执行 prototype 连接 
   ...
   > 👀 上面两步可以通过 `Object.create(prototype)` 来实现
...
- new 绑定 优先级高于 bind
  ...
  // 会报错。new绑定 和 call、apply 不允许同时使用
...
除了 `bind` 以外，<font color=fuchsia size=4>**new 绑定 和 `call`、`apply` 不允许同时使用**，所以不存在谁的优先级更高</font>
...
typeof 一般被用于判断一个变量的类型，可以利用 typeof 来判断 number , string , object , boolean , function , undefined , symbol 、BigInt 这八种类型。很遗憾的一点是，typeof 在判断一个 object 的数据的时候只能告诉我们这个数据是 object， 而不能细致的具体到是哪一种 object。（ 👀 参见：[[JavaScript备忘录#关于 typeof 判断类型的不足 解决方法的补充]]）。要想判断一个数据具体是哪一种 object 的时候，需要利用 instanceof 这个操作符来判断

> [!NOTE]
>
> 本文档中没看到 JS 的八种类型，这里补充一下：
>
> Undefined、Null、Boolean、String、Number、BigInt、Symbol、Object
...
> [!NOTE]
> 长时间不看这张图，再看有点懵，感觉可以以 ① ② ③ 这三点作为抓手 / 入口
...
>  根据上图的一些想法：
>
> 构造函数（比如 `function Foo() {}` ）是有 `prototype` 属性的，用来指向 “生成新对象 所绑定的” 原型对象；但它本身也是一个函数（实例），所以它的 `__proto__` （比如 `Foo.__proto__`）是 `Function.prototype` （即：`Foo.__proto__ === Function.prototype`）。类似的： `function Object() {}` 同样是一个构造函数，根据上面的说法： `Object.__proto__ === Function.prototype` 也是在情理之中的。此外，`Function.__proto__ === Function.prototype` 也是同样的道理
> 
...
// 虽然感觉在搞懂 Object 和 Function 的关系（或者搞懂 instanceof 的原理）之后，这些都是可以推导出的，难度不大。另外，甚至可以直接收：Object 本身也就是一个构造函数
...
<font color=FF0000>每个函数都有一个 `prototype` 属性</font>，就是我们经常在各种例子中看到的那个 prototype。另外，<font color=fuchsia>`prototype` **是函数才会有的属性**</font>。如下示例：
...
所以，实例对象和构造函数都可以指向原型（👀 分别通过 `__protot__` 和 `prototype`）

> [!NOTE]
> 
> 根据 [这些简单的js基础面试题你会多少？【渡一教育】](https://www.bilibili.com/video/BV177xkzwEej) 的说法：`__proto__` 被称为 “隐式原型” ，`prototype` 为构造函数的原型；“隐式原型” 指向构造函数的原型
...
> [!TIP]
>
> 可以在 Chrome DevTools 打印一下 `Person.prototype` ：
>
> ```js
> function Person() {}
...
闭包很有用，因为<font color=FF0000>它允许将函数与其所操作的某些数据（环境）关联起来</font>。这<font color=FF0000>显然类似于面向对象编程</font>。在面向对象编程中，对象允许我们将某些数据（对象的属性）与一个或者多个方法相关联。

> [!NOTE]
> 这里和 OOP 类似的说法，示例参见 [[#后盾人视频的“闭包”讲解#构造函数与闭包]]
...
2. <font color=fuchsia>对 **外部词法环境** 的引用</font>，与外部代码相关联。
   ...
   > [!NOTE]
   >  
   > 由下面可知，这就是通过 `[[Environment]]` 内部槽 ( internal slot ) 保存了 “对创建该函数的词法环境的引用”
...
> ###### 词法环境是一个规范对象
>
> <font color=fuchsia>“词法环境” 是一个规范对象</font> ( specification object )：它仅仅是存在于 编程语言规范 中的 “理论上” 存在的，用于描述事物如何运作的对象。我们无法在代码中获取该对象并直接对其进行操作。
>
...
<font color=fuchsia size=4>**所有的函数在“诞生”时都会记住创建它们的词法环境**</font>。从技术上讲，这里没有什么魔法：<font color=red>**所有函数都有名为 `[[Environment]]` 的隐藏属性**</font>（ 👀 internal slot ），<font color=fuchsia size=4>**该属性保存了「对 “创建该函数” 的词法环境的 引用」**</font>（ 👀 即上面说的“词法环境对象 组成的另一部分”）。
...
> [!NOTE]
> 
> 即，如果一个函数还有可能被使用，则不会被回收销毁；则它以及它可达的这条链上的 词法环境 将不会被销毁
...
> [!TIP]
> 
>  这个说法之前没有听说过，有点重要。不过，并不完整，更完整的说法是：
>
...
执行上下文是 JS 引擎执行一段可执行代码时维护的运行状态记录。按代码入口理解，常见来源包括：

- **Script / 全局代码执行上下文**：普通脚本顶层代码执行时创建，顶层 `var`、函数声明等会进入全局环境。
- **Module 执行上下文**：ES Module 顶层代码执行时创建，模块顶层声明属于模块环境，不会直接挂到全局对象上。
- **函数执行上下文**：每次函数调用都会创建新的函数执行上下文，用于保存函数调用过程中的词法环境、变量环境、`this` 等状态。
- **Eval 执行上下文**：执行 `eval()` 代码时创建；由于它会影响作用域分析和优化，实际开发中应慎用。

> 💡 《JS高程 第四版》的 4.2.1 作用域链增强 ( P89 ) 中也有提到执行上下文相关内容。
>
> Worker、iframe、Service Worker 等属于宿主环境提供的独立全局环境 / Realm / 浏览上下文；其中运行 JS 时，仍会进入 Script / Module / Function / Eval 等执行上下文。Promise 回调作为微任务执行，回调函数运行时创建的是函数执行上下文；`with` 影响的是词法环境链；原型链负责属性查找，不属于执行上下文。

在同一个 JS agent 中，JS 处理多个执行上下文的方式是通过“栈”的形式。
...
1. **创建阶段**：会创建：
   ...
   >
   > ⚠️ 值得注意的是：在 ES6 及以后，更常使用 **词法环境** ( Lexical Environment )、**变量环境** ( Variable Environment )、**环境记录** ( Environment Record ) 和 **outer 引用** 来解释作用域与标识符查找；旧教程里的 VO / AO / 作用域链是偏 ES3 / ES5 的讲法。
...
在执行上下文中记录了代码执行过程中的状态信息，<font color=lightSeaGreen>按代码入口理解，执行上下文常见来源包括：Script / 全局代码执行上下文、Module 执行上下文、函数执行上下文、Eval 执行上下文</font>（ 👀 [[#技术蛋老师的视频讲解]] 中有详细讲解，这里略）
...
**有了执行上下文，就要有<font color=FF0000>合理管理它的工具</font>**。而 <font color=fuchsia>**执行上下文栈 ( Execution Context Stack ) 是用来 <font size=4>管理执行期间创建的所有执行上下文的数据结构</font>**</font>，它是一个 LIFO（后进先出）的栈，<font color=FF0000>它也是我们熟知的 **JS 程序运行过程中的 调用栈**</font>。 <font color=fuchsia>**程序开始运行时，会先创建入口代码对应的执行上下文（普通 script 通常是全局 / 脚本执行上下文，ES Module 则是模块执行上下文）并压入执行栈；之后每当有函数被调用，都会创建新的函数执行上下文并压入栈内**</font>。
...
继续运行 `bar` 函数，<font color=FF0000 size=4>由于函数上下文内有 `outer` 引用实现层层递进引用，因此**在 `bar` 函数内仍可以获取到 `console` 对象并调用 `log`**</font>

> [!NOTE]
> 
> 为防止忘记，提醒下（上面也有说到）创建全局执行上下文时，会初始化一些全局对象或全局函数，包含 console 
...
- **<font color=FF0000>模块环境 ( ModuleEnvironment )</font>** ：ES Module 会创建模块环境，<font color=fuchsia size=4>**模块中的顶层声明、`import` 引入的绑定、`export` 导出的绑定，都会由模块环境记录管理**</font>。<font color=FF0000>模块环境的 outer 引用指向全局环境</font>。
...
- <font color=red>**object ER** 可以由 with 语句触发的</font>

  > [!NOTE]
  > 根据 GPT 5.5 的说法：Object ER 除了 with 语句，还有 “**全局环境中的对象部分**”
  > 
  > 全局环境 Global Environment Record 不是单纯的 Object ER，但它内部包含一个 Object ER，绑定对象就是全局对象，比如浏览器里的 globalThis/window。
...
另外，值得补充的是：在 Vue2 Options API 中，<font color=lightSeaGreen>this 是指向组件实例的</font>，因为 组件配置对象 和 组件实例 是两个东西 ，所以 this 显然不会指向组件配置对象。同时，`methods` 中定义的内容在生成 组件实例 时会通过 `bind` 绑定到组件实例上 ( `methods.method.bind(vm)` )，这也是为什么 `methods` 中的方法不允许使用箭头函数，是因为会导致 `bind` 失效；同时，因为 `data` 是一个工厂函数的原因，所以 `data` 选项不需要 `bind` 。所以，在写 Vue2 时，会发现有人会在 `data` 中定义作为组件 prop 值的方法的情况，可能就是这个原因。
...
> [!NOTE]
> 
> 在 Vue3 中 `$on` 、`$off`、`$once` 被移除，虽然事件总线未获得官方推荐，但不失为一种方法
> 
> > 在绝大多数情况下，不鼓励使用全局的事件总线在组件之间进行通信。虽然在短期内往往是最简单的解决方案，但从长期来看，它维护起来总是令人头疼。
> > 
> > [Vue 3 迁移指南 - 事件 API # 事件总线](https://v3-migration.vuejs.org/zh/breaking-changes/events-api.html#%E4%BA%8B%E4%BB%B6%E6%80%BB%E7%BA%BF)
...
>
> 摘自：[internet archive - Vue3官方文档 -  实例 property - $options](https://web.archive.org/web/20220713032137/https://v3.cn.vuejs.org/api/instance-properties.html#options)
...
> [!NOTE]
> 关于这里为什么要用 `call(this)` ，是为了防止 data 选项中使用了 this。在实际开发中，经常会在 data 中引用 props 传过来的初始值，或者引用 $route、某些方法等。这时候如果不是用 `call(this)`，就会直接抛出 “TypeError: Cannot read properties of undefined” 的错误。代码示例如下：
> 
> ```js
> export default {
>   props: ['defaultName'],
>   data() {
>     return {
>       // 依赖了 this.defaultName
>       userName: this.defaultName 
>     }
>   },
>   methods: {
>     reset() {
>       // ❌ 这里会报错，因为执行 data() 时，内部的 this 是 undefined
...
>
>
> 可以参考一下 [与 Gemini 的对话](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221GfdNnEfuiSIfASMEn1ylnJJfLsrTYAv7%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)
...
```vue
<template>
  <!-- 通过 @hook:updated 监听组件的updated生命钩子函数 -->
...
- `.stop` ：和 `Event.stopPropagation()` 作用一样，阻止事件继续传播

- `.prevent` ：和 `Event.preventDefault()` 作用一样，阻止默认行为。比如 `submit` 事件触发后默认重新加载页面，而加上后则不会再重新加载

- `.capture` ：<font color=FF0000> 添加事件监听器时使用事件捕获模式</font>，即：内部元素触发的事件先在外部处理，然后才交由内部元素进行处理

- `.self` ：<font color=FF0000> 只当在 `event.target` 是当前元素自身时触发处理函数</font>

- `.once` ：事件只会触发一次

- `.passive` ：<font color=FF0000>Vue 为 `EventTarget.addEventListener` 中的 `passive` 选项添加了 `.passive` 修饰符</font>。<font color=fuchsia> 事件的默认行为将会 <font size=4>**立即触发**</font>，而不会等待绑定的事件完成</font>。在 `addEventListener` 中，`passive` 为 `true` 表示：`listener` 永远不会调用 `preventDefault()` 
...
> [!NOTE]
> 
> 官方文档的解释有点费解，一段时间后很容易忘记；下面是我的理解，做下笔记：
>
> 如下 BaseInput 组件带有一个 focus 事件，但是：作为一个 「自定义组件」不应该自带一个 focus 事件；这个 focus 事件应该是 BaseInput 中的 `<input>` 原生组件自带的（这里只是假设是 `<input>` 组件，也有可能是其他带有 focus 事件的原生组件）。所以：正常来说，这个BaseInput 自定义组件是无法直接监听 focus 原生事件的，所以需要加上 `.native` 修饰符。
...
> [!WARNING]
> 
> <font color=FF0000>**Vue2 组件中没有规范定义 `emits` option**</font>
>
> 另外，值得补充的是：2024/11/8 在修改一个 uni-app + vue2 的项目时，发现 `<van-button @click.stop="onClick">` 的 `.stop` 始终无法生效，哪怕在 `onClick` 中调用 `event.stopPropagation()` 也不行，总之让人非常困惑，问了 GPT ，给出的建议是在外层套一个 `<view>` 对其添加 `@click.stop` ，试了下，确实生效了。然后想到了：能不能用 `<van-button @click.native.stop="onClick">` ？试了下，也确实是可以的。
...
document 是一个组件，接收一个 prop `title` 用于展示；现在要在父组件上修改 title ：document 子组件使用和以往一样，单向数据流，emit 触发 `update:title` 事件 (  `this.$emit('update:title', 'newVal')` )
...
`Vue.extend()` 是Vue 组件实例化中一个非常重要的方法，就是用于生成组件的构造函数。
...
<font color=dodgerBlue>过滤器可以串联：</font>
...
从 Vue 3.0 开始，过滤器已移除，且不再支持。虽然过滤器看起来很方便，但它需要一个自定义语法，打破了大括号内的表达式“只是 JavaScript ” 的假设，这不仅有学习成本，而且有实现成本。<font color=lightSeaGreen>建议用方法调用或计算属性来替换它们</font>
...
<font color=FF0000>**让一个对象可响应**</font>。<font color=FF0000>**Vue 内部 会用它来处理 data 函数返回的对象**</font>。
...
学习自：[（完整版）快速掌握Vue2响应式原理【Vue】](https://www.bilibili.com/video/BV1VA411x76D) 具体是在第三集
...
在初始化 Vue 的每个组件时，会对组件的 data 进行初始化，就会由普通对象变成响应式对象；<font color=FF0000>**在这个过程中便会进行 “依赖收集” 的相关逻辑**</font>，如下所示：
...
function defieneReactive (obj, key, val) {
  ...
  Object.defineProperty(obj, key, {
    ...
    get: function reactiveGetter () {
      if (Dep.target) { // 这里的 `target` 是一个静态属性，下面 Dep 的定义中有介绍
        dep.depend();
        // ...
      }
      return val
...
Dep 是一个 class ，其中有一个关键的<font color=fuchsia>静态属性 `target` ，**它指向了一个全局唯一 Watcher**，**保证了同一时间全局只有一个 watcher 被计算**</font>，另一个属性 `subs` 则是一个 Watcher 的数组，所以 Dep 实际上就是对 Watcher 的管理

> [!NOTE]
> 上面的 `Dep.target.addDep` 中的 `addDep` 确实可以从 [[#Watcher]] 中找到定义
...
在实例化 Vue 时，依赖收集的相关过程如下：初始化状态 `initState` ， 这中间便会通过 `defineReactive` 将数据变成响应式对象，其中的 getter 部分便是用来依赖收集的。 初始化最终会走 mount 过程，其中会实例化 Watcher ，进入 Watcher 中，便会执行 `this.get()` 方法，<font color=red>get 方法中的 `pushTarget` 实际上就是把 `Dep.target` 赋值为当前的 watcher</font>。
...
1. <font color=FF0000><font size=4>**Observer**</font> 对需要 observe 的数据对象进行递归遍历，包括子属性对象的属性，都加上 setter 和 getter</font> 。这样的话，给这个对象的某个值赋值，就会触发 setter，那么就能监听到数据变化。
   > [!NOTE]
   > 上面介绍的 [[#Dep]] 和 [[#Watcher]] 都分别存在于 observer 文件夹下，而 Observer 定义在 `observer/index.ts` 下
...
学习自：codingstartup 微信群 群友

***

Vue 2 的 nextTick 是一个带有浏览器兼容降级策略的异步任务队列封装；而 Vue 3 的 nextTick 则是直接基于原生 Promise 和微任务机制的极简实现。

##### Vue3 中 nextTick 的实现原理

Vue 3 的 nextTick 源码非常短，本质上就是利用了原生的微任务：
...
###### 与 Scheduler（调度器）的紧密结合

在 Vue 3 中，响应式数据改变触发组件更新时，更新任务（Job）是被放入一个微任务队列中排队的（通过 `queueJob` ）。

- 当 Vue 调度器开始处理这些 DOM 更新任务时，它本身就是在一个 `Promise.resolve().then()` 中运行的。
  
- 这个正在执行更新的 Promise 被赋给了内部变量 `currentFlushPromise` 。
  
- 当你在代码中调用 `nextTick(fn)` 时，如果 DOM 正在准备更新，Vue 3 会将你的回调函数 fn 挂载到 `currentFlushPromise.then()` 后面。
  
- 根据微任务队列先进先出的执行顺序，你的 nextTick 回调自然就会在 DOM 更新的 Job 执行完毕之后被调用。

##### Vue 2 中 nextTick 的实现原理

Vue 2 的 nextTick 核心原理基于浏览器的**事件循环（Event Loop）机制**，采用的是**“优雅降级”**的异步策略。

###### 核心数据结构

Vue 2 在内部维护了一个回调队列 `callbacks` 和一个状态标志 `pending` ：

- **`callbacks`**：数组。每次调用 `nextTick(cb)` 时，都会把回调函数包装一下放入这个数组中。
  
- **`pending`**：布尔值。用于保证在同一个事件循环中，无论调用多少次 `nextTick`，都只会向任务队列中推入一个异步执行清空队列的任务（即只触发一次 timerFunc）。

###### 异步 API 的优雅降级策略

> [!NOTE]
> 根据 [「2022」寒冬下我的面试知识点复盘【Vue3、Vue2、Vite】篇](https://juejin.cn/post/7166446028266733581) 中的说法：
> 
...
为了兼顾性能（微任务优先于宏任务执行，能更快响应且避免页面闪烁）和浏览器兼容性，Vue 2 定义了一个 `timerFunc` 函数，它会根据当前环境支持情况，按优先级选择合适的异步 API：

1. **Promise (微任务 - 首选)**：如果浏览器原生支持 Promise，就使用 Promise.resolve().then(flushCallbacks)。
   
2. **MutationObserver (微任务 - 备选)**：如果不支持 Promise 但支持 MutationObserver（常见于一些老版本浏览器或 PhantomJS），就通过监听一个文本节点的变化来触发回调执行。
   
3. **setImmediate (宏任务 - 降级)**：如果连微任务都不支持，则退化为宏任务。如果环境（如 IE 浏览器）支持 setImmediate，则优先使用它。
   
4. **setTimeout (宏任务 - 兜底)**：如果以上全都不支持，最后只能使用 `setTimeout(flushCallbacks, 0)` 。

###### 执行流程 flushCallbacks

当异步任务被触发时，会执行 `flushCallbacks` 函数。这个函数会把 `pending` 重置为 false，复制一份现有的 `callbacks` 队列并清空原队列，然后依次遍历执行复制出来的回调函数。

###### 总结

**收集回调 -> 检查 pending -> 根据环境选择最快的异步API (Promise -> MutationObserver -> setImmediate -> setTimeout) -> 异步批量执行。**

摘自：[AI Studio - Vue 中 nextTick 实现原理](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221RqGbbTvI0ytMqgxQnuQKK68i-pGCNbIO%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)
...
Teleport 的实现原理可以概括为：<font color=red>在 **逻辑上** 保留原本的组件树（VNode 树）结构</font>，但 <font color=red>**在物理渲染（DOM 树）**时，将指定的子节点挂载到目标容器中</font>。

###### 核心思想：逻辑树与物理树的分离

在 Vue 的内部，组件之间的关系是通过虚拟 DOM（VNode）树来维系的。

- **逻辑层（VNode 树）**：即使使用了 <Teleport>，它及其内部的子组件在 VNode 树中的位置并没有改变。因此，<font color=red>依赖于组件树上下文的功能（如 Provide / Inject、生命周期、`$parent` 组件实例引用等）依然能按预期正常工作</font>。
  
- **物理层（DOM 树）**：在渲染真实 DOM 时，<font color=red>Vue 的渲染器会 **拦截** Teleport，并改变其内部 DOM 节点的挂载目标</font>。

###### 内部表示 (TeleportImpl 与 ShapeFlag)

在 Vue 的源码（runtime-core）中，Teleport 并不是一个普通的组件，它的<font color=red>底层类型对应着一个特殊的对象 `TeleportImpl`</font> 。

当 Vue 编译模板时，遇到 `<Teleport>` 标签，会为其创建一个 VNode，并打上特殊的 ShapeFlag（即 `ShapeFlags.TELEPORT` ）。

在渲染器的核心函数 patch 中，有一段专门针对 Teleport 的拦截逻辑：

```ts
if (shapeFlag & ShapeFlags.TELEPORT) {
  // 交由 TeleportImpl 自己提供的 process 方法来处理渲染
...
当 Teleport 首次被挂载时，`TeleportImpl.process` 会执行挂载逻辑：

1. **创建占位符（Anchor）**：在原始位置（原本应该挂载的容器 container）插入一个注释节点（Comment Node）作为锚点。这个占位符的作用是为了日后如果 Teleport 被禁用（disabled）或者销毁时，能找到正确的位置。
   
2. **获取目标容器（Target）**：根据 `to` 属性的值（通常是一个 CSS 选择器字符串或真实的 DOM 元素），使用 `document.querySelector` 找到目标 DOM 节点。
   
3. **挂载子节点**：将 Teleport 内部包含的所有子级 VNode，挂载到解析出的**目标 DOM 节点**下，而不是原来的 container 中。
   

如果 `disabled` 属性为 true，则直接将子节点挂载在原始的 container 的占位符后面，不发生传送。

###### 更新过程（Patch）

当触发响应式更新时，`TeleportImpl.process` 会执行更新逻辑：

1. **更新子节点**：依然在当前的 DOM 目标容器中对子节点进行常规的 patch 对比更新。
   
2. **处理 disabled 状态变化**：
   
    - 如果<font color=dodgerBlue>由 `disabled: false` 变为 true</font>，会<font color=red>将已经传送到外部的真实 DOM 节点**移动回**原来的占位符（Anchor）位置</font>。
      
    - 如果<font color=dodgerBlue>由 `disabled: true` 变为 false</font>，会<font color=red>将当前所在位置的 DOM 节点**移动到** to 指定的目标容器中</font>。
    
3. **处理 `to` 目标变化**：<font color=dodgerBlue>如果 `to` 属性的值发生了改变</font>（例如从 `to="#modal-1"` 变为了 `to="#modal-2"` ），Vue 会重新查找新的目标容器，并将当前所有的子 DOM 节点从旧容器**移动到**新容器中。
   
###### 卸载过程（Unmount）

当 Teleport 组件所在的父组件被销毁，或者通过 v-if 被移除时，Vue 会触发卸载流程：

1. 清理逻辑会首先找到 Teleport 的实际挂载目标容器。
   
2. 将目标容器中 Teleport 包含的真实 DOM 子节点一一移除。
   
3. 同时，移除原本在组件树原位置留下的那个占位符（注释节点）。

摘自：[AI Studio - Vue Teleport 实现原理解析](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221OV7Rqi_6XcieSEwrMlO0Tzcpl_hCIzVt%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)
...
虽然看到的真实 DOM 被挂载到了别的地方，但在虚拟 DOM 中，它依旧留在原地。这意味着：
...
摘自：[🔥 面试官：Teleport 是怎么实现的？为啥它能把组件“传送”走？](https://juejin.cn/post/7509033290054631476)
...
这是因为 <font color=FF0000>vue-router 会识别出两个路由使用的是同一个组件从而进行复用，并不会重新创建组件，而且组件的生命周期钩子自然也不会被触发，导致跳转后数据没有更新</font>。那<font color=dodgerBlue>如何解决这个问题呢？</font>

可以为 `<router-view>` 组件添加属性 `key` ，例子如下：
...
1. **<font color=dodgerBlue>不是相同节点</font>（TagName 或 key 不相同）：**
   ...
   > 👀 当前的 vue2 的源码是通过 `sameVnode` 函数返回 boolean 进行判断是否是相同节点，`isSameNode` 通过全局搜索已经找不到了...
  ...
  return (
    a.key === b.key && (
      (
        ...
        isDef(a.data) === isDef(b.data) && /* 👀 isDef 判断是否为非空值，返回值为 bool */
...
1. 旧首节点和新首节点用 `sameNode` 对比。
   ...
   > 👀 感觉原文是有问题的，应该是 same**V**node 进行对比？下面的类似
...
5. <font color=dodgerBlue>如果**以上逻辑都匹配不到**</font>，<font color=fuchsia size4>再 **把所有旧子节点的 `key` 做一个映射到旧节点下标的 `key -> index` 表**</font>，然后 <font color=fuchsia size=4>**用新 `vnode` 的 `key` 去找出在旧节点中可以复用的位置**</font>。

<font color=FF0000>**然后不停的把匹配到的指针向内部收缩**</font>，<font color=fuchsia size=4>**直到新旧节点有一端的指针相遇**</font>（<font color=red>说明这个端的节点都被 patch 过了</font>）
...
- eslint 的 eslint-plugin-react 甚至也专门有一个 rule 叫做 `react/no-array-index-key`
  ...
  > > [!NOTE]
  > > 
  > > 关于 React key & diff 相关，见 [React Doc - Reconciliation](https://reactjs.org/docs/reconciliation.html)
  >
...
- `defineProperty` <font color=fuchsia>只能通过递归劫持对象的属性</font>，而 `Proxy` 是 <font color=fuchsia>代理整个对象，且是惰性的，只会代理对象的第一层</font>
  ...
  > 在 Vue3 中使用 `Proxy` 并不能监听到对象内部深层次的属性变化，因此<font color=red>它的处理方式是在 `[[Get]]` 中递归响应式</font>，这样的好处是：（只有）真正访问到的内部属性才会变成响应式，减少性能消耗。
  >
...
- **`Object.defineProperty` 对新增属性需要手动进行 Observe**
  ...
  <font color=FF0000>由于 `Object.defineProperty` 劫持的是对象的属性，所以新增属性时，需要重新遍历对象，对新增属性再使用 `Object.defineProperty` 进行劫持</font>。也正是因为这个原因，使用 Vue 给 data 中的数组或对象新增属性时，需要使用 `vm.$set` 才能保证新增的属性也是响应式的。
  ...
  <font color=dodgerBlue>如果采用 `Proxy` 实现</font>，<font color=red>`Proxy` 通过 `set(target, propKey, value, receiver)` 拦截对象属性的设置，是可以拦截到对象的新增属性的</font>

- `defineProperty` 不支持 `Map`、`Set` 等数据结构

  > Vue3 对 `Map`、`Set` 做了很多特殊处理，由于 <font color=fuchsia>`Proxy` 无法直接拦截 `Set`、`Map`</font> ，<font color=red>因为 `Set`、`Map` 的方法必须得在它们自己身上调用；**`Proxy` 返回的是代理对象**</font>
  >
...
- `Proxy` 支持 13 种拦截操作，`defineProperty` 只支持 `[[Get]]` 和 `[[Set]]`
  ...
  >
  > Proxy 用来 拦截 和 重定义 一个对象的基本操作，它的拦截是全面的，没有任何遗漏的；而 defineProperty 只是众多基本操作之一。正因为此，Vue2 的监听改变数组长度的操作时，无法直接监听（示例如下），只能通过修改数组原型的方式，在其中夹了一个中间对象类型，方便监听 `push` 等修改数组长度的操作
  > ```js
  > const arr = [1, 2, 3]
  ...
  > > 
  > > > <font color=fuchsia>**使用 Reflect 保证了 this 指向的是 “代理对象”，而非 “原始对象”**</font>
  > > > 
  > > > 学习自：[面试官：你知道 Vue3 的响应式为什么比 Vue2 更强吗？](https://juejin.cn/post/7506501808836853794)
  > > 
  > > 摘自：[Proxy 有哪些东西是无法拦截的？深入解释 Reflect 与陷阱盲区](https://juejin.cn/post/7519356060715270194)
...
Vue3 的 `computed` 本质上是一个 “带缓存的派生状态 ref” ：它会自动收集 getter 里读取到的响应式依赖，依赖没变就直接返回缓存，<font color=red>依赖变了先标记为脏，等下次读取 `.value` 时再重新计算</font>。

###### 核心流程

1. 创建时不立即执行 getter  
    `computed(() => count.value + 1)` 会创建一个 `ComputedRefImpl`，里面保存：
     - `_value`：缓存值
     - `dep`：谁依赖了这个 computed
     - `deps`：这个 computed 依赖了谁
     - `flags`：比如 `DIRTY`，表示是否需要重新计算  
        源码里 `computed()` 最终就是 `new ComputedRefImpl(getter, setter, isSSR)`。见 Vue core 的 [computed.ts](https://github.com/vuejs/core/blob/main/packages/reactivity/src/computed.ts)
    
2. 第一次读取 `.value` 时才计算  
    读取时会做两件事：
	- `this.dep.track()`：让外层 effect 或组件渲染依赖这个 computed
	- `refreshComputed(this)`：如果是脏的，就运行 getter，更新 `_value`

3.  getter 执行期间会收集依赖
    Vue 会把当前正在执行的订阅者设为这个 computed。此时 getter 内部如果读取了 `count.value`、`state.name` 等响应式数据，这些响应式数据的 `dep` 就会记录：“这个 computed 依赖我”。
    
4. 依赖变化时，不会立刻重新计算
    比如 `count.value++`，Vue 会通知依赖它的 computed。computed 的 `notify()` 会把自己标记为 `DIRTY`，并通知依赖这个 computed 的组件或 effect。关键点是：这里只是标脏，不马上跑 getter。
    
5. 下次读取 `.value` 时再重新计算
    如果依赖版本变了，`refreshComputed()` 会重新执行 getter。新值和旧值不同，才更新 `_value` 并递增版本号。Vue 当前实现里还有 `globalVersion` 快路径，用来判断“自上次计算以来有没有任何响应式变化”，避免不必要检查。相关逻辑在 [effect.ts 的 `refreshComputed`](https://github.com/vuejs/core/blob/main/packages/reactivity/src/effect.ts)。


##### 计算属性的回调什么时候执行

当下面两项<font color=fuchsia>同时成立时</font>，计算属性的回调才会执行

1. 读取到计算属性
2. 当前计算属性为脏数据，其中<font color=fuchsia>计算属性默认就是脏的</font>，体现在：哪怕什么都不做，直接打印一个计算属性，就会执行该计算属性的回调。在读取之后，该脏数据就不脏了
	1. 初始化后为脏数据
	2. 读取后，不为脏数据
	3. 计算属性依赖的数据变化，将该计算数据标记为脏数据。与此同时，值得注意的是：如果依赖的数据存在变化，只要不去读该计算属性，那么回调还是不会执行

***

Vue3.5 做了一个改变：watch 的回调（下面的 `depFn` ）执行前，会检查依赖的计算属性是否有变化（调用 [`isDirty`](https://github.com/vuejs/core/blob/325eb1d9a12e3b90fe36f755291b8f9ba5977820/packages/reactivity/src/effect.ts#L353) ），就会刷新一下计算属性的值 ( `refreshComputed` )，也就是<font color=fuchsia>会先运行计算属性的回调，判断计算属性的值本身是否存在变化</font>
...
学习自：[vue3-5的effect优化【渡一教育】](https://www.bilibili.com/video/BV1FYHDzJESe) 以及 [困扰袁老师的vue3.5优化问题破案啦！](https://v.douyin.com/Sia8KO6rFe0/)
...
Vue3.5 中计算属性的实现不再使用 ReactiveEffect，`ComputedRefImpl` 自己实现了 `Subscriber`，并通过 `refreshComputed(this)` 进行依赖追踪与脏值刷新。

> In 3.5, a computed is automatically de-referenced from all its deps when it loses all subscribers (effects or other computeds) so they can be garbage collected.
> 
> 摘自：[Vue issues - 11886 # 尤雨溪的解释](https://github.com/vuejs/core/issues/11886#issuecomment-2348643106)
...
其中 watchEffect 是 computed b 唯一的订阅者

如果 `stop()` 被调用，computed `b` 失去了唯一的订阅者，那么 Vue 3.5 会自动让 `b` 从 `a.value` 的依赖列表中移除。可以理解为：既然没人关心 `b` 的变化，那 `a` 也没必要继续通知 `b`。

我的理解是：因为计算属性是懒加载的，失去订阅者，也就不会再被 get，哪怕计算属性依赖的数据再变化，计算属性也不会对程序产生影响

学习自：[Vue issues - 11886](https://github.com/vuejs/core/issues/11886)

#### 计算属性和函数的区别

> 我们可以将同一函数定义为一个方法而不是一个计算属性。<font color=LightSeaGreen>（计算属性和函数）两种方式的最终结果确实是完全相同的</font>。然而，<font color=dodgerBlue>不同的是</font> <font color=red>计算属性是基于它们的 <font size=4>**响应依赖关系 缓存 **</font>的</font>。<font color=FF0000> 计算属性只在相关响应式依赖发生改变时它们才会重新求值</font>。<font color=FF0000> 这就意味着只要 `author.books` 还没有发生改变</font>，多次访问 `publishedBookMessage` 计算属性会立即返回之前的计算结果，<font color=FF0000> 而不必再次执行函数</font>。
>
> <font color=lightSeaGreen>相比之下，每当触发重新渲染时，调用方法将总会再次执行函数</font>
>
...
>
> 使用 <font color=FF0000> watch 选项允许我们 **执行异步操作** (访问一个 API )，限制我们执行该操作的频率</font>，<font color=FF0000> 并在我们得到最终结果前，设置中间状态</font>。这些都是计算属性无法做到的
>
...
`v-show` 改变元素显隐的核心原理是**通过修改 DOM 元素的内联 CSS 属性 `display` 来控制可见性**。

当绑定的条件为 false 时，Vue 会在元素上添加内联样式 `display: none;`；当条件为 true 时，Vue 会移除 `display: none;`，恢复元素原本的 display 值。

###### 核心共性原理（Vue 2 & Vue 3）

无论是哪个版本，Vue 在处理 `v-show` 时都会面临一个问题：**如果元素原本是 `display: flex` 或 `display: inline-block`，当 `v-show` 从 `false` 变成 `true` 时，不能简单地把它变成 `display: block`，而是需要恢复原样。**

为此，Vue 的底层处理逻辑是：

1. 在初次绑定或挂载时，<font color=red>读取元素当前的内联 `style.display`</font>。
   
2. 如果本来就是 none，则视为空字符串 `''`；<font color=dodgerBlue>否则</font>，<font color=red>记录下这个初始值</font>。
   
3. 把这个初始值缓存到 DOM 元素对象本身的一个自定义属性上。
   
4. <font color=dodgerBlue>当状态更新时</font>，根据布尔值将 `style.display` 设为 `none`，或者<font color=red>恢复为缓存的初始值（空字符串则交回给外部 CSS 样式表控制）</font>。

***

> [!NOTE]
> 
> 原回答中，还包含 Vue2 和 Vue3 具体实现的步骤和差别，这里略。

摘自：[AI Studio - Vue v-show 显隐原理](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221kgj3JEHCwTf90ptjDDfNnhTX24Pgxk8D%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)
...
v-show 始终会生成 vnode，render 的时候也会渲染成真实节点，只是在 render 过程中会在节点的属性中修改 show 属性值，也就是常说的 display
...
感觉文档中说的不够明白，<font color=dodgerBlue>用更容易理解的说法描述</font>：一个子组件中 `emit` 的自定义事件名必须为 `input`，同时父子组件中传的值键名必须为 `value`，这会导致麻烦和困扰。所以，可以通过配置 `model` 来解决该问题；通过`model.prop` 来指定子组件中的 value 名称，通过 `model.event` 来指定子组件中触发的事件名称。在最后 `$emit` 是操作的就是 `this.$emit('model_event', model_prop_processed_result)`

学习自：[父子组件prop值的双向绑定](https://mirai027.github.io/blog/interview/vue/父子组件prop值的双向绑定.html)

##### Vue3 v-model 实现

在 Vue3 中除了可以使用 `modelValue="modelValue.keyword"` 和 `@update:model-value="val => modelValue.keyword = val"` 实现 `v-model` ，也可以直接使用语法糖 `defineModel` 创建一个 `v-model` 。

看了 [使用computed拦截v-model【渡一教育】](https://bilibili.com/video/BV17i44zoEbu) ，视频录制的时候 `defineModel` 应该还没推出，视频也在推崇 VueUse 中 [useVModel](https://vueuse.org/core/useVModel/#usevmodel) 中使用计算属性作为中转，避免破坏单项数据流，也保证 `v-model` 的实现。

> [!NOTE]
>
> 现在 VueUse 官方推荐使用官方的 `defineModel` ，而不是用 `useVModel`
>
...
> >
> > 摘自：[VueUse - `useVModel`](https://vueuse.org/core/useVModel/)

<font color=dodgerBlue>这里做下摘抄：</font>

一种常见的错误是直接对在子组件中使用父组件传入的 `v-model` 的值，示例如下，显然破坏了单项数据流
...
<img src="https://s2.loli.net/2025/10/21/NEcRJaHg4jr8OU9.png" alt="image-20251021230745042" style="zoom:40%;" />

同时，使用 modelValue 的方式又过于麻烦：

```vue
<!-- 父组件实现与调用子组件 -->
<el-input
  :modelValue="modelValue.keyword"
  @update:model-value="val => modelValue.keyword = val"
/>
```

更不用说存在多个 `v-model` 的情况。

<img src="https://s2.loli.net/2025/10/21/e3IdMc7FDvzQ9kn.png" alt="image-20251021232029348" style="zoom:40%;" />

所以这就引出了使用计算属性 + 代理的实现方案。使用计算属性是为了作为中间层，修改的是计算属性，而不是传入的 `v-model`  本身；使用代理是为了处理存在多个 `v-model` 的情况
...
<img src="https://s2.loli.net/2025/10/21/jvb2qYgzILl1Vn5.png" alt="image-20251021232156831" style="zoom:40%;" />
...
prop 中的 type 对应的是 类型的构造函数，所以首字母大写。另外，在自定义对象（自定义构造函数）会通过 `instanceof` 来检查确认。或许原生类型可以通过 JavaScript 自带的方法进行检测，但是原理上也是可以通过 `instanceof` 来进行检测。
...
在 Vue 3 中，“Effect”（副作用）是其响应式系统的核心概念。它指的是那些**依赖于响应式数据，并且在这些数据发生变化时需要被重新执行的代码逻辑**。

Vue 3 中的 Effect 可以从**开发者使用的 API 层（Composition API）**和**底层架构层**两个维度来进行分类和理解：

###### 开发者常用的 Effect API (Composition API)

- watchEffect / watchPostEffect / watchSyncEffect
- watch
- computed
  > [!NOTE]
  > Vue 3.5 之前，computed 基于 `ReactiveEffect` 实现；Vue 3.5 之后，computed 自身实现 `Subscriber`，不再创建 `ReactiveEffect`，但仍复用 effect 系统中的依赖追踪、dirty 标记、调度和清理机制。

###### 核心底层 Effect 概念与 API

在 Vue 3 的 `@vue/reactivity` 源码或底层逻辑中，Effect 包含了更核心的实现：

1. **effect 函数 (底层 API)**
   
    - 它是 `@vue/reactivity` 包导出的基础函数。watchEffect 和 computed 底层都是基于它封装的。如果在非组件环境下（比如只使用 Vue 的响应式包做 Node.js 开发），可以通过调用 `effect(fn)` 来创建一个底层的副作用。
    
2. **ReactiveEffect 类**
   
    - 这是 Vue 3 响应式系统内部真正的**副作用核心类**。当调用 `effect()` 或 `watchEffect()` 时，Vue 内部会实例化一个 `ReactiveEffect` 对象，它负责依赖收集（`track`）和触发更新（`trigger`）。
    
3. **渲染副作用 (Render Effect)**
   
    - **组件的核心机制**：在 Vue 3 中，**每一个组件实例都有一个专属的渲染副作用 (Render Effect)**。
      
    - 当组件初次挂载时，会执行这个 Effect，运行 render 函数生成虚拟 DOM 并收集相关的响应式数据依赖。当这些依赖变化时，这个 Render Effect 会被重新触发，从而导致组件的重新渲染。

###### Effect 的高级管理 API

为了在复杂的组合式函数（Composables）中管理多个副作用的生命周期，Vue 3 还提供了副作用作用域相关的 API：

1. **effectScope**
   
    - **作用**：创建一个副作用作用域对象。在这个作用域内部创建的所有响应式副作用（如 computed、watch、watchEffect）都会被自动收集在一起，方便统一管理。你可以调用 scope.stop() 来一次性销毁作用域内的所有副作用，防止内存泄漏。
    
2. **getCurrentScope / onScopeDispose**
   
    - 在副作用作用域中使用的钩子，用于获取当前作用域或在作用域被销毁时执行清理逻辑。

摘自：[AI Studio - Vue 3 Effect: 概念与API](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221Plzqqi-yD8iLb3RVcMtJ8Vl_4CbrOVUD%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)

#### Computed 和 Watch

##### computed 函数

computed 函数，如果只给它设置了一个 getter，它将<font color=FF0000>接受 getter 函数并为 getter 返回的值</font> <font color=fuchsia> **返回一个不可变的响应式 ref 对象** </font>。如果设置了 setter，则返回一个可变的 ref 对象。
...
- 它 <font color=fuchsia>**支持缓存**</font>，只有依赖的数据发生了变化，才会重新计算。<font color=FF0000>Computed 的值会默认走缓存</font>，计算属性是基于它们的响应式依赖进行缓存的：也就是基于 data 声明过，或者父组件传递过来的 props 中的数据进行计算的
   > [!NOTE]
   > watch 也有同样的性质，见 [[#Watch 之前被忽略的特点]] 中的第三点
...
- `watch` <font color=FF0000>**不支持缓存**</font>，数据变化时，它就会触发相应的操作
...
- <font color=fuchsia size=4>**支持异步监听**</font>
  ...
  > > [!TIP]
  > > 
  > > 对于确实要使用 watchEffect，且 watchEffect 的回调函数确实要使用 `async await` 的情况，可以将 `await` 表达式放在最后，<font color=red>因为 `await` 表达式后面的依赖将无法被收集到</font>。如果确实有依赖必须要放在 `await` 表达式后面（比如，依赖 `await` 表达式的返回结果），可以将依赖放在括号中，放在 `await` 表达式前面，示例如下：
  > >
  > > ```js
  > > watchEffect(async () => {
...
- <font color=fuchsia>**watch 在定义后会立即执行一次第一个函数，因为它要收集依赖**</font>
  ...
  <font color=dodgerBlue>**更重要的是**</font>：根据 [watch的诸多疑点【渡一教育】](https://www.bilibili.com/video/BV1JnJXzEEZa) 中所讲述的： `watch` 的第一个参数应该是一个 “返回要监听数据的函数” ；如果传入的并非一个函数，而是一个响应式对象 、或 ref，将会被转换为函数；同时，<font color=fuchsia>**在监听数据发生变化时，这个函数同样会执行一次**</font>。如下示例：
  ...
  watch(() => {
    alert('first fn param runs')
    return msg.value
  }, () => {
    alert('second fn param runs')
  })
  </script>
  ...
  详见：[Vue SFC Playground 地址](https://play.vuejs.org/#)
...
> [!NOTE]
> 在 Vue3 中，watch 和 watchEffect 返回值类型是一样的，都是 `WatchHandle`，结构如下：
> ```ts
> interface WatchHandle { 
>   (): void // 可调用，与 `stop` 相同
>   pause: () => void
...
> }
> ```
> 详见 [Vue3 doc - API - 响应式 API：核心](https://cn.vuejs.org/api/reactivity-core.html)
...
- 具备一定的惰性 lazy（数据不改变则不会执行）
  ...
  如果想要让 `watch` 变成 `immediate` （即：<font color=red>**`watch` 创建时**</font>，数据改变相应的处理的回调立即执行一次），只需要在 watch 函数之后加上配置：`{ immediate: true }`

- 参数可以拿到原始和当前值

- 可以同时侦听多个数据的变化，用一个侦听器承载
...
<font color=fuchsia>使用侦听器来比较一个数组或对象的值，这些值是响应式的，**要求它有一个由值构成的副本**</font>。
...
> [!TIP]
>
> 在使用 webpack 时，Tree Shaking 何时会失效？
>
> - 首先必须保证 `mode` 为 `"production"`
>
> - 代码没有使用 ESM，而是混用了 CJS；因为 CJS 是动态的，webpack 无法进行静态分析
>
> - Babel 没有设置 `module: false` ，导致在 webpack 处理前，模块就被转译为 CJS
>
>   ```js
>   // babel.config.js
>   module.exports = {
...
>   ```
>
>   因为 babel 和 webpack 是串型工作的，babel 先工作，如果 babel 将 import 转换为 require ，那么 webpack 接手时将会看到 CJS，导致 Tree shaking 失败
>
> - `package.json` 中的 `sideEffect` 配置不当，导致 webpack 认为某些文件具有副作用，而不敢删除；或者反过来，直接配置 `sideEffects: false` ，导致错误地删除了全局 CSS 以及 Polyfill。
>    
>    > [!TIP]
>    >
>    > 以下内容，摘自或学习自 [Gemini](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221jhfvB19xSYrmVYirOlHaJpm16cOO9Tc7%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing) ：
>    >
>    > > <font color=dodgerBlue>副作用代码是指</font> 那些“引入后不导出任何变量，但执行时会影响全局”的代码，最典型的就是**全局 CSS 文件**（修改页面样式）和 **Polyfill 垫片**（修改全局 window 对象，比如 core-js）
>    >
>    > <font color=dodgerBlue>关于为什么 CSS 和 Polyfill 会被删</font>：CSS 和 Polyfill 经常是全局导入，但是并没有使用它们抛出的变量，而不是类似于 `import styles from './global.css'` 的写法。
>    >
>    > ```js
>    > import './global.css'; // 全局样式
>    > import 'core-js';      // ES6+ 垫片 (Polyfill)
>    > ```
>    > 再加上 `sideEffects: false` 保证没有副作用，导致会被误删。
>    >
>    > 推荐的写法是：
>    >
>    > ```json
...
>    > ```
>
> 学习自：[webpack tree-shaking 在什么情况下会失效](https://www.bilibili.com/video/BV1SpUZBYEWH)
...
###### 支持的数据类型
- reactive API 对**传入的类型是有限制的**，要求必须传入的是**一个对象或者数组类型**（👀 即引用类型）；如果传入一个基本数据类型 ( String、Number、Boolean ）会报一个警告
- ref 支持 所有数据类型，自然可以被用于 原始数据类型

###### 访问和修改数据的方式

- ref 在 JavaScript / TypeScript 逻辑中，必须通过 `.value` 属性来访问和修改值。在 Vue 的模板（Template）中，Vue 会进行自动解包（Auto-unwrapping），无需加上 `.value` 
- reactive 无论是在 JS 逻辑中还是在模板中，都不需要任何后缀，直接像普通对象一样访问和修改属性即可。

###### 重新赋值的表现

- ref 可以对 `.value` 重新赋予一个全新的对象，不会丢失响应式。
- reactive 不能直接重新赋值整个对象，否则会导致原本的响应式连接断开。因为重新赋值会让变量指向一个非 Proxy 的普通对象。可以使用 `Object.assign(reactiveObj, { /* ...newObj */ })` 替代重新赋值

###### 解构时的表现

- 直接对 reactive 对象进行 ES6 解构或使用展开运算符，解构出来的属性会失去响应式。为了保持响应式，需要配合 `toRefs()` 或 `toRef()` 使用。
- ref 包含的本身就是单一的值或引用，通常不会直接解构 ref 对象本身；哪怕进行解构也不会失去响应式

###### 底层实现

- **ref**：
  
    - <font color=dodgerBlue>对于基本数据类型</font> ：内部通过 `RefImpl` 类，使用对象的 getter 和 setter（即类属性访问器）来实现响应式拦截。
      
    - <font color=dodgerBlue>对于引用数据类型</font> ：<font color=red>**ref 内部会自动调用 `reactive()`**</font>，将其转化为 Proxy 对象，然后再挂载到 `.value` 上。
    
- **reactive**：直接基于 ES6 的 Proxy 实现对对象属性的深度拦截（包括属性的读取、设置、删除等操作）。

学习自：https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221jGu33mO5Vbw4hQVuQWMiPItK5ILFJRXG%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing
...
文档中所说 “ <font color=dodgerBlue>JavaScript 没有可以作用于所有值类型的 ‘引用’ 机制</font> ”，即：ref 包装的数据可以是 原始类型 ( primitive types ) ，因为不是对象也就无法进行引用。所以需要 `RefImpl` 类 先对 数据进行包裹，然后再使用 Proxy 进行代理。

##### ref 为什么要用 `.value` 取值

因为 <font color=red>Vue3 使用 Proxy 实现响应式</font>，而 <font color=fuchsia>Proxy 只能代理引用类型</font>；而 ref 包装的数据可以是 原始类型 ( primitive types ) ，不是对象也就无法进行引用。所以，<font color=fuchsia>**ref 使用 `RefImpl` 类对 ref 包装的变量进行了包装**</font>（实现原理参见 [[Vue原理与实现#Ref 实现]]）。

学习自：codingstartup 交流群 群友

#### 解构与响应式丢失

<font color=dodgerBlue>对 Vue3 响应式数据进行解构</font>，<font color=fuchsia>解构出来的是一个 “引用对象类型” 时，**它还是响应式的**</font>；但是<font color=red>解构出的是 “基本数据类型” 时，响应式会丢失</font>。
...
因为`Proxy`只能监听对象的第一层，深层对象的监听 Vue 是通过`reactive` 方法再次代理，所以返回的引用仍然是一个 `Proxy` 对象，而基本数据类型就是值。
...
setup 函数中的 props 参数 是响应式的，因此不能使用 ES6 的解构，这样会使得 prop 的响应式丢失；如果需要使用解构，可以使用 `toRefs()` 将 props 解构为 响应式的 prop

##### context

`setup(props, context)` 函数中的 context 参数包含：`attrs`、`slots`、`emits`、`expose` 。同时，<font color=lightSeaGreen>`context` 不是响应式的，所以可以放心的使用 ES6 解构</font>。
...
`attrs` 和 `slots` 是有状态的对象，它们总是会随组件本身的更新而更新。这意味着你应该<font color=lightSeaGreen>避免对它们进行解构</font>，并始终通过 `attrs.x` 或 `slots.x` 的形式使用其中的属性。此外还需注意，和 `props` 不同，`attrs` 和 `slots` 的属性都 **不是** 响应式的。如果你想要基于 `attrs` 或 `slots` 的改变来执行副作用，那么你应该在 `onBeforeUpdate` 生命周期钩子中编写相关逻辑。
...
`<script setup>` 是 Vue 3 单文件组件里给 Composition API 用的<font color=red>“编译期语法糖”</font>。核心作用：让你少写 `export default`、`setup()`、`return`、`components` 注册这些样板代码。

简单说，它做了这些事：

1. 把代码编译进组件的 `setup()` 里。所以这里的代码会在每个组件实例创建时执行，而不是模块首次 import 时只执行一次。
   
2. 顶层变量、函数、import 可以直接给模板用；不再需要通过 `components` 注册，通过 `return` 返回

3. 提供编译期宏来声明组件能力：`defineProps()`、`defineEmits()`、`defineExpose()`、`defineOptions()`、`defineSlots()`、`withDefaults()` 这些宏不用 import，编译时会被 Vue 处理掉

4. 更适合 TypeScript。props 可以直接用类型声明，Vue 编译器会尽量推导对应的运行时选项

5. 模板和逻辑在同一个作用域里。这能减少中间代理，对运行时和 IDE 类型推导都有好处。也就是说 `<template>` 更像是在“看见” `<script setup>` 的顶层作用域。
...
<font color=dodgerBlue>所以，使用虚拟 DOM 的原因有两个：</font>
...
Vue SFC 是框架指定的文件格式，必须由 [`@vue/compiler-sfc`](https://github.com/vuejs/core/tree/main/packages/compiler-sfc) 预编译为标准的 JavaScript 与 CSS。编译后的 SFC 是一个标准的 JavaScript ( ES ) 模块
...
- **`created`** ：在<font color=red>组件实例处理完所有与状态相关的选项后调用</font>。
  ...
  当这个钩子被调用时，<font color=dodgerBlue>以下内容已经设置完成</font>：<font color=red>响应式数据、计算属性、方法和侦听器</font>。然而，<font color=lightSeaGreen>此时挂载阶段还未开始</font>，因此 <font color=red>`$el` 属性仍不可用</font>
...
- **`mounted`** ：在组件被挂载之后调用。
  ...
  <font color=dodgerBlue>组件在以下情况下被视为已挂载：</font>

  - <font color=fuchsia>所有 **同步子组件** 都已经被挂载</font>。(不包含异步组件或 `<Suspense>` 树内的组件)

    > [!warning]
    > 
    > 这里和 Vue3 早期文档有所变动，当时说的是：只保证当前组件挂载完成，如果要等待到子组件挂载完成，需要在 mounted 内部调用 `nextTick()` ，在 `nextTick` 内部或者 `await nextTick` 之后是可以保证子组件挂载完成的
...
- **`unmounted`** ：在一个组件实例被卸载之后调用。
  ...
  > [!important]
  >
  > 之前的理解有误：在 unmount 生命周期中依然是可以拿到组件实例的，自然很适合作为清理副作用的时机。
  > 
  > > 在 Vue3 中，在 unmounted（Options API）或 onUnmounted（Composition API）生命周期钩子中，**仍然可以访问到当前组件实例**。
  > >
  > > <font color=dodgerBlue>虽然组件此时已经被销毁</font>，但<font color=fuchsia>它在内存中的实例并没有立刻消失，你依然可以读取它上面的状态和方法</font>，主要用于执行清理收尾工作
  > >***
  > > ###### 注意事项（unmounted 阶段的限制）
  > >
  > > 虽然实例还能访问，但此时组件的状态已经发生了根本性的改变：
  > >
  > > 1. **DOM 已脱离**：当前组件的 DOM 树已经被移除。虽然可以通过之前的 DOM 引用（或 `this.$el` ）拿到 DOM 节点，但它**已经不在页面 DOM 树里**了。
  > > 2. **响应式系统已停止**：在此阶段，所有的 watch、computed 以及模板渲染副作用（render effect）**都已被完全销毁或停止**。如果你在 unmounted 中修改了一个响应式数据（比如 `count.value = 0`），是**不会**触发任何视图更新或 watcher 监听的。
  > > 3. **子组件已全销毁**：当前组件的所有子组件此时也已经走完了它们的 unmounted 流程。
  > >
  > > **最佳实践**：通常我们只在 unmounted 钩子中做**清理副作用**的操作，比如：清除定时器、<font color=red>销毁第三方库实例（如 ECharts/Map 实例）</font>、移除 window 或 document 上绑定的事件监听器等
  >
  > 摘自：[AI Studio 问答](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221jneBb0TR8riSv0JfOqCodpK-j4rPHYp7%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)
...
> [!NOTE]
> 
> 总结与助记：无论是 mount<font color=red>ed</font>、updat<font color=red>ed</font> 还是 destroy<font color=red>ed</font> / unmount<font color=red>ed</font> ，父都要等子执行完了再执行
...
在打包时，webpack 会把 `assets/` 中放置的静态资源文件进行打包上传，在 file-loader 和 url-loader （ webpack5 中是 asset modules ）处理压缩，体积缩小。而压缩后的静态资源文件最终也都会放置在 `static/` 文件夹中跟着 `index.html` 一同上传至服务器。而 `static/` 在打包时是直接复制一遍，不会经过 webpack 处理。
...
- [vue-meta-info](https://github.com/muwoo/vue-meta-info) 配合 [prerender-spa-plugin](https://github.com/chrisvfritz/prerender-spa-plugin) 进行预渲染（👀 就是 SSG ）
  ...
  > >
  > > 摘自：[Vike doc - Guides - Pre-rendering (SSG) # What is pre-rendering?](https://vike.dev/pre-rendering#what-is-pre-rendering)
  > >
...
-   **依赖** 大多为在开发时不会变动的纯 JavaScript。一些较大的依赖（例如有上百个模块的组件库）处理的代价也很高。依赖也通常会存在多种模块化格式（例如 ESM 或者 CommonJS ）。
    ...
    Vite 将会使用 esbuild <font color=red>预构建依赖</font>。esbuild 使用 Go 编写，并且比以 JavaScript 编写的打包器预构建依赖快 10-100 倍。
    ...
    >
    > - 包管理器的 lockfile 内容，例如 `package-lock.json`，`yarn.lock`，`pnpm-lock.yaml`，或者 `bun.lock`
    > - 补丁文件夹的修改时间
...
-   **源码** 通常包含一些并非直接是 JavaScript 的文件，需要转换（例如 JSX，CSS 或者 Vue/Svelte 组件），时常会被编辑。同时，并不是所有的源码都需要同时被加载（例如基于路由拆分的代码模块）。
    ...
    Vite 以 原生 ESM 方式提供源码。这实际上是<font color=red>让浏览器接管了打包程序的部分工作</font>：<font color=fuchsia>Vite 只需要在浏览器请求源码时进行转换并 <font size=4>**按需提供源码**</font></font>。<font color=fuchsia>根据情景动态导入代码</font>，即只在当前屏幕上实际使用时才会被处理。
...
> [!NOTE]
> 所以，Vite 8.1 引入了 `bundledDev` 选项，即 Bundled Dev Mode，允许开发者不使用默认的 no-bundle 模式，这对于大型项目的热更新提速明显
...
首先，Vite 具有 “约定大于配置” 的理念，其次是 Vite 内置 PostCSS 处理器；另外，可见 [GitHub - Vite  - vite package - package.json](https://github.com/vitejs/vite/blob/main/packages/vite/package.json)

##### Vite 与 webpack 的区别

webpack 是打包器，而 Vite 在开发环境更接近 服务器 + 即时编译器；Vite 的快也更多体现在开发阶段

###### 工作模式不同

webpack 先打包，再启动。

- 运行 `npm run serve` ，会从入口文件开始，递归地“爬取”所有的 `import` 和 `require` ，将项目中所有的模块加载到内存中，通过 loader 和 plugin 进行处理；最后将这些文件“粘合”在一起，生成 bundle.js 文件；同时，bundle.js 文件被注入到 index.html 中，并启动一个服务器来提供这个 index.html 。

- 所以核心瓶颈在于：项目越大，模块越多，需要便利和处理的文件就越多，构建依赖图和生成 bundle 的时间就越长
- 热更新慢，是因为在修改一个文件时，webpack 需要重新计算这个文件及其依赖的

Vite 先启动，再按需编译。

- 运行 `npm run dev` 时，Vite 几乎不做任何的打包工作，只会做两件事：依赖预构建 和 启动 dev server
- 依赖预构建：检测 package.json 中的依赖，使用 ESBuild 将这些依赖提前打包为 ESM 格式，并放入 `node_modules/.vite` 下。预构建速度很快，同时只需要在第一次、以及依赖变更时才运行
  > [!TIP]
  > 
  > 存在如下问题：node_modules 中存在很多小文件，且有一些是 CJS 格式的。如果浏览器挨个请求，页面会卡死；所以 Vite 的 “依赖预构建” 将这些第三方依赖打包为几个大的 ESM 模块。
  > 
  > 好处是：
  > 
  > 1. 将 CJS 统一转化为 ESM
  > 2. 将大量小文件<font color=red>合并</font>为大文件，提高加载性能
  > 
  > 学习自：[vite 编译器有啥特点？](https://www.bilibili.com/video/BV1qQC7BXE9F)
  > 
  >
  > > ###### 为什么需要预编译 & 预构建
  > > 
...
- 依赖预构建完成后，启动一个 web 服务器，将 index.html 返回给浏览器，浏览器会解析 index.html ；当遇到如 `<script type="module" src="/src/main.ts"></script>` 时，会向服务器按需请求 `/src/main.ts` ，<font color=red>**Vite 的 Dev Server 会拦截这个请求**</font>，并对 `/src/main.ts` 进行即时编译，并将编译好的 js 模块返回给浏览器
  > 此外，浏览器重新请求修改的模块（热更新），更新速度为 $O(1)$ ，不会随着项目变大而变大。因为拦截这种机制，所以很合理
  > 
  > 学习自：[Vite 相比 Webpack 等传统打包工具，其核心优势是什么](https://www.bilibili.com/video/BV1yzmgBpEbA)
  
  > [!TIP]
  > 
  > 这里顺带补充一下 Vite 热更新的原理
  > 
  > 和大多数构建工具一样，Vite 使用 [chokidar](https://github.com/paulmillr/chokidar) 来监听文件系统的变化 `chokidar.watch(targetDirectoryOrFile).on()` ，发现变化，就会通过 WebSocket 的 `send` 通知浏览器哪个文件变了。对应的，浏览器端也通过 WebSocket 接收消息，拿到变更的文件路径，通过 `` await import(payload.path + `?t=${payload.timestamp}`) `` 获取最新的代码。加上时间戳是为浏览器认为这是个新资源，而发起新的 HTTP 请求，拿到最新的代码，而不是读缓存；再之后就是执行模块的热更新回调。整个过程没有 webpack 那些复杂的依赖图重算，所以速度快很多。
  > 
  > 学习自：[Vite 的热模块替换为何比 Webpack 快](https://www.bilibili.com/video/BV12bm7BVEp6)
- Vite 的核心在于：将打包这个“重任”下放给浏览器，只在浏览器请求到某个模块时才去编译那个模块，做按需编译

学习自：[为什么 Vite 速度比 Webpack 快](http://bilibili.com/video/BV1JEk6BDEmR)

##### Vite 涉及到哪些底层原理

###### 开发环境

1. 利用原生 ESM 实现无打包( no-bundle )。不再需要像 webpack 一样提前打包好所有文件，而是利用的浏览器对 ESM 的原生支持。当浏览器请求一个模块时，Dev Server 会拦截这个请求，按需对该文件进行即时编译，再返回给浏览器
2. 使用 ESBuild 进行 “依赖预构建”
3. 基于 ESM 的 HMR。传统的 HMR 需要重新计算整个模块依赖图并打包，Vite 的 HMR 则利用 ESM 的特性，当一个模块被请求时，Vite 会精确地让浏览器重新请求那个被修改的模块
...
服务器端：通过 `fs.watcher` 监听文件系统的变化，当文件变化时，Vite Server 会分析依赖图，找到受影响的模块。通过 WebSocket 向客户端发送一个消息，告诉那个模块需要更新。收到消息后，通过动态 import 去请求新的模块，并执行 HMR 逻辑

###### 插件机制

Vite 的插件机制基于 Rollup 的插件接口设计，可以在开发环境和生产环境同时工作，实现了生态的统一

学习自：[vite 涉及到了哪些底层原理？](https://www.bilibili.com/video/BV17UC4B2EHK)
...
- **请求长度** ：<font color=fuchsia>浏览器由于对 URL 长度的限制</font>，所以<font color=fuchsia>会影响 GET 请求发送数据时的长度</font>。这个限制是浏览器规定的（不同浏览器的规则不同），并不是 RFC 规定的
  ...
  > [!NOTE]
  > 对于过长的 GET 请求 URL，<font color=lightSeaGreen>有可能</font> 会触发 414 URI Too Long 状态码；可以通过 POST 请求把一些不必要的参数放到请求体中，虽然有可能触发 413 Content Too Large。此外，还有 431 Request Header Fields Too Large 状态码 表示 首部字段过大
  >
  > ###### 更多补充
  >
...
- **参数类型** ：<font color=fuchsia>POST 的参数传递支持更多的数据类型</font> ( MIME types )，而 GET 只能使用 ASCII 字符（的子集）；另外，GET 只能使用 URL 编码，而 POST 支持各种编码方式
  > [!TIP]
  > 
  ...
  > 
  > 摘自：[get和post本质上并没有多大的区别，就看你愿不愿意遵守规范了](https://juejin.cn/post/7326330936432771135)
...
- <font color=lightSeaGreen>获取服务器支持的所有 HTTP 请求方法</font>
...
2. **缓存与请求策略判断** ：浏览器会根据请求方式、HTTP 缓存、Service Worker、已有连接等情况，判断是否可以直接得到响应，或者是否需要发起网络请求。<font color=fuchsia>如果命中 **仍然新鲜的强缓存** ( fresh cache )，浏览器可以直接使用缓存响应，不需要进行 DNS 解析和网络连接</font>；如果缓存已经过期 ( stale )，则通常需要向服务器发起条件请求进行协商缓存校验，例如通过 `If-None-Match` / `ETag` 或 `If-Modified-Since` / `Last-Modified` 判断资源是否变化。如果服务器返回 `304 Not Modified`，浏览器会继续使用本地缓存；如果资源已变化，则返回新的资源内容。这里具体可以看下 [[#浏览器缓存的全过程]] 中以及后面相关的内容

   > [!NOTE]
   > 这里不要把顺序写死成“缓存判断一定先于 DNS”。更准确地说：**如果本地已有可直接使用的响应，就可以避免 DNS 和网络请求；如果需要重新验证或重新请求资源，就仍然可能进入 DNS、建立连接、发送 HTTP 请求等流程**。另外，Service Worker、HSTS 升级、DNS 预解析、preconnect、连接复用等机制，也会影响实际执行顺序。

3. **DNS 解析** ：下一步首先需要获取的是输入的 URL 中的 <font color=LightSeaGreen>域名的 IP 地址</font>，<font color=FF0000>首先会判断本地</font> （ 👀 localhost 。另外，这里应该也分 浏览器 DNS 缓存和 OS 的 DNS 缓存）<font color=red>是否有该域名的 IP 地址的缓存，如果有，则使用</font>；<font color=red>如果没有，则向本地 DNS 服务器发起请求</font>。本地 DNS 服务器也会先检查是否存在缓存，如果没有就会先向根域名服务器发起请求，获得负责的顶级域名服务器的地址后，再向顶级域名服务器请求，然后获得负责的权威域名服务器的地址后，再向权威域名服务器发起请求，最终获得域名的 IP 地址后，本地 DNS 服务器再将这个 IP 地址返回给请求的用户。

   <font color=FF0000>**用户向本地 DNS 服务器发起请求属于递归请求，本地 DNS 服务器向各级域名服务器发起请求属于迭代请求**</font>

   > 👀 这里可以看下 [[#DNS 解析过程]] ，有更完整的过程

4. **TCP 三次握手** ：下面是 TCP 建立连接的<font color=dodgerBlue>三次握手的过程</font>：首先，<font color=FF0000>客户端向服务器发送一个 **`SYN` 连接请求报文段** ( `SYN = 1` ) 和 一个 **随机序号 `seq=x`**</font>，服务端接收到请求后向客户端发送一个 `SYN` `ACK` 报文段，确认连接请求；并且向客户端发送一个随机序号 `seq=y` 。客户端接收服务器的确认应答后，进入连接建立的状态，同时向服务器也发送一个 `ACK` 确认报文段。<font color=FF0000>服务器端接收到确认后，也进入连接建立状态</font>；此时双方的连接就建立起来了。
...
5. **HTTPS 握手** ：如果使用的是 HTTPS 协议，在通信前还存在 <font color=FF0000>**TLS 的一个四次握手的过程**</font>。首先，由 <font color=dodgerBlue>**客户端** 向服务器端发送</font>：<font color=red>使用的协议的版本号、一个随机数 ( nonce ) 和 可以使用的加密方法</font>。<font color=dodgerBlue>服务器端收到后</font>，<font color=LightSeaGreen>确认加密方法，也向客户端发送一个随机数和自己的 CA 证书</font>。<font color=dodgerBlue>客户端收到后</font>，首先检查数字证书是否有效，如果有效，则再生成一个随机数，并<font color=fuchsia>使用证书中的公钥对随机数加密</font>，然后发送给服务器端，并且还会提供一个前面所有内容的 hash 值供服务器端检验。<font color=dodgerBlue>服务器端接收后</font>，使用自己的私钥对数据解密，同时向客户端发送一个前面所有内容的 hash 值供客户端检验。<font color=FF0000>**这个时候双方都有了三个随机数**，按照之前所约定的加密方法，**使用这三个随机数生成一把秘钥**，以后双方通信前，就使用这个秘钥对数据进行加密后再传输</font>。
   ...
   > > 之后双方发送加密的 HTTP 数据
   > > ```
   > > 摘自：让 gpt-5.5xl 验证本节内容正确性和准确性给出的解答

6. **返回数据** ：当页面请求发送到服务器端后，<font color=FF0000>**服务器端会返回一个 html 文件作为响应**</font>；<font color=FF0000>浏览器接收到响应后，**开始对 html 文件进行解析**，开始页面的渲染过程</font>

7. **页面渲染** ：浏览器首先会根据 HTML 文件构建 DOM 树，根据解析到的 CSS 文件构建 CSSOM 树；<font color=FF0000>如果遇到 `<script>` 标签，则判端是否含有 `defer` 或者 `async` 属性，要不然 script 的加载和执行会造成页面的渲染的阻塞</font>。当 DOM 树 和 CSSOM 树 建立好后，根据它们来构建渲染树。渲染树构建好后，会根据渲染树来进行布局 ( layout ) 。布局完成后，最后使用浏览器的 UI 接口对页面进行绘制 ( paint ) 。这个时候整个页面就显示出来了。

8. **连接复用与关闭** ：页面资源加载完成后，TCP 连接通常不会立刻断开。HTTP/1.1 默认使用持久连接 ( keep-alive )，浏览器会将连接暂时放入连接池，后续同源请求可以继续复用；HTTP/2 还可以在同一条 TCP 连接上进行多路复用。因此，<font color=FF0000>**TCP 四次挥手不是页面加载完成后的必然 “最后一步”**</font>。

   当连接空闲超时、客户端或服务器主动关闭连接、浏览器关闭页面，或者响应头显式要求关闭连接时，才会进入 TCP 连接释放过程。TCP 连接释放通常通过四次挥手完成：主动关闭方发送 `FIN`，对方回复 `ACK`；等对方也没有数据要发送后，再发送 `FIN`，主动关闭方回复 `ACK`。主动关闭方随后进入 `TIME_WAIT` 状态，等待一段时间后再进入 `CLOSED` 状态。

   <img src="https://s2.loli.net/2023/09/12/UxvVQlmM21dRrtH.png" alt="image-20230912105424140" style="zoom:38%;" />
   
   > [!NOTE]
   > HTTP/3 基于 QUIC/UDP，不使用 TCP，因此也没有 TCP 三次握手和四次挥手。
摘自：[「2021」高频前端面试题汇总之计算机网络篇 ](https://juejin.cn/post/6908327746473033741)
...
- 在 <font color=lightSeaGreen>HTTP/1.1 下</font>，<font color=red>**浏览器对一个域名下最大 TCP 连接数为 6**</font>，所以会请求多次。可以用多域名部署解决。这样可以提高同时请求的数目，加快页面图片的获取速度。
...
- <font color=dodgerBlue>在 HTTP/2 下</font>，可以一瞬间加载出来很多资源，因为，<font color=FF0000>**HTTP/2 支持多路复用**，可以在同一个 TCP 连接中发送多个 HTTP 请求</font>
...
- **资源请求方面**：支持断点续传
  ...
  在 HTTP/1.0 中，存在一些浪费带宽的现象；例如<font color=dodgerBlue>客户端只是需要某个对象的一部分，而服务器却将整个对象送过来了</font>，并且 <font color=red>HTTP/1.0 不支持断点续传功能</font>。<font color=fuchsia>**HTTP/1.1 则支持断点续传**</font>，<font color=red>在请求头引入了 `Range` 首部，它允许只请求资源的某个部分</font>，即响应状态码是 `206 Partial Content` ，这样就方便了开发者自由的选择以便于充分利用带宽和连接。
...
- **二进制协议**：<font color=fuchsia>**HTTP/2 是一个二进制协议**</font>。在 <font color=FF0000>**HTTP/1.1 中**，**报文的头信息必须是文本（ ASCII 编码 ）**，数据体可以是文本，也可以是二进制</font>（ 👀 比如媒体文件）。<font color=fuchsia>**HTTP/2 则是一个彻底的二进制协议**，头信息 和 数据体 都是二进制，并且**统称为 “帧”** ( Frame )，**可以分为 头信息帧 和 数据帧**</font>。 <font color=FF0000>**帧的概念是 HTTP/2 实现多路复用的基础**</font>
...
- **多路复用** ：<font color=fuchsia>**HTTP/2 实现了多路复用**</font>，HTTP/2 仍然复用 TCP 连接，但是<font color=fuchsia>在一个 ( TCP ) 连接里</font>，<font color=fuchsia>客户端和服务器都可以同时发送多个请求或回应，而且不用按照顺序一一发送</font>（ 👀 即，可以乱序发送），<font color=fuchsia>这样就 **缓解** 了 “队头阻塞” 的问题</font>。
 
  > [!NOTE]
  > 
  > 此外，还 <font color=fuchsia>**引入了优先级**</font>（ ⚠️ 这点有所遗忘），如下：
  ...
  >
  > 另外，上面中原文是 “解决了队头阻塞的问题”，但实际上并没有解决，而只能说是（通过 **优先级** 和 **乱序发送**，避免无意义的等待）缓解了队头阻塞，但并没有消除，还是有其他情况会导致队头阻塞。这些问题，可以通过 HTTP/3 解决

  > [!TIP]
  >
  > ###### 关于队头阻塞
  >
  > **队头阻塞** 是由 HTTP 基本的 “请求 - 应答” 模型所导致的。HTTP 规定报文必须是 “一发一收”，这就形成了一个先进先出的 “串行” 队列。队列里的请求是没有优先级的，只有入队的先后顺序，排在最前面的请求会被最优先处理。如果队首的请求因为处理的太慢耽误了时间，那么队列里后面的所有请求也不得不跟着一起等待，结果就是其他的请求承担了不应有的时间成本，造成了队头阻塞的现象。
  > 
  > ###### HTTP/2 下会引发队头阻塞的场景
  > 
  > > 在 HTTP/2 中，所有请求共用一个 TCP 连接，如果上传大文件，会因为队头阻塞 ( HOL Blocking ) 影响其他请求。为了解决这个问题，可以将大文件请求和小请求分离。
  > > 
  > > > [!warning]
  > > > 下面的示例代码是错误的，不要照搬，再后面有说原因和正确视线方案
  > >
  > > ```js
  ...
  > > ```
  > > > [!warning]
  > > > 上面的代码是有问题的，因为：[undici](https://github.com/nodejs/undici) 的 `Agent` 似乎并没有 `maxStreamsPerConnection` 这一选项，只在找到了 C# 中的 [`Http2Limits.MaxStreamsPerConnection` 属性](https://learn.microsoft.com/zh-cn/dotnet/api/microsoft.aspnetcore.server.kestrel.core.http2limits.maxstreamsperconnection?view=aspnetcore-9.0) ，在 Google 搜索不到 “undici maxStreamsPerConnection” 有用的信息，问了下 [GPT-5](https://chatgpt.com/share/68b012f5-8708-800d-8aa4-6160f9e01d09) 得到肯定的答复后，更加确定了这一观点。此外，GPT 也给出了有效的方法：
  > > > ```js
  ...
  > > >       ...opts,
  > > >       connections: 1,  // 控制连接数
  > > >      // 如果需要 HTTP/2 支持，也可以传 allowH2: true
  > > >    });
  > > >   }
  > > > });
  > > > 
...
- **数据流** ：<font color=fuchsia>**HTTP/2 使用了数据流的概念**</font>，因为 <font color=red>HTTP/2 的 “帧” 是 **不按顺序发送的**</font>（乱序发送），<font color=fuchsia>同一个连接里面连续的帧，可能属于不同的请求</font>。因此，<font color=FF0000>必须要做标记，指出它属于哪个请求</font>。<font color=fuchsia>HTTP/2 将每个请求或回应的所有帧，称为一个数据流</font>。<font color=FF0000>每个数据流都有一个独一无二的编号</font>。<font color=lightSeaGreen>**帧发送时，都必须标记数据流 ID ，用来区分它属于哪个数据流**</font>。最后，这些 “帧” 根据每个帧首部的 “流标识符” 重新组装

- **头信息压缩** ：<font color=FF0000>**HTTP/2 实现了头信息压缩**</font>，由于 <font color=dodgerBlue>HTTP/1.1 协议不带状态，每次请求都必须附上所有信息。所以，**请求的很多字段都是重复的**</font>，比如 <font color=dodgerBlue>Cookie</font> 和 User Agent ，<font color=red>一模一样的内容，每次请求都必须附带</font>，这会浪费很多带宽，也影响速度。HTTP/2 对这一点做了优化，引入了头信息压缩机制。

   > [!warning]
   > 这里 [Cookie](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Cookie) 请求头，每次请求都会携带；这一概念，相当容易疏忽。
...
- **服务器推送** ( Server Push )：HTTP/2 允许服务器未经请求，主动向客户端发送资源，这叫做 <font color=FF0000>**服务器推送**</font>。使用服务器推送提前给客户端推送必要的资源，这样就可以相对减少一些延迟时间。
  ...
  ⚠️ 需要注意的是： <font color=fuchsia size=4>HTTP/2 下服务器绝大多数情况 **主动推送的是静态资源**</font>，<font color=red>和 WebSocket 以及使用 SSE 等方式向客户端发送即时数据的推送是不同的</font>。
  
  > [!TIP]
  > 
  > 对 HTTP/2 的 Server Push 特性是否只支持推送静态资源有点好奇，便询问了 ChatGPT，摘录内容如下：
  >
  > > HTTP/2 的服务器推送 ( Server Push ) <font color=red>并不仅限于静态资源</font>，但<font color=fuchsia>**通常更适用于推送静态资源**</font>。HTTP/2 的服务器推送允许服务器在客户端请求之前主动将额外的资源推送给客户端，以提高页面加载性能。这些资源可以包括静态资源（如 CSS、JavaScript、图像等）以及动态生成的内容。
  > >
  > > 服务器推送的<font color=dodgerBlue>主要优势</font>在于<font color=red>它可以减少客户端请求的往返次数，从而缩短页面加载时间</font>。这对于减少延迟和提高性能非常有帮助，特别是在高延迟网络环境下。
  > >
  > > 然而，<font color=dodgerBlue>**服务器推送并不是在所有情况下都适用**</font>。它<font color=red>需要服务器能够预测客户端可能需要的资源，并在适当的时候将这些资源推送给客户端</font>。这可能需要一定的逻辑和资源管理，以<font color=red>确保不会浪费带宽和客户端资源</font>。
  > > 
  > > > [!NOTE]
  > > > 
  > > > 👀 关于这点，可以先看下下面的 [[#wikipedia 关于 HTTP/2 Server Push 的介绍]]
  > >
...
**HTTP 性能的关键在于低延迟而不是高带宽**。<font color=lightSeaGreen>**大多数 HTTP 连接的时间都很短，而且是突发性的**</font>，但 <font color=lightSeaGreen>TCP 只在长时间连接传输大块数据时效率才最高</font>。<font color=fuchsia>HTTP/2 通过 **让所有数据流共用同一个连接**，可以更有效地使用 TCP 连接，让高带宽也能真正的服务于 HTTP 的性能提升</font>。

> [!TIP]
>
> ###### HTTP/1.x 中
>
> 和很多人想象不一样的是，浏览器从服务器获取的一个页面，通常由很多资源链接所组成。服务器给浏览器推送的第一个页面，页面里通常嵌入了图片资源文本链接、以及动态页面资源链接、或第三方网站的链接资源，还需要浏览器根据这些文本链接内容，去连接所对应的服务器，继续下载链接所对应的内容。
>
...
>
> > HTTP/2 支持多路复用的特性，但<font color=red>它并不限制只建立一个TCP连接</font>。相反，HTTP/2 允许在一个 TCP 连接上同时处理多个请求和响应，从而提高了性能和效率。
> >
...
> [!TIP]
>
> HTTP Header 是大小写不敏感的，不过，在 HTTP/2 中有了点不同：
>
...
HTTP 协议采用 **明文** 传输信息，存在 **信息窃听**、**信息篡改** 和 **信息劫持** 的风险；协议 TLS/SSL 具有 **身份验证**、**信息加密** 和 **完整性校验** 的功能，可以避免此类问题发生。
...
对称加密的方法是：<font color=FF0000>双方使用 **同一个秘钥** 对数据进行 加密 和 解密</font>。但是<font color=dodgerblue>对称加密的存在一个问题</font>，就是：<font color=dodgerBlue>如何保证秘钥传输的安全性</font>，因为<font color=lightSeaGreen>秘钥还是会通过网络传输的，一旦秘钥被其他人获取到，那么整个加密过程就毫无作用了</font>。 这<font color=FF0000>就要用到非对称加密的方法</font>。
...
<font color=dodgerblue>使用 TLS/SSL 后不一定是安全的</font>，因为<font color=lightSeaGreen>**没办法确定得到的公钥就一定是安全的公钥**</font>：**可能存在一个 <font color=FF0000 size=4>中间人</font> ( MITM )**，截取了对方发给我们的公钥，然后将他自己的公钥发送给我们，当我们使用他的公钥加密后发送的信息，就可以被他用自己的私钥解密。然后他伪装成我们以同样的方法向对方发送信息，这样我们的信息就被窃取了，然而自己还不知道。<font color=dodgerblue size=4>为了解决这样的问题，可以使用数字证书</font>。

首先，<font color=FF0000>使用一种 Hash 算法来对 **公钥** 和 其他信息 进行加密，生成一个 **信息摘要**</font>；然后，<font color=fuchsia>让有公信力的认证中心</font> ( <font color=FF0000>CA</font> / Certificate Authority ) <font color=fuchsia>用它的私钥对 **消息摘要** 加密，形成 **签名**；最后将原始的信息和签名合在一起，称为 **数字证书**( Digital Certificate )</font> 。当接收方收到数字证书时，先根据原始信息使用同样的 Hash 算法生成一个摘要，然后<font color=FF0000>使用 **CA 的公钥** 来对数字证书中的摘要进行解密，最后将解密的摘要和生成的摘要进行对比，就能发现得到的信息是否被更改了</font>。

> [!TIP]
> 
> 有点好奇 “数字证书” 的组成，便问了 GPT-5 ，[回答](https://chatgpt.com/share/68b00124-70e0-800d-a539-a73ff1907b69) 内容稍多，做了点概括，详见源链接：
> 
> > 数字证书（通常是由 **CA - Certificate Authority** 签发的 X.509 证书）本质上是对某个公钥和主体身份的绑定证明。它是一个结构化的数据文件，常见于 HTTPS、TLS 等场景。
> >
> > <font color=dodgerBlue>在 **X.509 v3 数字证书** 中，主要包含以下信息：</font>
> >
> > - **主体名称 ( Subject Name )** ：证书归属的实体信息（可以是个人、组织、网站域名）
> > - **公钥信息 ( Subject Public Key Info )** ：<font color=red>包含 公钥算法（RSA, ECDSA, Ed25519 等）和 公钥值本身</font>。<font color=lightSeaGreen>这部分是证书的核心</font>：用来让别人加密消息或验证签名。
> > - **签发者信息 ( Issuer )** ：签发该证书的 **CA 的名称**（与 Subject 类似的字段，如 CN、O、C 等）
> > - **有效期 ( Validity )** ：Not Before 证书生效时间、Not After 证书到期时间
> > - **序列号 ( Serial Number )** ：证书唯一编号，由 CA 分配，用于区分不同证书
> > - **证书版本 ( Version )** ：常见为 v3（当前广泛使用），旧的有 v1/v2。
> > - **签名相关 ( Signature )** ：签名算法（例如 SHA256-RSA）、CA 对证书内容的签名值（保证证书未被篡改）
> > - **扩展字段 ( Extensions，X.509 v3 特有)** ：略
>  >
> > **总结一下**：数字证书主要包含 **身份信息 ( Subject ) 、公钥 ( Public Key )、签发者 ( Issuer )、有效期、序列号**，再加上 **扩展字段** 和 **签名**。
> >
> > 它的核心作用就是 **把一个公钥和实体身份绑定起来，并由 CA 背书**。

这个方法最要的是认证中心的可靠性，一般浏览器里会内置一些顶层的认证中心的证书，相当于我们自动信任了他们（💡 这里存在一个 “信任链” 的概念），只有这样才能保证数据的安全。

##### HTTPS 通信（握手）过程

> [!TIP]
> 
> “四次握手” 通常指的是 **TLS 1.2 及之前** 在 **全握手 (Full Handshake)** 下的典型流程。
> 
> 此外，**TLS 1.3** 里流程被优化为 **“1-RTT 握手”**，甚至可以在部分场景下做到 **0-RTT**（会话恢复时）
> 
> 学习自：[GPT - 请介绍一下 TLS 四次握手的流程](https://chatgpt.com/share/68b1055c-2650-800d-b7df-c4f3ee1463f8)
> 
> > 关于 TLS1.3 的流程（ ClientHello & ServerHello ）可见 [[#在浏览器中输入 URL 并且按下回车之后发生了什么]] 中的补充内容
> 
> > 👀 此外，下面的内容也是经过上面 GPT 给出回答内容的优化和补充的

即：<font color=fuchsia>**TLS 四次握手**</font>

1. <font color=dodgerBlue>客户端向服务器 **发起请求**</font>，请求中包含：<font color=fuchsia>使用的 TLS 协议版本号</font>、<font color=lightSeaGreen>生成的一个 **随机数1** </font>( nonce )、<font color=fuchsia>**客户端支持的 加密方法** 和 压缩方法</font>；可能还会附带 SNI ( Server Name Indication ) 等扩展

2. 服务器端接收到请求后，<font color=fuchsia>**确认双方使用的 TLS 版本、加密方法**</font>；并 <font color=fuchsia size=4>**给出服务器的证书**</font>、以及 <font color=lightSeaGreen>一个服务器生成的 **随机数2** </font>；将这些作为回应发送给客户端

   > [!TIP]
   > 
   > 上面的证书是指 CA ( Certificate Authority ) 颁发的 SSL/TLS 数字证书
   >
   > 另外，根据 [AWS 文档 - 什么是 SSL/TLS 证书？](https://aws.amazon.com/cn/what-is/ssl-certificate/) 中的内容：
   >
   > > Web 服务器发送包含公钥的 SSL/TLS 证书作为回复。
   >
   > 说明了：服务器返回的证书是包含公钥的，也对应了步骤3 中客户端使用 证书中的公钥加密 这一过程。关于证书的组成，可以看下 [[#数字证书]] 处的补充内容

3. <font color=fuchsia size=4>**客户端确认服务器证书有效**</font>（校验合法性，包含： CA 签发、域名匹配、是否过期） 后，<font color=lightSeaGreen>生成一个新的 **随机数3** </font>，并<font color=fuchsia>使用 **证书中的公钥**，加密这个随机数</font>，然后发给服务器。并且<font color=red>还会提供一个前面所有内容的 hash 的值，用来供服务器检验</font>。
...
- <font color=fuchsia>**SSL 证书需要绑定 IP ，不能在同一个 IP 上绑定多个域名**</font>
...
> HTTP/3 是第三个主要版本的 HTTP 协议。与其前任 HTTP/1.1 和 HTTP/2 不同，在 HTTP/3 中，将弃用 TCP 协议，<font color=dodgerBlue>改为使用基于 UDP 协议的 QUIC 协议实现</font>。<font color=fuchsia>此变化 <font size=4>**主要为了解决 HTTP/2 中存在的队头阻塞问题**</font></font>。由于 <font color=red>HTTP/2 在单个 TCP 连接上使用了多路复用，受到 TCP 拥塞控制的影响，少量的丢包就可能导致整个 TCP 连接上的所有流被阻塞</font>。
>
...
> [!TIP]
> QUIC 是<font color=fuchsia>传输层协议</font> ，同时 TLS 也是（毕竟 <font color=red>**T**ransport **L**ayer</font> Security ）
...
- **集成 TLS 加密功能**：QUIC 内置 TLS1.3 ，减少了 HTTPS 握手所花费的 RTT 数
  > [!NOTE]
  >  
  >  可以看下 [[#HTTPS 通信（握手）过程]] 下面 GPT 回答的补充
...
- **多路复用**：<font color=red>同一物理连接上可以有多个独立的 **逻辑数据流**</font>，实现了数据流的单独传输，解决了（ HTTP/2 中 ）TCP 的 “队头阻塞” 问题
  ...
  ![](https://s2.loli.net/2022/05/16/dXmCtnbcPuEIMQZ.png)

- **快速握手**：由于基于 UDP（👀 即不需要实现 TCP 三次握手 和 TLS 四次握手，共 3个 RTT），<font color=FF0000>可以实现使用 0 - 1 个 RTT 来建立连接</font>  

  > [!TIP]
  >
  > 这里的范围是没有 “ HTTP事务” 的情况，否则分别要 +1 ；**0RTT 是重连**（会话恢复），1RTT 是重头开始连接。
  >
...
- **`302 Found` **：**临时重定向**。表示请求的资源被分配到了新的 URI，希望用户（本次）能使用新的 URI 访问资源。<font color=FF0000>和 `301 Moved Permanently` 状态码相似，但是 **`302` 代表的资源不是被永久重定向，只是临时性质的**</font>。也就是说已移动的资源对应的 URI 将来还有可能发生改变。<font color=lightSeaGreen>若用户把 URI 保存成书签，但不会像 301 状态码出现时那样去更新书签</font>，而是仍旧保留返回 `302` 状态码的页面对应的 URI 。同时，搜索引擎会抓取新的内容而保留旧的网址；因为服务器返回 302 代码，搜索引擎认为新的网址只是暂时的
...
- **`303 See Other` **：表示：<font color=FF0000>由于请求对应的资源存在着另一个 URI，**应使用 `GET` 方法定向获取请求的资源**</font>。 <font color=dodgerBlue>**`303` 状态码和 `302 Found` 状态码有着相似的功能**</font>，但是 <font color=fuchsia>**`303` 状态码明确表示客户端应当采用 `GET` 方法获取资源**</font>。
  ...
  > [!warning]
  > 这里作为一个细节在面试中确实是被问到了，虽然记得是有 “POST 转成 GET” 的状态码，但是记不住是哪个了

  <font color=red>303 状态码通常作为 `PUT` 或 `POST` 操作的返回结果</font>，它<font color=red>表示重定向链接指向的不是新上传的资源，而是另外一个页面</font>，比如消息确认页面或上传进度页面。而 <font color=red>请求重定向页面的方法要总是使用 `GET`</font> 。

  ###### 注意

  - 当 `301`、`302`、`303` 响应状态码返回时，几乎所有的浏览器都会把 `POST` 改成 `GET` ，并删除请求报文内的主体，之后请求会再次自动发送。
  - <font color=red>`301`、`302` 标准是禁止将 `POST` 方法变成 `GET` 方法的</font>，但实际大家都会这么做。
...
- **`403 Forbidden`**：表明<font color=FF0000>请求资源的访问被服务器拒绝了</font>，服务器端没有必要给出详细理由，但是可以在响应报文实体的主体中进行说明。<font color=fuchsia>**进入该状态后，不能再继续进行验证（ 而 401 是可以的 ）**</font>。<font color=red>该访问是永久禁止的</font>，并且与应用逻辑密切相关。

- **`404 Not Found`**：表明<font color=FF0000>服务器上无法找到请求的资源</font>。除此之外，<font color=lightSeaGreen>也可以在服务器端拒绝请求且不想说明理由时使用</font>
  > [!NOTE]
  > 
  >  在 GitLab 中进入一个无权限的仓库时遇到过这种情况。当时考虑了下，这样确实可以最小程度防止信息泄漏
...
- 首先会在 **浏览器的缓存** 中查找对应的 IP 地址，如果找到就直接返回；否则，继续下一步

- 检查 **操作系统缓存**，常见的 如 Hosts 文件，找到就直接返回；否则，继续下一步

- 将请求发送给 **本地 DNS 服务器**（ 路由器 / ISP 提供的 DNS ） ，在本地域名服务器缓存中查询。如果查找到，就直接将查找结果返回；若找不到，则继续下一步
...
- 不保证消息交付：不确认，不重传，无超时
- 不保证交付顺序：<font color=red>不设置包序号，不重排，不会发生队头阻塞</font>
...
<font color=FF0000>TCP 在发送一个数据之后，就开启一个定时器</font>；<font color=fuchsia>若是 **在这个时间内** **没有收到** 发送数据的 **ACK 确认报文**，则对该报文进行重传</font>；在达到一定次数还没有成功时放弃并发送一个<font color=red>复位信号</font>
...
第一步：客户机打算关闭连接时，向其 TCP 发送一个连接释放报文段，并停止发送数据，主动关闭 TCP 连接，该报文段的 <font color=red>**FIN 标志位被置 1**</font>， <font color=fuchsia>seq = u</font> ，它<font color=fuchsia>**等于前面已传送过的数据的最后一个字节的序号加 1**</font> （ FIN 报文段即使不携带数据，也要消耗一个序号）。TCP 是全双工的，即可以想象为一条 TCP 连接上有两条数据通路。<font color=lightSeaGreen>发送 FIN 报文时，**发送 FIN 的一端不能再发送数据**，即关闭了其中一条数据通路，但对方还可以发送数据</font>。
...
> [!NOTE]
> 
> 这里描述文字遗漏了 ACK = 1 ，seq=w 以及 ack=u+1

第四步：客户机收到连接释放报文段后，必须发出确认。在确认报文段中，ACK 字段被置为 1，确认号 ack=w+1，序号 seq=u+1。<font color=red>此时 TCP 连接还未释放</font>，<font color=fuchsia>**必须经过时间等待计时器设置的时间 2MSL 后**，客户机才进入连接关闭状态</font>。
...
|---|---|---|
|**通信模式**|<font color=red>全双工</font>（客户端和服务端都能主动发送消息）|单向（服务端推送到客户端，客户端不能主动用 SSE 发消息）|
|**底层协议**|基于 TCP，独立于 HTTP，初始握手通过 HTTP Upgrade|基于 HTTP 协议（<font color=red>长连接，text/event-stream</font>）|
|**数据格式**|<font color=fuchsia>任意格式</font>（二进制、文本、JSON、Protobuf 等）|<font color=red>文本格式</font>（UTF-8 编码，消息体是事件流）|
|**浏览器支持**|需要 API 支持（大部分现代浏览器支持）|原生支持较好（几乎所有现代浏览器支持 EventSource）|
|**服务器端实现**|相对复杂，需要 WebSocket 服务器或支持库|简单，只要支持返回持续的 HTTP 响应即可|
|**客户端实现**|使用 WebSocket 对象，需手动解析消息|<font color=lightSeaGreen>使用 EventSource 对象</font>，自动解析事件流|
|**传输方向**|双向|单向（服务端 → 客户端）|
|**重连机制**|<font color=red>需要自己实现心跳/重连逻辑</font>|内置重连机制（EventSource 自动重连）|
|**性能**|更高效，适合高频交互|较轻量，适合低频但持续的消息推送|
|**使用场景**|聊天应用、多人协作、在线游戏、股票行情|实时新闻、通知推送、监控数据、日志流|
|**代理/防火墙兼容性**|可能会被部分代理阻断（需配置支持 Upgrade）|兼容性好，因为就是 HTTP 流式响应|
|**二进制支持**|支持二进制（ArrayBuffer、Blob）|不支持（只能传文本，需要 Base64 编码）|
|**规范/标准化**|IETF RFC 6455|WHATWG HTML Living Standard|

> 摘自：[GPT5 - WebSocket 和 SSE 两者存在哪些区别？请将结果以表格形式呈现](https://chatgpt.com/share/68ba9a32-0db0-800d-984e-10c77ff8a154)
...
XSS 攻击（ cross-site scripting / 跨站脚本攻击 ） 指的是 **跨站脚本攻击**，是一种 <font color=FF0000>**代码注入攻击**</font>。<font color=red>攻击者通过在网站 **注入恶意脚本**，**使之在用户的浏览器上运行**</font>，<font color=lightSeaGreen>从而盗取用户的信息如 Cookie 等</font>。
...
- DoS 攻击，发送合理请求，占用服务器资源，使用户无法访问服务器
- 破坏页面结构
- 流量劫持（将链接指向某网站）
...
- **反射型** ：攻击者诱导用户访问一个带有恶意代码的 URL 后，<font color=lightSeaGreen>服务器端接收 URL 拼接在 HTML 中，把 HTML 发送到浏览器端，**浏览器端解析这段带有 XSS 代码的数据后当做脚本执行**</font>，最终完成 XSS 攻击。 
...
- <font color=FF0000>**使用 CSP**</font> ：CSP 的本质是建立一个白名单，告诉浏览器哪些外部资源可以加载和执行，从而防止恶意代码的注入攻击
  ...
  > - 设置 `meta` 标签的方式 ( `<meta http-equiv="Content-Security-Policy" content="..." />` )
  > 
  > > [!NOTE]
  > > 
  > > 根据 [什么？2025年了发版后还要手动清浏览器缓存？](https://juejin.cn/post/7527977068417957939) 中的表述：
  > > 
  > > > HTTP 响应头（Response Headers）的优先级 > HTML Meta 标签
  > > > 
  > > > `meta` 标签更像是一种“建议”，而由服务器（如 Nginx）在 HTTP 响应中返回的 `Cache-Control`、`Expires` 等头部信息，才是浏览器必须严格遵守的“最高指令”。
  > > 
  > > 可知：最好还是在 HTTP 响应头中设置 CSP
...
>***
> 名称类似的攻击，还有 SSRF ( Server-Side Request Forgery ) 。
...
> 
> 另外，可以看下 [单点登录的模式【渡一教育】](https://www.bilibili.com/video/BV1oE421u71n) 讲了一些 JWT 中 refresh_token 没听过的东西
...
- **进行同源检测：**<font color=fuchsia>服务器 **根据 http 请求头中 `origin` 或者 `referer`** 信息来 **判断请求是否为允许访问的站点，从而对请求进行过滤**</font>；当 `origin` 或者 `referer` 信息都不存在的时候，直接阻止请求。
  ...
  > > > [!NOTE]
  > > > 
  > > > 感觉可以概括为：只有是 同源 且是 `GET` 请求，才不会自动添加 `Origin` 请求头
  > > 
  > > 摘自：[MDN - Origin](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Origin)
...
- **对 Cookie 进行双重验证**：<font color=FF0000>服务器在用户访问网站页面时，向请求域名注入一个 Cookie，内容为 **随机字符串**</font>；<font color=lightSeaGreen>**当用户再次向服务器发送请求的时候，从 Cookie 中取出这个字符串，添加到 URL 参数中，服务器通过对 Cookie 中的数据和参数中的数据进行比较，进行验证**</font>。
  ...
  使用 Cookie 双重验证，利用了攻击者只能利用 Cookie，但是不能访问获取 Cookie 的特点。<font color=lightSeaGreen>该方法比 CSRF Token 的方法更加方便</font>，并且<font color=red>不涉及到分布式访问的问题</font>。<font color=dodgerBlue>缺点</font>是如果网站存在 XSS 漏洞，会失效。同时，这种方式<font color=red>不能做到子域名的隔离</font>。
...
- **在设置 Cookie 时设置 `Samesite` 属性 ，限制 cookie 不能作为被第三方使用**：可以避免被攻击者利用。
 ...
 > [!NOTE]
 > 
 > 该情况在 [Cookie中的SameSite【渡一教育】](https://www.bilibili.com/video/BV1shRnYyELE) 中也有提及，不过说法不同：会阻止发送 Cookie，但是对超链接放行，毕竟都已经离开该站点了。另外，视频中还有提及：`SameSite` 的默认值为 `lax` 
...
- **GPU** ：Chrome 刚开始发布的时候是没有 GPU 进程的，而 GPU 的使用初衷是为了实现 3D CSS 的效果，只是随后网页、Chrome 的 UI 界面都选择采用 GPU 来绘制，这使得 GPU 成为浏览器的普遍需求
  ...
  > [!NOTE]
  > 
  > 关于 GPU 进程还是要注意下，虽然更早记录的 [[#Chrome 浏览器架构图]] 也有提到 “ GPU 进程”，但一点印象都没有了...
  > 
  > 另外，上面把 “网络进程” 划到 “浏览器主进程” 中了；感觉应该要分出来
...
- **渲染进程**：<font color=FF0000>核心任务是 **将 HTML、CSS 和 JavaScript 转换为用户可以与之交互的网页**</font>，<font color=red>**排版引擎 Blink 和 JavaScript 引擎 V8 都是运行在该进程中**</font> （ 👀 详见 [[#渲染进程有哪些线程]] ）。<font color=fuchsia>默认情况下，**Chrome 会为每个 Tab 标签创建一个渲染进程**</font>。<font color=lightSeaGreen>出于安全考虑，渲染进程都是运行在沙箱模式下</font>
   > [!NOTE]
   > 注意：Chrome 和一般浏览器不一样，一般的浏览器是区分 render 引擎和 JS 引擎的
...
以 **Chrome / Chromium 的 renderer process** 来说，通常可以这样理解：

- **主线程（main thread）**：负责 HTML 解析、CSS 解析、样式计算、布局、绘制记录生成，以及大部分页面脚本执行
- **合成线程（compositor thread）**：负责把已经准备好的图层合成为帧；在某些输入场景下，它可以不等主线程就先合成新帧
- **光栅线程池（raster threads）**：把图层切成多个 tile，并把它们栅格化成位图
- **工作线程（worker threads）**：页面使用 `Web Worker`、`Shared Worker` 或 `Service Worker` 时，对应脚本会在后台线程里执行

更准确地说，很多旧资料里说的 **“GUI 渲染线程”**、**“JS 引擎线程”**、**“事件触发线程”**、**“定时器触发线程”**、**“异步 HTTP 请求线程”**，都不太适合作为 renderer process 的标准线程划分来记。

对页面里的 JavaScript 来说，通常是运行在**主线程**上，所以脚本执行、样式计算、布局和绘制会互相影响；但这不等于“JS 引擎”本身就是 renderer 里的一个独立固定线程。

一句话记忆：**渲染进程核心是主线程 + 合成线程 + 光栅线程池，再加上可选的 worker 线程。**
...
摘自：[MDN - worker 简介](https://developer.mozilla.org/zh-CN/docs/Learn_web_development/Extensions/Async_JS/Introducing_workers)

##### Service Worker 和 Shared Worker 的区别

**Service Worker 像浏览器层的“网络代理/后台事件处理器”，Shared Worker 像多个页面共享的一条后台 JS 线程。**

|区别|Service Worker|Shared Worker|
|---|---|---|
|主要用途|PWA、离线缓存、请求拦截、Push、后台同步|多个页面/标签页共享计算、状态、连接|
|生命周期|事件驱动，页面关了也可能被唤醒|只要还有页面连接就活着|
|是否能拦截网络请求|可以，监听 `fetch`|不可以|
|是否能服务多个页面|可以，控制同一 scope 下页面|可以，被同源多个页面连接|
|创建方式|`navigator.serviceWorker.register()`|`new SharedWorker()`|
|通信方式|`postMessage`、`clients`、事件|`MessagePort`|
|DOM 访问|不能|不能|

简而言之：要控制网络和离线能力，用 Service Worker；要让多个页面共享一个后台 JS 实例，用 Shared Worker。
...
1. **Service Worker** ：Service Worker 运行在 JavaScript 主线程之外，虽然由于脱离了浏览器窗体无法直接访问 DOM，但是它可以完成 **离线缓存**、**消息推送**、**网络代理** 等功能。它可以让我们自由控制缓存哪些文件、如何匹配缓存、如何读取缓存，并且缓存是持续性的。<font color=FF0000>当 Service Worker 没有命中缓存的时候，需要去调用 `fetch` 函数获取数据</font>；<font color=dodgerBlue>也就是说</font>：如果没有在 Service Worker 命中缓存，会根据缓存查找优先级去查找数据。但是，<font color=FF0000>不管是从 Memory Cache 中还是从网络请求中获取的数据，浏览器都会显示是从 Service Worker 中获取的内容</font>

2. **Memory Cache** ：Memory Cache 就是 <font color=FF0000>内存缓存</font>，它的效率最快；**但是内存缓存虽然读取高效，可是<font color=FF0000>缓存持续性很短，会随着进程的释放而释放</font>**。<font color=LightSeaGreen>一旦关闭 Tab 页面，内存中的缓存也就被释放了</font>
...
2. 下一次加载资源时，<font color=fuchsia>由于 强制缓存优先级较高，先 **比较当前时间与上一次返回 `200` 时的时间差**。**如果没有超过 `cache-control` 设置的 `max-age`**  ，则没有过期，**命中强缓存**，直接从本地读取资源</font>。<font color=dodgerBlue>**如果浏览器不支持 HTTP/1.1**</font>（ 👀 即：不支持 `cache-control` ），则<font color=red>**使用 `expires` 头判断是否过期**</font>
...
3. <font color=dodgerBlue>**如果资源已过期**（或者 `cache-control` 的值是 `no-cache` ）</font>，<font color=fuchsia>**则表明强制缓存没有被命中，则开始 协商缓存**</font> ：向服务器发送带有 `If-None-Match`（有 `ETag` 时）和 `If-Modified-Since`（没有 `ETag` 时）的请求
   ...
   > > 
   > > > 对于 `GET` 和 `HEAD` 请求方法来说，当且仅当服务器上没有任何资源的 `ETag` 属性值与这个首部中列出的相匹配的时候，服务器端才会返回所请求的资源，响应码为 `200`。
   > > >
...
> [!NOTE]
> 
> 上面的 可选指令 不全，比如 `stale-while-revalidate`，详见 [MDN - `Cache-Control`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Cache-Control)
...
> [!WARNING]
> 现在关于 “Render Tree” 这一概念，在 “浏览器渲染流程” 相关话题中已经较少提及了。
> 
> 在询问 GPT-5 的问题 [上面提及了 Render Tree ，这个词在现在探讨“浏览器渲染流程”时很少提及了，一般在样式计算后就是生成 Layout Tree 了，请解释下为什么会出现这种情况？](https://chatgpt.com/share/68b4738b-3c98-800d-a838-81911e10582e) 中，GPT 给出了解释
> 
> > 在 **早期的 WebKit / Blink** 文档、博客中，通常把 DOM + 样式生成的可视化树叫做 **Render Tree**。
> > - 其节点是 RenderObject（WebKit）或后来 Blink 中的 RenderBox 等。
> > - 它既包含了“哪些元素要绘制”，也包含了“它们的几何信息（大小、位置）”。
> > 因为名字里有 “Render”，容易给人错觉：这棵树直接就是绘制（paint）的依据。
> >
> > 后来 Blink 项目逐渐把 pipeline 拆分得更明确：
> > - **Style 阶段**：决定每个元素的样式。
> > - **Layout 阶段**：负责计算几何信息（width, height, position, margin collapse 等）。
> > 	- 这一步生成的是 **Layout Tree**，其节点类型叫做 LayoutObject。
> > 	- 它对应的就是元素的 **几何/排版结构**，而不是直接渲染的最终图层。
> > - **Paint 阶段**：才会把 Layout Tree 遍历一遍，输出绘制指令，进入 Display List，再映射到图层。
> >
> > 也就是说，现在浏览器开发团队更倾向于强调：
> > - 样式计算之后得到的树，本质上是用于 **布局计算（layout）**，而非直接渲染。
> > - 所以它被更准确地称为 **Layout Tree**。
...
DOM Tree -> CSSOM Tree -> Render Tree -> Layout Tree -> Paint -> Composite
...
1. 首先解析收到的文档，根据文档定义构建一棵 DOM 树，DOM 树是由 DOM 元素及属性节点组成的。
   ...
   > > [!TIP]
   > > 也就是防御 DOM XSS 攻击。此外 [简略图解：输入 url 到出现页面，浏览器做了什么？](https://segmentfault.com/a/1190000040566642) 中也有提到：
   > >  > 在渲染引擎中，DOM 有三个层面的作用：
   > >  > 
   > >  > - 从页面角度：DOM 是生成页面的基础数据结构。
   > >  > - 从 JS 角度：DOM 提供了 JS 操作的接口。通过这套接口，JS 可以对 DOM 接口进行访问，从而使开发者拥有改变文档结构、样式、内容的能力。
   > >  > - 从安全角度：<font color=lightSeaGreen>DOM 是 HTML 经过解析的内部数据结构，它将 Web 页面和 JS 链接起来，并 **过滤了一些不安全的内容**</font>。
   > >  > 
   > >  > **生成 DOM 可以过滤一些不安全内容**，这<font color=red>主要是渲染引擎中的一个名为 XSSAuditor 安全检查模块实现的</font>。它会监测词法安全，<font color=red>**在分词器解析出 Token 之后，检查这些模块是否引用了外部脚本，是否符合 CSP 规范，是否存在跨站点请求等**</font>。如果出现不符合规范的内容。XSSAuditor 会对该脚本或下载任务进行拦截。
   > >  
   > >  此外，在这篇文章有提及到一些之前未听说的名词：Layer Tree ( 虽然在 [[#合成阶段，由绘画树生成图形树，每个图形层也就是我们平时所说的合成层]] 中有提及 “PaintLayer Tree” 和 “GraphicsLayer Tree”) 、Draw Quads ，这里做下记号，以防止未来遇到
...
3. 根据 DOM 树和 CSSOM 规则树构建渲染树（ 👀 这里的说法并不清楚，根据下面 MDN 的摘抄，“DOM 和 CSSOM 树<font color=red>结合为渲染树</font>”，确实更清楚些）。<font color=red>渲染树的节点被称为 **渲染对象**</font>，<font color=fuchsia>**渲染对象是一个包含有颜色和大小等属性的对象**</font>（ 👀 见下面的 “注” ），<font color=red>渲染对象和 DOM 元素相对应</font>，但<font color=red>这种**对应关系不是一对一的**</font>，（ 👀 这是因为）<font color=fuchsia>不可见的 DOM 元素不会被插入渲染树</font>。还有一些 DOM元素对应几个可见对象，它们一般是一些具有复杂结构的元素，无法用一个矩形来描述。
   ...
   > 将 DOM 树和 CSSOM 树结合，<font color=red>去除不可见元素，生成渲染树</font> ( Render Tree )。
   >
...
> > ###### 另一种 “浏览器渲染原理” 的说法
> >
> > - 主线程：解析 parse -> 样式计算 style computing -> 布局 layout -> 分层 layer -> 绘制 paint
> >
...
> [!Warning]
> 在看下面的内容时，记得看下 [[#关键渲染路径步骤]] 和 [[#合成阶段，由绘画树生成图形树，每个图形层也就是我们平时所说的合成层]] 下面补充的警告内容
...
>
>   - <font color=red>屏幕大小调整</font>
>
>     > [!NOTE]
>     >  
>     > 屏幕大小调整，这点不容易想到；另外，还有屏幕旋转也会导致重排
>
...
<font color=dodgerBlue>**现代浏览器会对 频繁的 回流或重绘操作 进行优化**</font>，<font color=red size=4>**浏览器会维护一个队列，把所有引起回流和重绘的操作放入队列中**</font>；<font color=fuchsia>**如果队列中的任务数量或者时间间隔达到一个阈值的，浏览器就会将队列清空，进行一次批处理，这样可以把多次回流和重绘变成一次**</font>
...
> [!NOTE]
> 
> <font color=fuchsia>**当触发回流时，一定会触发重绘；但是重绘不一定会引发回流**</font>。可以说这是由 “CRP 中 Layout 在 Paint 之前” 这种顺序关系决定的；也可以参考 [[#管道针对指定帧的运行的三种方式]]
...
- **使用硬件 ( GPU ) 加速**：样式中的合成层 类似于 PS 中图层的概念，不同层中的 Layout 和 Paint 互不影响。开启 GPU 加速元素会被单独提升到一层。
  ...
  **使用方法** ：使用 `will-change` ，以创建一个新的合成层；如果不支持 `will-change` ，则使用 `transform: translateZ(0)` 。不过也不建议滥用。
  ...
  > [!TIP]
  >
  > > 绘制可以将布局树中的元素分解为多个层。将内容提升到 GPU 上的层（而不是 CPU 上的主线程）可以提高绘制和重新绘制性能。<font color=dodgerBlue>有一些特定的属性和元素可以实例化一个层</font>，<font color=red>**包括 `<video>` 和 `<canvas>`**</font> ，任何 CSS 属性为 `opacity` 、3D `transform`、`will-change` 的元素，还有一些其他元素（ 👀 更多可以看下 [MDN - 层叠上下文](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Guides/Positioned_layout/Stacking_context) 中的列表）。这些节点将与子节点一起绘制到它们自己的层上，除非子节点由于上述一个（或多个）原因需要自己的层。
  > >
  > > 摘自：[MDN - 渲染页面：浏览器的工作原理 # 绘制](https://developer.mozilla.org/zh-CN/docs/Web/Performance/How_browsers_work#绘制)
...
- **内联的 `<script>` 也不会触发 Paint**

> 👀 另外，如下面 [[#`<script>` 和 `<link>` 对 DOM 解析 和 渲染 的补充#结论]] 中所说，还需要： `<script>` 标签之前的 `<link>` 标签需加载完毕 // TODO：这里说法很怪，建议改一下

因此，<font color=FF0000>**建议 `<script>` 标签放在 `</body>`（ body 结束标签）之前**</font>，这样不会阻塞页面整体内容的 DOM 解析和渲染；但 <font color=FF0000>**如果页面中只有内联的 `<script>` 标签，那么放在任何位置对页面的渲染影响都是一样的**</font>。
...
> [!TIP]
> 
> 在推文 https://twitter.com/Barret_China/status/1684192099024146432 中说明了 meta 标签推荐的排列顺序，截图如下：
>
...
摘自：[前端缓存：最被低估的性能优化技巧](https://mp.weixin.qq.com/s/KCOqAhb5jwqTlPquxFjXFQ)
...
>
> <font color=dodgerBlue>**询问 ChatGPT “解释一下 计算机网络中的 eTLD” 的 [回复](https://chat.openai.com/share/a2b5601b-1b61-4ecb-a38c-1a3a232e19a4) 如下：**</font>
>
...
**对于简单请求，浏览器会直接发出 CORS 请求**（👀 不需要预检）；<font color=fuchsia>它会**在请求的头信息中增加一个 `Origin` 字段**，说明本次请求来自哪个源</font>（ `Origin: <scheme>://<hostname>:<port>`。另外，根据 [MDN - `Origin` # 语法](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Reference/Headers/Origin#语法) 中的内容：端口是选传的），<font color=FF0000>服务器会根据这个值来决定是否同意这次请求</font>。<font color=dodgerBlue>**如果 `Origin` 指定的域名在 白名单 内**</font>，<font color=dodgerBlue>服务器返回的响应就会多出以下信息头</font>：
...
<font color=dodgerBlue>服务器回应的 CORS 的字段如下</font>：
...
<font color=dodgerBlue>在 CORS 请求中 想要传递 Cookie 需要满足条件</font>
...
学习自：[跨域问题解决方案应用场景【渡一教育】](https://bilibili.com/video/BV1UNg6zqEhL)
...
> [!NOTE]
> 
> 考虑该问题时，只想到了 Shared Worker，没想到 Service Worker。另外，看了 [[#Service Worker 和 Shared Worker 的区别]] 确实感觉 Shared Worker 更适合。
...
###### window.srcoll（建议加上节流函数） + dataset + 高度触发
...
比如，React 的 [react-lazyload](https://github.com/twobin/react-lazyload)，Vue 的 [vue-lazyload](https://github.com/hilongjw/vue-lazyload)
...
- **节流** ：<font color=red>规定一个单位时间，在这个单位时间内，只能有一次触发事件的回调函数执行</font>；如果在同一个单位时间内某事件被触发多次，只有一次能生效。节流可以使用在 onScroll 函数的事件监听上，通过事件节流来降低事件调用的频率。
...
> [!important]
>
> **Core Web Vitals 目前还有 3 个核心指标：**
>
> | 指标  | 含义                                                 | 好的阈值 |
> | ----- | ---------------------------------------------------- | -------- |
> | `LCP` | Largest Contentful Paint，最大内容绘制，衡量加载速度 | ≤ 2.5s   |
> | `INP` | Interaction to Next Paint，衡量交互响应              | ≤ 200ms  |
> | `CLS` | Cumulative Layout Shift，衡量视觉稳定性              | ≤ 0.1    |
>
> `FID` 已经不再是 Core Web Vital。Google 在 **2024-03-12** 用 `INP` 正式替代了 `FID`。
>
> 来源：[web.dev Web Vitals](https://web.dev/articles/vitals)、[Google Search Central](https://developers.google.com/search/blog/2023/05/introducing-inp)。
>
>  npm 包 **`web-vitals`**，当前主分支 `v5.3.0` 仍导出的测量 API 是 5 个：
>
> - `onCLS`
> - `onFCP`
> - `onINP`
...
> - `onTTFB`
>
> 也就是：`CLS`、`FCP`、`INP`、`LCP`、`TTFB`。其中真正的 Core Web Vitals 是 `CLS / INP / LCP`，`FCP / TTFB` 属于辅助诊断指标。
...
FP 是时间线上的第一个 “时间点”，是指浏览器从响应用户输入网址地址，到浏览器开始显示内容的时间，简而言之就是<font color=red>浏览器第一次发生变化的时间</font>。
...
FMP 全称 “ First Meaningful Paint ”，译为 “首次有效绘制”。表示<font color=red>页面的 “主要内容” 开始出现在屏幕上的时间点</font>，它以前是我们测量用户加载体验的主要指标。本质上是通过一个算法来猜测某个时间点可能是 FMP，但是<font color=lightSeaGreen>最好的情况也只有 77% 的准确率</font>，<font color=lightSeaGreen>在 lighthouse6.0 的时候废弃掉了这个指标</font>，<font color=red>取而代之的是 LCP 这个指标</font>。
...
首次内容绘制 ( FCP / First Contentful Paint ) 是测量 [感知加载速度](https://web.dev/user-centric-performance-metrics/#types-of-metrics) 的一个以用户为中心的重要指标，因为该项指标会在用户首次在屏幕上看到任何内容时，在页面加载时间轴中标记出相应的点，迅捷的 FCP 有助于让用户确信某些事情正在[进行](https://web.dev/user-centric-performance-metrics/#questions)。
...
总阻塞时间 ( TBT / Total Blocking Time ) 指标<font color=red>测量 First Contentful Paint 首次内容绘制 ( FCP ) 与 Time to Interactive 可交互时间 ( TTI ) 之间</font> <font color=fuchsia>**主线程被阻塞的时间过长，无法作出输入响应 的 总时间**</font>

> [!NOTE]
> 
> 上文应该是存在翻译问题，导致出现了错误的含义；已经在摘抄部分做了修改。英文版如下：
>
...
> [!WARNING]
> TBT 计算的是长任务的时间之和。此外，每个长任务的时间在计算时也是要减去 50ms 的
...
> [!warning]
> 
> 注意这里，仅仅计算超过 50ms 的长任务，且阻塞时间是要减去 50ms 的
...
<font color=dodgerBlue>一般来说，发生输入延迟（又称输入延时）是因为</font> <font color=red>浏览器的主线程正忙着执行其他工作</font>，所以（还）不能响应用户。可能导致这种情况发生的 <font color=dodgerBlue>一种常见原因是</font> 浏览器正忙于解析和执行由您的应用程序加载的大型 JavaScript 文件。在此过程中，浏览器不能运行任何事件侦听器，因为正在加载的 JavaScript 可能会让浏览器去执行其他工作
...
FID 是测量页面加载期间响应度的指标。因此，<font color=fuchsia>FID **只关注** 不连续操作对应的输入事件</font>，<font color=red>如点击、轻触和按键</font>。
...
TTI ( Time to Interactive ) 指标测量页面 <font color=red>从开始加载 到 主要子资源完成渲染，并能够快速、可靠地响应用户输入所需的时间</font>。

如需根据网页的 [性能跟踪](https://developer.chrome.com/docs/devtools/evaluate-performance/reference/) <font color=dodgerBlue>计算 TTI，请执行以下步骤</font>：

1. 先进行 First Contentful Paint 首次内容绘制 ( FCP )。

2. <font color=red>**沿时间轴正向搜索时长至少为 5 秒的安静窗口**</font>，其中，<font color=dodgerBlue>安静窗口的定义为</font>：<font color=fuchsia>没有 [长任务](https://web.dev/custom-metrics/#long-tasks-api) ，且不超过两个正在处理的网络 GET 请求</font>。

   > 👀 长任务即：执行时间超过 50 毫秒 (ms) 的任何任务；另外，下图中也有说明

3. <font color=red>沿时间轴反向搜索 **安静窗口** 之前的最后一个长任务</font>，如果没有找到长任务，则在 FCP 步骤停止执行。
...
CLS ( Cumulative Layout Shift ) 测量整个页面生命周期内发生的所有 [意外布局偏移](https://web.dev/cls/#expected-vs-unexpected-layout-shifts) 中<font color=red>最大一连串的布局偏移分数</font>。
...
<video controls="" height="452" loop="" muted="" playsinline="" poster="https://web.dev/articles/cls/web-dev-assets/layout-instability-api/session-window-poster.png?hl=zh-cn" width="658">
    <source src="https://web.dev/static/articles/cls/video/web-dev-assets/better-layout-shift-metric/session-window.webm?hl=zh-cn" type="video/webm">
    <source src="https://web.dev/static/articles/cls/video/web-dev-assets/better-layout-shift-metric/session-window.mp4?hl=zh-cn" type="video/mp4">
</video>
...
请注意，<font color=fuchsia>**只有当现有元素的起始位置发生变更时才算作布局偏移**</font>。<font color=red>如果将新元素添加到 DOM 或是现有元素更改大小，则不算作布局偏移</font>，前提是元素的变更不会导致其他可见元素的起始位置发生改变。
...
> [!NOTE]
> 感觉和找最大/最小值很相似：默认将第一个元素设置为最大/最小值，并依次向后寻找是否有更大或更小的值
...
下次绘制交互 ( INP / Interaction to Next Paint ) 是一项新的指标，<font color=lightSeaGreen>浏览器计划于 2024 年 3 月将其取代取代首次输入延迟 ( FID / First Input Delay ) ，成为最新的 Web Core Vitals</font>

INP 是一种指标，通过 <font color=lightSeaGreen>观察用户访问页面的整个生命周期中发生的所有单击、敲击和键盘交互的延迟</font> 来评估页面对用户交互的整体响应能力。

<font color=red>INP 考虑的是所有页面的交互</font>，而首次输入延迟 (`FID`) 只会考虑第一次交互。而且它只测量了第一次交互的输入延迟，而不是运行事件处理程序所需的时间或下一帧渲染的延迟。
...
*因此，INP 的目的不是衡量互动的所有最终影响（例如网络提取和其他异步操作的界面更新），而是衡量下一次绘制被阻塞的时间*。延迟视觉反馈可能会给用户造成网页响应速度不够快的印象，而 INP 旨在帮助开发者衡量这部分用户体验。
...
INP 是 First Input Delay (FID) 的后继指标。虽然这两者都是响应速度指标，但 <font color=lightSeaGreen>FID 仅衡量网页上 *首次* 互动的输入延迟</font>。<font color=red>INP 通过观察网页上的 *所有* 互动</font>（从输入延迟开始，到运行事件处理脚本所需的时间，最后到浏览器绘制下一个帧为止）来改进 FID。

这些差异意味着 INP 和 FID 是不同类型的响应能力指标。<font color=lightSeaGreen>FID 是一种加载响应能力指标，旨在评估网页给用户的第一印象</font>；而 <font color=red>INP 是一种更可靠的总体响应能力指标</font>，无论网页互动发生在网页生命周期的什么时间点，它都能反映网页的总体响应能力。

摘自：[Interaction to Next Paint (INP)](https://web.dev/articles/inp?hl=zh-cn)

##### TTFB

TTFB ( Time to First Byte ) 是一个衡量对资源的请求和响应的第一个字节开始和到达之间时间的指标。
...
- [使用 HTTP/2](https://developer.chrome.com/docs/lighthouse/best-practices/uses-http2/) or [HTTP/3](https://en.wikipedia.org/wiki/HTTP/3)。
  > 使用 TLS1.3 以减少 RTT，使用 HTTP/3 避免队头阻塞
...
> - 将标记流式传输到浏览器
>
>   浏览器经过优化，可在标记以流式传输时高效处理标记，这意味着标记会以块的形式从服务器到达，并被相应处理。这对于大型标记载荷至关重要，因为这意味着<font color=lightSeaGreen>浏览器可以逐步解析标记块，而不是等待整个响应到达后再开始解析</font>。
>
> - 使用 Service Worker
>
>   Service Worker API 可以对文档及其加载的资源的 TTFB 产生很大影响。这是因为<font color=red>服务工作线程充当浏览器和服务器之间的代理</font> （👀 参考 PWA ），但这对网站的 TTFB 是否有影响取决于您如何设置服务工作线程，以及该设置是否符合您的应用要求。
>
>    - **为资源使用 stale-while-revalidate 策略。**<font color=dodgerBlue>如果资源</font>（无论是文档还是文档所需的资源）<font color=dodgerBlue>位于 Service Worker 缓存中</font>，<font color=red>stale-while-revalidate 策略将 *首先* 从缓存中提供该资源，然后在后台下载该资源，并从缓存中提供该资源以供日后互动使用</font>。
>      - 如果您有不经常更改的文档资源，使用 stale-while-revalidate 策略可以使网页的 TTFB 几乎达到即时效果。不过，如果您的网站发送的是动态生成的标记（例如，根据用户是否已通过身份验证而变化的标记），此方法的效果就不是很好。在这种情况下，您始终希望 *先* 访问网络，以便获取尽可能最新的文档。
>      - 如果您的文档加载的非关键资源会以一定频率发生变化，但提取过时的资源不会对用户体验造成太大影响（例如，某些非关键图片或其他资源），则可以使用 stale-while-revalidate 策略大幅缩短这些资源的 TTFB。
>    - **对于客户端渲染的应用，请 [使用应用 Shell 模型](https://developer.chrome.com/blog/app-shell?hl=zh-cn)** 。此模型最适合 SPA，因为页面的 “外壳” 可以立即从服务工作线程缓存中传送，而页面的动态内容会在页面生命周期的后期填充和呈现。
>   
> - 为渲染关键资源使用 `103 Early Hints`
>
>   无论应用后端优化得多么出色，服务器仍可能需要执行大量工作才能准备好响应，包括代价高昂（但必要）的数据库工作，这会延迟导航响应的到达时间。这可能会导致某些后续的渲染关键资源（例如 CSS 或在某些情况下在客户端上渲染标记的 JavaScript）延迟加载。
>
>   [`103 Early Hints` 标头](https://developer.chrome.com/blog/early-hints?hl=zh-cn)是服务器在后端忙于准备标记时可以发送给浏览器的早期响应代码。<font color=dodgerBlue>此标头可用于向浏览器提示</font>，<font color=red>在准备标记时，网页应开始下载渲染关键资源</font>。对于支持的浏览器，此效果可加快文档渲染速度 (CSS) 并缩短网页加载时间。
>   
>   > ⚠️ **注意**：如果您的网站在后端不会进行大量处理来准备标记（例如静态网站），那么 `103 Early Hints` 可能不会有太大帮助。对于在向用户发送标记之前需要大量后端时间的网站，此方法往往效果最好。
>   
> - 使用 `Server-Timing` 调试现场的高 TTFB
>
> 摘自：[优化加载第一个字节所需时间](https://web.dev/articles/optimize-ttfb?hl=zh-cn)
...
> [!TIP]
>
> > <font color=dodgerBlue>未构建完的 CSSOM Tree 是不准确的</font>，<font color=fuchsia>浏览器必须等到 CSSOM Tree 构建完毕后才能进入下一阶段</font>。
> >
> > 所以，<font color=red>CSS 的加载速度与构建 CSSOM Tree 的速度将直接影响首屏渲染速度</font>，因此<font color=red>**在默认情况下 CSS 被视为阻塞渲染的资源**</font>，需要将它尽早、尽快地下载到客户端，以便缩短首次渲染的时间。
> >
...
使用 hash，会让所有产物的文件哈希相同；chunkhash 根据不同的入口文件进行依赖文件解析，构建对应的 chunk，生成对应的哈希值；contenthash 计算与文件内容本身有关，主要用于 CSS 抽离 CSS 文件。

##### hash

hash 是基于整个项目构建的，这意味着项目中的任何一个文件发生变化，所有文件的 hash 值都会改变。

不适合用于长期缓存，因为任何一个文件的改动都会导致所有文件的哈希值变化，从而使得缓存失效
...
摘自：[webpack中hash、chunkhash、contenthash三者区别](https://juejin.cn/post/7336419469579829258)
...
且不是说性能谁好，对比下这两框架版本迭代可以发现：React  迭代是增加了一个个避免刷新的钩子函数或者 `API` ( `useMemo` / `useCallback` / ... ) ，采用 `Fiber` 架构做时间分片也是来优化渲染的性能；而 Vue 每个版本虽然改的东西多，但核心都是围绕响应式来优化的，所以我觉得这是这两框架之间最重要的区别。
...
在打包时直接分析代码，一旦看到 `count` 变量绑定了某个 `<p>` 标签，就会生成一行原生 JS 代码 `p.textContent = count` ，当 `count` 变了，直接执行这行代码去改 DOM。因为没有创建 vdom树，所以运行时没有 diff 这个过程，它把 diff 提前在编译时做完了，运行时只剩下“执行修改”的任务，路径最短，所以很快

###### 不用 vdom ，怎么做到精准更新不失控

靠的是“细粒度的观察者模式”，以 Solid.js 为例，它的状态被拆成一个个独立的原子 Signal，每个 DOM 节点都作为一个 Effect 订阅自己的关心的 Signal。状态一变，不像 React 那样以组件为粒度全量重渲染，再 Diff；它直接通过依赖追踪，精准地通知到绑定这个状态的那个文本节点区更新

学习自：[无虚拟DOM性能也不错是为什么](https://www.bilibili.com/video/BV1mXj36WEjJ)
...
- 完整的 URL
- 省略协议：比如  `//localhost:8080` 。在浏览器中访问时，浏览器会把当前页面的协议照搬过来，作为补全。这样写的好处是：在开发时往往地址使用的协议是 `http://` ，而上线生产时是 `https://` ，让浏览器自动补全，也就不需要程序员一个一个手动修改协议了，减小了出错的风险
...
1. 确认动机：为什么要封装组件？
	...
	- 另外，这也就需要讨论组件的使用范围：是为了一个页面，还是一个项目服务，甚至是为了跨项目、跨公司、跨行业而服务
...
AccessToken 需要被 JS 读取，并放在 HTTP 请求的 `Authorization` 头里（ `Bearer xxx` ）发送给后端。将它放在内存中，可以避免 XSS 直接从 localStorage 里扫荡。当用户关闭标签页或刷新页面时，内存中的 AccessToken 会丢失。
...
**页面级别** 的 “控制权限” 可以通过 `router.addRoute()` 和 `router.removeRoute()` 实现。

**按钮级别** 的 “控制权限” 在 Vue 中一般通过指令实现，根据指令传入的 `binding.value` 的 role 或者对应的权限操作名（一般可以通过类似于 Linux 文件系统中的 rwx 的权限设置，或者 React Lane 模型二进制表示权限 ），从而控制的展示与否（默认展示，如果不在权限范围内则调用 `el.parentNode?.removeChild(el);` ）

> [!TIP]
> 
> 虽然早就使用位运算作为权限控制是相当不错的实践，不过好在哪里有点遗忘，看了 [用位运算实现权限组合【渡一教育】](https://www.bilibili.com/video/BV1Qa3kzDEhC) 做下补充：
> 
> ```js
> const READ = 0b1
> const WRITE = 0b10
...
- <font color=red>适用于复杂的权限规则</font>：后端可以使用更复杂的逻辑和规则来处理权限控制，例如基于用户角色、用户组、权限等复杂控制逻辑。
...
> [!abstract] TL;DR：面试回答框架
> 先通过 `TTFB`、`FCP`、`LCP` 和 Performance Trace 判断白屏发生在哪个阶段，再沿着 **网络加载 → JS 执行 → 渲染展示** 三个方向展开：
>
> 1. **网络与资源加载方向**：让首屏必需资源更早、更快到达浏览器。降低 `TTFB`（CDN、服务端缓存、接口优化）；通过 Gzip / Brotli、Tree Shaking、代码分割、图片压缩减小传输体积；使用 HTTP/2 或 HTTP/3、`preconnect`、`preload` 优化连接和加载顺序；懒加载非首屏资源，非关键脚本使用 `defer` / `async`。
> 2. **JS 执行与主线程方向**：避免资源下载完成后，页面仍因主线程阻塞而白屏。减小首屏 JS，使用路由和组件懒加载；减少同步计算、拆分长任务；延迟埋点等非核心逻辑；将复杂计算放入 Web Worker；避免等待多个接口全部返回后才挂载页面。
> 3. **渲染与展示方向**：尽快产生首个像素和首屏主要内容。内联 Critical CSS、减少渲染阻塞；降低首屏 DOM 和 CSS 复杂度；优先展示基础框架、骨架屏或 Loading；SPA 可考虑 SSR、SSG、流式渲染；避免字体和首屏大图阻塞主要内容。
>
> 可概括为：**网络阶段解决“资源到达慢”，执行阶段解决“主线程阻塞”，渲染阶段解决“内容迟迟不能上屏”**。骨架屏、Loading 等也可单独归入“感知体验优化”，但它们主要是减少用户感知到的白屏，不一定真正缩短加载时间。

“白屏优化” 就是减少用户从打开页面到看到第一个可用内容之间的空白时间。它关注的不是页面完全加载完，而是用户不要长时间面对一片空白。可以把它理解为前端性能优化里最靠前的一段体验：从请求 HTML、加载 CSS/JS、执行 JS、渲染首屏内容，到用户看到东西。

###### 常见原因

白屏通常不是一个单点问题，而是链路问题：

- HTML 返回慢：服务端慢、网络慢、CDN 没命中。
- CSS 阻塞渲染：关键 CSS 太大，样式文件加载慢。
- JS 包太大：首屏必须等大 bundle 下载、解析、执行完。
- SPA 客户端渲染：HTML 只有一个空 `div`，必须等 JS 和接口都回来才有内容。
- 接口阻塞首屏：页面等核心接口返回前不渲染。
- 第三方脚本阻塞：埋点、广告、监控脚本影响主线程。
- 字体、图片、资源加载策略不合理。

###### 优化手段

- **先监控定位** ：不直接“拍脑袋优化”。先用 Lighthouse、Chrome Performance、Web Vitals、Performance API、<font color=red>前端监控平台</font> 看瓶颈在哪里：是 `TTFB` 慢、资源慢、JS 执行慢，还是接口慢。
  
- **服务端与网络层**：使用 CDN、<font color=red>缓存</font>、Gzip / Brotli 压缩、HTTP/2 或 HTTP/3、DNS 预解析、preconnect，减少 HTML 和静态资源的等待时间。
  
- **资源加载层**：做代码分割、Tree Shaking、<font color=red>懒加载非首屏模块</font>，压缩 JS / CSS，图片使用 WebP / AVIF，首屏关键资源用 `preload`，非关键脚本用 `defer` / `async`。
  
- **渲染层**：抽离或内联关键 CSS，减少阻塞渲染的资源；<font color=lightSeaGreen>首屏优先渲染骨架屏、Loading、基础框架，避免用户看到纯白页</font>。
  
- **框架层**：对 SPA 项目，可以考虑 SSR、SSG、流式渲染、路由懒加载、组件懒加载、接口预取、缓存首屏数据，减少 “JS 没执行完什么都没有” 的问题。
  
- **JS 执行层**：<font color=red>减少首屏**同步**计算，拆分长任务，延迟非核心逻辑</font>，第三方脚本异步加载，复杂计算放到 Web Worker。

学习自：gpt-5.5 xh 的回答，也可以参考 [面试拷打系列：vue组件通信和白屏优化【渡一教育】](https://www.bilibili.com/video/BV1ix7f63ESz)

#### 弱网环境下的前端优化

> [!abstract] TL;DR：面试回答框架
> 先把弱网拆分为 **低带宽、高延迟、连接不稳定** 三类问题，优化目标也不只是“加载更快”，而是同时做到：**首屏尽快可用、关键操作可靠、失败后能够恢复**。
>
> 1. **减少传输量**：代码分割、Tree Shaking、压缩 JS / CSS、优化图片与字体、接口分页和字段裁剪。
> 2. **优化加载顺序**：使用 CDN 和缓存，优先加载首屏关键资源，延迟非关键脚本；必要时使用 SSR、SSG 或流式渲染。
> 3. **提高请求可靠性**：设置超时和取消机制；对幂等请求进行有限次数的指数退避重试；限制并发、请求去重；上传采用分片与断点续传。
> 4. **让交互可恢复**：展示缓存数据或骨架屏，区分离线、超时等状态，保存表单草稿，提供重试入口，并为乐观更新设计失败回滚。
> 5. **动态降级**：弱网下使用低清图片、关闭视频自动播放、停止非关键预加载和后台刷新，优先保证文字与核心操作。
>
> 实施时还要考虑 **缓存失效、重复提交、重试风暴** 等风险，最后用 LCP、首屏资源体积、接口失败率和关键操作成功率等指标验证效果。
...
因此，弱网优化不能只关注资源加载速度，还要保证核心业务操作能够成功，并且在失败后可以恢复。

###### 减少传输量

- 对路由和组件进行代码分割，动态加载非首屏模块
- 通过 Tree Shaking、压缩 JS / CSS、Brotli 或 Gzip 减小资源体积
- 图片使用 WebP / AVIF、`srcset` 响应式图片，并懒加载非首屏图片
- 对字体进行子集化，优先使用 WOFF2，并设置 `font-display: swap`
- 接口采用分页、字段裁剪和增量更新，避免一次返回大量数据
- 减少不必要的第三方脚本、埋点和重复请求

###### 让关键资源优先到达

- 优先加载首屏 CSS、核心 JS 和 LCP 图片
- 非关键脚本使用 `defer`、`async` 或延迟执行
- 仅对真正关键的资源使用 `preload`、`preconnect`，避免资源争抢带宽
- 使用 SSR、SSG 或流式渲染，让用户先获得可用的 HTML
- 使用 CDN，将静态资源部署到距离用户更近的节点

关键在于建立资源优先级，而不是把所有资源都设为高优先级。
...
- HTML 使用协商缓存或较短的缓存时间，避免新版本无法及时更新
- 接口配合 `ETag`、`If-None-Match`，减少重复传输
- 允许短时间使用旧数据的接口可以采用 `stale-while-revalidate`
- 使用 Service Worker 缓存应用外壳和必要数据
- 使用 IndexedDB 保存表单草稿、历史数据或待提交操作

> [!WARNING]
> 缓存策略必须配合资源指纹、版本控制和失效机制，否则发布后用户可能仍然看到旧页面。

###### 提升请求可靠性

- 设置合理的超时时间，并通过 `AbortController` 取消不再需要的请求
- 只对幂等请求进行有限次数的自动重试
- 重试使用 **指数退避 + 随机抖动（jitter）**，避免服务恢复时出现请求风暴
- 对请求去重并限制并发，避免弱网下大量请求互相阻塞
- 大文件采用分片上传、断点续传和失败分片重传

> [!WARNING]
> 不应直接重试支付、创建订单等非幂等操作。确实需要重试时，应由服务端提供幂等键，防止重复提交。

###### 设计可恢复的交互

- 优先显示骨架屏或已有缓存数据，避免长时间白屏
- 明确区分加载中、离线、超时和服务端错误等状态
- 提供重新加载或重新提交入口
- 自动保存表单草稿，防止断网后内容丢失
- 乐观更新必须包含失败回滚和状态提示
- 部分请求失败时保留已经成功加载的内容，不要让整个页面不可用
- 网络恢复后重新同步数据，必要时处理数据冲突

###### 根据网络状况动态降级

可以将 `navigator.connection` 作为参考，但不能完全依赖它，因为该 API 的兼容性和准确性有限。更可靠的做法是结合真实请求耗时、失败率和 Resource Timing 数据判断网络质量。
...
- **用户体验指标**：FCP、LCP、INP、首屏可操作时间
- **资源指标**：首屏资源体积、请求数量
- **接口指标**：P95 / P99 响应时间、超时率、失败率、重试成功率
- **业务指标**：弱网下关键操作成功率

先使用 Chrome DevTools 网络限速和 Lighthouse 进行实验室测试，再通过线上 RUM（Real User Monitoring）数据验证真实效果。

###### 面试回答示例

> 我会先把弱网拆成低带宽、高延迟和连接不稳定三个问题。优化目标不只是缩短加载时间，还要让首屏尽快可用、关键操作能够成功，并且失败后可以恢复。
>
> 第一层是减少传输量，包括代码分割、图片与字体优化、接口分页和字段裁剪；第二层是通过 CDN、缓存、关键资源优先级以及 SSR 或 SSG，让首屏内容优先到达；第三层是提高可靠性，对幂等请求设置超时、指数退避重试和并发限制，并为表单、上传提供本地草稿、断点续传和失败恢复；第四层是动态降级，例如弱网时使用低清图片、停止视频自动播放并减少后台请求。
>
> 实施时还需要处理缓存失效、重复提交和重试风暴等问题，最后通过 LCP、首屏资源体积、接口失败率和关键操作成功率评估结果。

如果有实际项目经历，应补充真实的优化前后数据，不要使用未经测量的数字。

整理自：Codex 会话 `019fb0ff-97af-7ca3-a2be-bb0290648521`
...
- 流式读取
  ...
  - fetch stream
    ...
    > >
    > > <font color=dodgerBlue>在这个阶段</font>，<font color=fuchsia>我们可以通过检查响应头，来检查 HTTP 状态以确定请求是否成功，**当前还没有响应体**（response body）</font>。
    > >
...
2. 合并请求：雪碧图、接口聚合（比如 BFF，或者后端改动）
3. 延迟请求：懒加载、请求队列（比如 p-limit ，也可以是按优先级发送请求，甚至是防抖节流）。这里感觉就是在 “削峰”
4. 用户体验：提示、loading
5. 扩容
...
> [!abstract] TL;DR：面试回答框架
> 秒杀场景的特点是：流量在活动开始的一瞬间集中爆发，并且包含大量重复请求和无效请求。前端优化的核心不是“让所有请求更快”，而是：**尽量在 CDN、边缘节点和浏览器侧消化流量，减少到达源站的无效请求，并保证接口拥塞或局部故障时仍有明确、稳定的用户反馈**。
>
> 可以从四层展开：
>
> 1. **加载层**：页面骨架和静态资源静态化并通过 CDN 分发；提前加载登录状态、活动资格和风控组件；使用服务端时间校准倒计时。
> 2. **请求层**：按钮锁定、请求去重、幂等标识；排队结果采用指数退避和随机抖动轮询；页面进入后台后暂停轮询。
> 3. **交互层**：区分提交中、排队中、成功、售罄、无资格、系统繁忙和结果未知；请求超时后查询最终状态，不盲目重新下单。
> 4. **降级层**：暂停评论、推荐、动画和非关键埋点，把网络和主线程资源留给核心链路。
>
> 需要明确职责边界：**前端限流、防抖和按钮禁用只能减少正常用户产生的无效流量，不是安全边界。库存原子扣减、一人一单、幂等、防超卖、限流和防刷必须由服务端、网关和数据库兜底。**
...
- 尽量减少请求到达源站
- 在接口拥塞甚至部分不可用时，仍然给用户明确、稳定的反馈

前端可以进行请求削峰、阻止正常用户重复提交并改善高并发下的体验，但不能依赖前端保证库存一致性、绝对公平和接口安全。前端代码、接口地址和请求参数最终都可以被分析或绕过。
...
秒杀前的商品介绍、活动规则、图片等内容通常变化不大，可以：

- 使用 SSG、预渲染或静态 HTML 生成页面
- 将 JS、CSS 和图片部署到 CDN，并在活动开始前预热
- 为静态资源添加内容哈希和长期缓存
- 压缩图片，使用 WebP / AVIF，并懒加载非首屏资源
- 拆分非核心代码，首屏只加载秒杀链路必需的资源

实时库存、用户资格等强动态数据不能直接作为静态内容长期缓存。可以将页面骨架静态化，再通过接口获取少量动态数据。

##### 提前初始化并校准倒计时

不要等倒计时结束后，再让所有用户同时加载登录信息、资格信息、商品信息和风控组件。活动开始前可以提前完成：
...
倒计时不能完全依赖用户设备时间。进入页面时由服务端返回 `serverTime` 和 `startTime`，前端计算时间偏移：
...
前端使用校准后的时间展示倒计时，但活动是否真正开始、用户是否具备资格，仍由服务端校验。

##### 防止重复提交

- 用户点击后立即禁用按钮，请求结束前不允许再次提交
- 同一次操作只发出一个请求，并复用同一个幂等标识
- 提交成功后停止发送购买请求
- 页面刷新后恢复“提交中”或“排队中”状态
- 使用 `BroadcastChannel` 协调同源的多个标签页，减少同一用户重复提交
...
> [!WARNING]
> 防抖、节流和按钮禁用只能限制正常的页面操作，脚本可以绕过前端，因此服务端仍然必须实现限流、幂等和唯一约束。
...
- 活动开始前禁止提交
- 未登录、无资格、已售罄的用户不发送购买请求
- 已经提交成功后不再重复请求
- 合并、节流或延迟非关键请求
- 页面进入后台后暂停结果轮询，重新回到前台时再查询

大型秒杀通常采用异步排队：服务端先受理请求，再异步校验资格、扣减库存和创建订单。前端需要区分“请求已受理”和“抢购成功”，只有服务端明确返回订单创建成功，才能展示成功。

排队结果可以采用 **指数退避 + 随机抖动（jitter）** 轮询，例如：
...
随机抖动可以避免大量客户端在同一时刻再次发起查询。WebSocket 或 SSE 也可以用于推送结果，但需要考虑连接规模和断线重连；很多场景下，带退避的短轮询实现更简单、稳定。
...
高并发下出现超时、排队和局部失败是正常现象，需要根据结果采用不同策略：

- `429`：请求过于频繁，根据 `Retry-After` 等待
- `503`：系统繁忙，提示用户稍后查询处理结果
- 请求超时：不能直接判定失败，应查询订单或排队状态
- 明确售罄或无资格：停止提交和重试
- 网络中断：恢复连接后查询最终状态

> [!WARNING]
> 超时只表示前端没有收到响应，不代表服务端没有处理成功。不要在超时后使用新的幂等标识自动重新下单；应复用原幂等标识，或者改为查询排队状态和订单状态。
...
埋点可以暂存在浏览器中，之后批量上报。核心原则是把网络和主线程资源留给倒计时、资格检查、购买提交和结果查询。

##### 风控和防刷

前端可以配合服务端完成验证码、登录校验、风控参数采集和一次性秒杀令牌申请。令牌应由服务端签发，并与用户、商品和活动场次绑定。

前端混淆、隐藏接口或动态修改接口地址只能增加分析成本，不能作为安全方案。真正的防刷需要依赖网关、WAF、账号 / IP / 设备限流、行为风控和服务端令牌校验。

##### 指标与验证

- **资源指标**：CDN 命中率、首屏资源体积、FCP、LCP、INP
- **请求指标**：重复提交比例、秒杀接口错误率、排队查询 QPS
- **业务指标**：从点击到获得最终结果的耗时、购买成功率、结果未知比例
- **稳定性指标**：超时率、`429` / `503` 比例、降级策略触发次数

##### 面试回答示例

> 我理解秒杀场景的特点是流量会在活动开始的一瞬间集中爆发，并且存在大量重复请求和无效请求。前端优化的目标主要有两个：第一，尽量减少请求到达源站；第二，在接口拥塞甚至部分不可用时，仍然给用户明确、稳定的反馈。
>
> 在加载层，我会把商品介绍、活动规则、图片和页面骨架静态化，通过 CDN 分发，并在活动开始前预热。用户登录状态、活动资格和风控组件也尽量提前初始化，避免活动开始时集中加载。
>
> 在请求层，我会使用服务端时间校准倒计时，活动开始前不允许提交；用户点击后立即锁定按钮，同一次操作只发送一个请求，并携带幂等标识。排队结果采用指数退避和随机抖动轮询，页面进入后台后暂停轮询。
>
> 在交互层，我会明确区分提交中、排队中、成功、售罄、无资格、系统繁忙和结果未知。请求超时时不能直接告诉用户失败，也不能盲目重新提交，而应该查询最终处理结果，避免第一次请求实际上已经成功。
>
> 秒杀期间还可以暂停评论、推荐、动画和非关键埋点，把浏览器主线程和网络资源留给核心链路。
>
> 需要明确的是，前端的防抖、按钮禁用和接口隐藏都不是安全方案。库存原子扣减、一人一单、幂等、防超卖、限流和防刷必须由服务端和网关保证。最后我会通过 CDN 命中率、重复提交比例、接口错误率、排队查询 QPS 和最终结果耗时验证优化效果。

一句话概括：**静态资源边缘化、动态请求最小化、提交操作幂等化、处理结果异步化、非核心功能可降级，并由服务端保证库存和交易的最终正确性。**

整理自：Codex 会话 `019fb10d-622d-7762-a686-325acfea8500`
...
学习自：[AI Studio 问答](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%2219G5IJJIlmB_krp_OetevIApyWwkeAMe4%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)

##### 原理

虚拟列表的核心思想是：**页面上看起来有很多项在滚动，但真实 DOM 里只渲染当前可视区域附近的一小段列表项**。
...
- `scroll-container`：真正发生滚动的容器。
- `phantom`：撑开总高度，让滚动条看起来像完整列表。
- `visible-list`：只放当前需要显示的那一小段数据。
- <font color=red>`translateY` 或 `absolute top`</font>：把这一小段数据移动到正确的位置。

下面分为列表项 定高和不定高 两种情况

###### 列表项定高

假设每一项高度都是 `itemHeight`，总数据量是 `total`，容器高度是 `containerHeight`。

> [!NOTE]
> 
> 想了下：容器高度和 viewport 的高度不一定一致

- 总高度可以直接算出来：`totalHeight = total * itemHeight`
- 当前滚动位置是 `scrollTop`
- 可见区域的起始下标是 `startIndex = Math.floor(scrollTop / itemHeight)`
- 可见数量是 `visibleCount = Math.ceil(containerHeight / itemHeight)`
- 结束下标是 `endIndex = startIndex + visibleCount`

实际渲染时一般还会加一点缓冲区，也叫 `overscan`，避免快速滚动时露白：

```js
renderStart = Math.max(0, startIndex - overscan)
renderEnd = Math.min(total, endIndex + overscan)
```

然后只渲染 `list.slice(renderStart, renderEnd)` ，最后把渲染出来的小列表移动到正确位置 `offsetY = renderStart * itemHeight`
...
通常需要维护一份“高度缓存”或“位置表”。例如

```js
[
  { index: 0, height: 50, top: 0, bottom: 50 },
  { index: 1, height: 80, top: 50, bottom: 130 },
  { index: 2, height: 40, top: 130, bottom: 170 },
]
```

当用户滚动时，需要根据 `scrollTop` 找到：`哪一个 item 的 top <= scrollTop < bottom`。这个 item 就是起始项。

如果数据很多，不能线性查找，一般会用**二分查找**：`startIndex = binarySearch(positionList, scrollTop)`

结束项则根据：`scrollTop + containerHeight`

继续找到可视区域底部对应的 item：`endIndex = binarySearch(positionList, scrollTop + containerHeight)`

然后同样加上 `overscan`，只渲染这一段。

不定高列表的难点在于：一开始并不知道每个 item 的真实高度。所以通常会先给一个预估高度`estimatedItemHeight = 60` ，先用估算高度生成初始位置 `estimatedTotalHeight = total * estimatedItemHeight`。等 item 真正渲染到页面后，再测量它的真实高度 `const realHeight = element.getBoundingClientRect().height`，或者用 `ResizeObserver`。当发现真实高度和预估高度不同，就更新缓存，并重新计算后面所有 item 的 `top` / `bottom`。比如第 10 项原来估计是 `60px`，实际测出来是 `100px`，差值是 `delta = 100 - 60` ，那么第 10 项后面的所有 item 的位置都要整体往下移动 `40px`。
...
视口附近的列表项渲染，远离视口的通过 padding 之类的去填充。

如果是列表项是定高的，是可以计算出远离视口项目的偏移距离的；如果不定高，那么可以通过先 **预估**，再**渲染**，最后根据渲染的结果，进行**修正**的；这样做出效果，会出现滚动条的抖动，所以往往会把滚动条做得很细，同时半透明，减少抖动时对用户的干扰，让用户注意不到，提高用户体验。

如果数据量更大，导致虚拟滚动的 DOM 都难以支撑，可以考虑使用 canvas 绘制；因为 canvas 直接使用绘制指令，性能会比 dom 好非常多。canvas 的性能如何提升？对于不会出现内容变化的 canvas，可以将这些 canvas 放到 “离屏 canvas ( OffscreenCanvas )” 中，在需要使用时，使用`CanvasRenderingContext2D.drawImage` 将 离屏 canvas 放到 canvas 上。

为什么离屏 canvas 会快很多？离屏 canvas 本质上就是创建一个 canvas，提前将它绘制好，保存的是位图 ( bitmap )，到时候直接将位图绘制到 canvas 上去

> [!NOTE]
> 
> 此外，这里提到了一个点：
> 
> 在使用 canvas 的情况下，如何对列表项中的数据进行修改？可以在点击事件触发后，在数据的上层盖上一个 input 元素，让用户可以修改，修改完成后，重新绘制 canvas

学习自：[面试拷打系列：工作流及其他面试题【渡一教育】](https://www.bilibili.com/video/BV1LREe6uEwR)
...
> [!NOTE]
> 前情提要：下面的 `Geolocation` 均是通过 `navigator.geolocation` 获得

使用 `Geolocation.getCurrentPosition(success[, error, options])` 获取定位，如果需要高精度，可以将 `options.enableHighAccuracy` 设为 true。

对于需要实时反馈定位信息的场景（比如外卖），可以考虑设置 `options.timeout` ，一般设置为 5s~10s，如果超时，就应该降级到缓存的坐标或者使用 IP 定位。不过，对于该场景，更推荐使用 `Geolocation.watchPosition(success[, error, options])`，适合持续更新位置；其中 options 和 `getCurrentPosition` 的一模一样（都是 `PositionOptions` ），不过它也有缺点：可能存在 pending、报错、耗电、持续触发权限/隐私成本，需要做节流、去重、精度判断；出问题也应该做降级处理。使用 `watchPosition` 记得使用它返回的 `id` ，并调用 `Geolocation.clearWatch(id)` 停止监听

>[!NOTE]
>
>值得注意的是：规范里 `timeout` 只从“开始获取位置”算起，**等待用户点权限弹窗的时间不算在 timeout 里**。如果用户一直不处理权限弹窗，也可能一直挂着。

当 geolocation 方案不可用时，可以根据 IP 定位接口来兜底，不过，根据 gpt-5.5 的说法，最多精度到城市就差不多了（可用，但不保证准确），而更精细的区和县就属于“偶尔能猜到，但不建议当真”；再有必要的情况下，可以弹框让用户自己填写当前所在位置

###### 实现位移轨迹平滑

> [!NOTE]
> 这里是直接用视频的截图让 gpt5.5 xh 检查和补充，感觉截图内容问题有点多，直接用的 gpt 给出的回答。不过 gpt 给出的补充有点多，这里只选了必要的部分

前端收到一堆高频推送的经纬度点，不要直接把高频 GPS 点连成 polyline。先做异常点 **过滤**，再做 **平滑**，然后使用 Douglas-Peucker 算法用于显示层 **抽稀**。点量大时可放 Web Worker；实时推送可用 WebSocket，但也要处理重连、补点、乱序和限频。

> [!NOTE]
> 
> **不要直接对经纬度做距离判断**  ，经纬度不是平面坐标，`0.0001°` 在不同纬度代表的实际距离不同。抽稀、速度、距离、漂移判断最好转换到米制坐标，或者用 Haversine / Web Mercator / 地图库投影后的点来算。

学习自：[如何实现前端地图定位功能](https://www.bilibili.com/video/BV173Ee6zEMj)

#### 工作流

可以使用已经形成国际标准的 bpmn 标准，同时跨平台通用，前端可以使用 [bpmn-js](https://github.com/bpmn-io/bpmn-js)

#### 基于请求头的灰度实现

###### header 被篡改怎么办？

用户登录后，后端生成一个带签名的灰度 Token，通过 `Set-Cookie` 或者响应体里安全地存入用户端，Token 里用 HMAC 加签名，服务器网关每次校验签名有效期。就算用户抓包改 Header ，没有密钥也无法签出合法 Token，自然无法通过校验。换句话说：Header 只能用作透传，判定灰度的逻辑必须有服务端信任链完成，前端不参与决策。

###### SPA 资源怎么跟着 Header 灰度

 不能只在 Nginx 处路由 HTML，JS 和 CSS 还在 CDN 上，你切了 HTML 版本，但用户本地缓存还是老的JS，直接白屏。

正确的做法是：构建时给灰度资源打上唯一的版本 Tag，比如灰度构建完的 manifest 里记录着 `app.gray.js` ，线上 Nginx 收到带灰度 Cookie 的请求时，不光返回灰度 HTML，还在 HTML 里注入一个全局变量 window.GRAY_VERSION

前端初始化时，读取这个变量，动态 publicPath 切到灰度 CDN 目录，再动态 import 灰度入口模块。

这样一来，同一个域名，通过 Header 驱动，前端拉到的就是完整的一套灰度静态资源，不会出现版本混搭。

##### 实时调比例和自动降级

不要改 Nginx 配置文件，必须要有个配置中心，比如 Apollo 或自研的 Config Service，灰度比例、地域、用户白名单全放在配置中心里。

Nginx 借助 Lua 或网关层脚本，每次请求去配置中心拉最新的灰度规则，或者更高效的做法，配置中心推送到网关内存，实现毫秒级生效。

降级怎么做？健康检查线程实时探测灰度服务，一旦连续失败，网关自动把灰度流量切回稳定版，前端用户零感知。

运营调比例就是改个配置项，网关瞬间生效，根本不用重启。

学习自：[基于请求头header的灰度怎么实现](https://www.bilibili.com/video/BV1vj7D6LE17)
