...
1. **解析 URL：** 首先会对 URL 进行解析，分析所需要使用的传输协议和请求的资源的路径。<font color=lightSeaGreen>如果输入的 URL 中的协议或者主机名不合法，将会把地址栏中输入的内容传递给搜索引擎</font>。如果没有问题，浏览器会检查 URL 中是否出现了非法字符，<font color=fuchsia>**如果存在非法字符，则 <font size=4>对非法字符进行转义</font> 后再进行下一过程**</font>。
