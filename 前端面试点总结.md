...
1. 事件循环模型影响回调时机：定时器是宏任务，浏览器会处理完同步任务和微任务后，才会处理宏任务
2. 存在 4ms 最小时间：定时器嵌套5层以上，必须做 4ms 的延时
3. 失活页面间隔强制调整为 1s：哪怕设置的间隔远小于 1s，也会将间隔调整为 1s

###### 如何实现

1. 通过 `Performance.now()` 计算间隔偏差，并实时调整
2. 使用 rAF ：通过渲染帧实现，不受失活页面影响，但是受到其他因素影响：比如 OS、浏览器、其他页面卡了，都会影响到渲染帧的延后。所以可以考虑结合上面的 “计算间隔偏差，并实时调整” 实现
