...
> 总结：
> 
> 除了 “监控请求进度” （比如 “跟踪上传进度” ）这一功能无法做到外，fetch 几乎是全能的。而 XHR 除了可以 “监控请求进度”（通过 `xhr.onprogress` 等事件实现，具体可见 [现代 JS 教程 - XMLHttpRequest # XMLHttpRequest 基础](https://zh.javascript.info/xmlhttprequest#xmlhttprequest-ji-chu) ） 和 取消请求（通过 `xhr.abort()` 实现）
...
- <font color=red>CJS 模块输出的是 **单个值的拷贝**</font>，而 <font color=red>ESM 输出的是**（多个）值的引用**</font>
  ...
  > [!TIP]
  >
  > 这里 ESM 输出的 “值的引用”，更准确的说：这叫 <font color=fuchsia>“**live bindings**”</font> （ MDN 译为 “动态绑定”）。这也<font color=fuchsia>是 JS 中唯二的两个存在 “值的引用” 机制（使用同一块内存空间）的地方</font>（另一个见下面。至于其他地方，哪怕是函数参数传递都是复制一份新的空间存储（对象的）地址（即：值传递），而不是引用传递，直接把原地址传过去）。也正是因为该机制，引入了 “消费者 / importee 可能会直接修改原数据” 的风险。所以，对于被导出的数据，务必将其定义为常量 ( constant )，而不是变量；这样杜绝了被修改导致数据错乱的风险
  >
  > > [!warning]
  > > 另外，**相当值得注意的是** ：<font color=fuchsia>live bindings 在中文中没有一个共识性的翻译</font>，所以渡一将其翻译为 “符号绑定”（参见 [带你对比分析一下，具名导入是在解构吗？【渡一教育】- 第 130秒开始]( https://www.bilibili.com/video/BV1M94y1V7jZ/?t=130) ），但是实际上 Google 搜索 “ JS 符号绑定” 是没有相关内容的；所以不要面试或者与人交谈不要说 “符号绑定” ，要说也是 “live bindings” 或者 MDN 的译法 “动态绑定”
  >
  > > [!note]
  > > 还有，在 [严格和非严格模式下JS输出的结果分别是？【渡一教育】](https://www.bilibili.com/video/BV1cFL7z3EW8) 中也有提及：还存在另一个 live bindings 的情况，就是在非严格模式下的 `arguments`。代码示例如下：
  > >
  > > ```js
  > > function func(a) {
...
此时开始第一步 “模块解析” ，浏览器会将相对路径转换为绝对路径，从而找到 `main.js` ，并开始下载。下载完成后，会进入 `main.js` 里面看 `main.js` 是否依赖其他模块，会去寻找所有
<font color=red>**顶层静态依赖**</font> （在循环、判断里面的依赖，以及动态依赖都不会找），并会将其全部提前（也就是 hoisting ）
...
同理，会下载 foo 和 bar 这两个模块，直到其下载完；以此类推，下载完入口即其子孙的所有依赖。

下载完之后就是模块执行了，依然是从入口文件开始执行。此外，当模块执行完了，同时该模块存在默认导出，会将导出结果放到一个缓存表格中（这里存在一个 live binding 的概念，即导入和导出使用的是同一块内存空间），当后面的模块再依赖则不会重复执行了。遇到动态导入，则同样执行 下载解析、执行 的过程
