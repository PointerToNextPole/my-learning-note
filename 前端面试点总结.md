...
这里浏览器对与CSS是先从 `.baz` 开始的，先找到 `.baz` ，在查找上层是否有 `.bar` ，一直向上。这样做是从性能方面去考虑的，只需要对 CSS 的一部分进行分析，如果存在，则继续向上；而不是直接对于整个 CSS 文档进行分析，这样很耗性能
...
>  👀 根据上图的一些想法：
>
> <font color=dodgerBlue>**构造函数**</font>（比如 `function Foo() {}` ）是有 `prototype` 属性的，用来指向 “生成新对象 所绑定的” 原型对象；但它<font color=dodgerBlue>本身也是一个函数</font>，所以它的 `__proto__` （比如 `Foo.__proto__`）是 `Function.prototype` （即：`Foo.__proto__ === Function.prototype`）。类似的， `function Object() {}` 同样是一个构造函数，根据上面的说法： `Object.__proto__ === Function.prototype` 也是在情理之中的。再进一步：由于 `new Function` 语法的存在，`Function` 也是一个构造函数，所以有 `Function.__proto__ === Function.prototype`
> 
> 文章 [写给自己的Object和Function的3个灵魂拷问](https://juejin.cn/post/6854573208109056007) 中关于上图的理解摘抄和总结
> 
...
<img src="https://s2.loli.net/2025/01/23/YcsDv74FJCV53ea.webp" style="zoom:80%;" />
