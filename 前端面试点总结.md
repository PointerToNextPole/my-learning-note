...
Vue3.5 做了一个改变：watch 的回调（下面的 `depFn` ）执行前，会检查依赖的计算属性是否有变化，会刷新一下计算属性的值 ( `refreshComputed` )，也就是会先调用计算属性的回调，判断计算属性的值本身是否存在变化
...
学习自：[vue3-5的effect优化【渡一教育】](https://www.bilibili.com/video/BV1FYHDzJESe)
...
使用 `navigator.geolocation.getCurrentPosition(success[, error, options])` 获取定位，如果需要高精度，可以将 `options.enableHighAccuracy` 设为 true。对于需要实时反馈定位信息的场景（比如外卖），可以考虑设置 `options.timeout`
