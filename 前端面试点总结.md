...
- setTimeout ：异步。
  ...
  > ```
  >
  > 这是一个代替语法，允许你包含在定时器到期后编译和执行的字符串而非函数。这个语法因为与 [`eval()`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/eval) 存在相同的安全风险所以**不推荐**使用。
  >
  > 摘自：[MDN - Window：setTimeout() 方法]
