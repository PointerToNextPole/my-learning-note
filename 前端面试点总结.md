...
1. 通过 class 创建的对象实例中，方法是不能通过 `new` 来调用的
...
学习自：[面试拷打系列：基础知识拷打 【渡一教育】](bilibili.com/video/BV1ox7f63Eoq)
#### Proxy
...
- **物理层（DOM 树）**：在渲染真实 DOM 时，<font color=red>Vue 的渲染器会拦截到 Teleport，并改变其内部 DOM 节点的挂载目标</font>。
...
“白屏优化” 就是减少用户从打开页面到看到第一个可用内容之间的空白时间。它关注的不是页面完全加载完，而是用户不要长时间面对一片空白。
...
- 延迟渲染
- 推测 API
- 虚拟列表，使用 canvas（包括离线 canvas）

学习自：[面试拷打系列：vue组件通信和白屏优化【渡一教育】](https://www.bilibili.com/video/BV1ix7f63ESz)
