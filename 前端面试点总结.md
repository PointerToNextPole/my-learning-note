...
- **资源加载层**：做代码分割、Tree Shaking、懒加载非首屏模块，压缩 JS/CSS，图片使用 WebP/AVIF，首屏关键资源用 `preload`，非关键脚本用 `defer` / `async`。
...
- **JS 执行层**：<font color=red>减少首屏同步计算，拆分长任务，延迟非核心逻辑</font>，第三方脚本异步加载，复杂计算放到 Web Worker。
