...
>
> - 在函数体内，当前执行上下文是函数执行上下文，所以 `var` 是函数作用域。
> - 在普通 script 顶层，当前执行上下文是 Script / 全局代码执行上下文，所以 `var` 会进入全局环境；在浏览器中，顶层 `var` 通常也会成为 `window` 的属性。
> - 在 ES Module 顶层，当前执行上下文是 Module 执行上下文，顶层 `var` 属于模块环境，不会成为全局对象的属性。
>
