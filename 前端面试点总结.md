...
> 以及下面补充的 INP ( Interaction to Next Paint ) ，FID ( First Input Delay ) ，TBT ( Total Blocking Time ) 总阻塞时间
...
**TTI**：全称 “Time to Interactive” ，翻译为 “可交互时间”。表示网页第一次完全达到可交互状态的时间点。可交互状态指的是页面上的 UI 组件是可以交互的（可以响应按钮的点击或在文本框输入文字等），不仅如此，此时主线程已经达到 “流畅” 的程度，主线程的任务均不超过 50ms 。
