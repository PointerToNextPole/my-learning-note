...
- 依赖预构建完成后，启动一个 web 服务器，将 index.html 返回给浏览器，浏览器会解析 index.html ；当遇到如 `<script type="module" src="/src/main.ts"></script>` 时，会向服务器按需请求 `/src/main.ts` ，Vite 的 Dev Server 会拦截这个请求，并对 `/src/main.ts` 进行即时编译，并将编译好的 js 模块返回给浏览器
...
- **多路复用** ：<font color=fuchsia>**HTTP/2 实现了多路复用**</font>，HTTP/2 仍然复用 TCP 连接，但是<font color=fuchsia>在一个 ( TCP ) 连接里</font>，<font color=fuchsia>客户端和服务器都可以同时发送多个请求或回应，而且不用按照顺序一一发送</font>（ 👀 即，可以乱序发送），<font color=fuchsia>这样就 **缓解** 了 “队头阻塞” 的问题</font>。👀 另外，还 <font color=fuchsia>**引入了优先级**</font>（ ⚠️ 这点有所遗忘），如下：
