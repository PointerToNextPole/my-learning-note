...
> ⚠️ 本教程的 正则引擎是基于 <font color=red>**.Net**</font> 的，所以和目前常用的 JS 存在些许不同（比如 JS 中 `\w` 不包含 “非拉丁字符”（也包括汉字），当然也会影响 `\W` ），<font color=fuchsia size=4>**所以一切以实践为准**</font> 。
...
| **\b**                           | 匹配 <font color=FF0000>单词的 **开始** 或 **结束**</font>（即：单词的分界处） |
...
>
> 👀 经过实践证明：JS 中 `\w` 是不包含 汉字的，对应的： `\W` 包含汉字 
