...
在 SQL 中，`NOT IN` 列表里只要包含 `NULL`，判断结果就可能变成 `UNKNOWN`。`WHERE` 只保留条件为 `TRUE` 的行，不会把 `UNKNOWN` 当作 true，所以查询可能返回空结果。
...
select *
from model_gas
where install_no not in ('', null);
```

这句看起来像是在查“既不是空字符串，也不是 NULL”的数据，但因为列表里有 `NULL`，比较结果无法确定，最终不会返回预期数据。
...
select *
from model_gas
where install_no is not null
  and install_no <> '';
```

##### 记忆点

- 判断 NULL 要用 `IS NULL` 或 `IS NOT NULL`，不要用 `=`、`<>`、`IN`、`NOT IN` 去判断。
- `NOT IN` 的列表中不要混入 `NULL`。
- 如果子查询可能返回 `NULL`，优先考虑先过滤掉 NULL，或改用 `NOT EXISTS`。
