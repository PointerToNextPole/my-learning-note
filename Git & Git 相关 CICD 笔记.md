...
> - `git switch -c <new-branch> [<start-point>]` ，即：创建一个分支并切换到该分支。
> 
> 此外，根据 [AI辅助开发+分支管理；学习分支的开发规范【渡一教育】](https://www.bilibili.com/video/BV14vfxBhEG7) 的说法：创建一个分支相当于创建了一个指针，删除一个分支也就是删除了指针，所以创建和删除过程是非常快的。同时，因为删除分支只是删除了指针，所以 <font color=fuchsia>**对应分支上的提交并不会因为分支的删除而被删除**</font> 。按照 [Gemini 给出的解释](https://gemini.google.com/share/77968090e967) ：
> 
> > 在 Git 中，“快照”通常指的是你每次提交（Commit）时，Git 对整个项目文件状态所拍下的一张“照片”。<font color=red>**这些快照作为对象（Objects）被稳妥地存储在 Git 的底层数据库（`.git/objects/` 目录）中**</font>。
> > 
> > 当你执行 `git branch -d <branch_name>` 删除一个分支时，**Git 仅仅是把那个指向某个快照的“指针”给折断了**，但它并没有去数据库里销毁那些快照数据本身。
> 
> 这也和 “可以通过 `git reflog` + 根据最后的 commit-id 创建新分支 或 `git cherry-pick` 这两种找回孤儿提交的方法” 相互验证
...
##### AI Coding 时代 worktree 的使用场景

1. **为 AI 提供“隔离的沙箱” (AI Sandbox)**

   <font color=red>worktree 是**物理隔离的文件夹**，所以它们拥有各自独立的 node_modules 或虚拟环境（venv）</font>。防止 AI Agent 改坏当前的 workspace

2. **人机并行开发 (Parallel Development)** ：使用隔离的沙箱保证 AI Agent 多任务并行

3. **AI 大规模重构后的“双屏代码审查” (Side-by-Side Review)** 

   当你让 AI 进行全局代码重构（例如：将整个项目从 JS 迁移到 TS，或者更换 UI 组件库）时，变更的代码量极大。单纯在 IDE 里看 Git Diff 往往很难看清整体逻辑的变化。

   这时，可以让重构在一个独立 worktree 进行；改完后，在两个独立的窗口中，完整地运行、浏览、对比新旧两套代码的结构

4. **保护 AI 的上下文 (Context Window Protection)** ：频繁使用 `git stash` 的场景，AI 的上下文缓存会被打乱，导致它“忘记”刚才在做什么，或者读取到错误的分支代码；使用 worktree 一劳永逸

5. **解决 AI 制造的依赖地狱 (Dependency Hell)** ：AI 有可能会修改项目依赖，比如安装一个新的依赖；这会导致，在一个分支内完成了任务，再切回其他分支会出问题。这时候，使用 worktree 随便造

总结自：[AI Studio - AI Coding 中的 Git Worktree](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221MCJzIzLas3POt02Z2ijdTqdYaJO-5BNk%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)
...
在开发软件时，<font color=LightSeaGreen>可能有多人同时为同一个软件开发功能或修复 BUG</font>，<font color=FF0000>可能存在多个 Release 版本</font>，并且<font color=FF0000>需要对各个版本进行维护</font>。所幸，<font color=FF0000 size=4>Git 的分支功能可以 **支持同时进行多个功能的开发和版本管理**</font>
...
<font color=LightSeaGreen>为了不受其他开发人员的影响，您 **可以在主分支上建立自己专用的分支**</font>。<font color=FF0000 size=4>完成工作后，将自己分支上的修改合并到主分支</font>。因为每一次提交的历史记录都会被保存，所以当发生问题时，定位和修改造成问题的提交就容易多了。
...
> ⚠️ <font color=dodgerBlue>注意，在实际开发时发现</font>：在运行 `git merge` 之前，需要将被合并分支的内容均 commited；否则无法将想要的内容<font color=FF0000 size=4>**完全**合并成功</font>。

> 💡 另一个开发时经验
> 在开发时出现了如下场景：没考虑太多，没有拉新的分支，而是直接对一个已经差不多测完的最新 feature 分支中做了修改，并产生了三次提交；而这个分支当天需要合并到生产，这修改的内容是优化改动，且涉及的场景较多，显然无法很快测试完并合并到生产，于是便产生了如下需求：是否可以在 merge 时，忽略倒数三次提交？问了下 AI，得到了如下方案：`git merge <branch>^3` ，显然 `^3` 和 HEAD 有关，表示从当前 HEAD 向上推 3 次提交。另外，当时还想到了其他的方案比如：
> 1. `git cherry-pick` 应该（当时不确定）是可以 pick 一个范围中的提交的；pick 一个范围的相关内容可见 [[#《Git不要只会pull和push，试试这5条提高效率的命令》中关于 cherry-pick 的补充]] 中的 “相关命令选项” ）。
> 2. 通过三次 `git diff <commitId> > commit-msg-related-name.patch` 将三次 commit 的修改内容保存为本地文件，然后 `git reset --hard` 将文件恢复，并合并生产；最后再根据 patch 文件恢复回来；虽然这样做很麻烦，不过也是有效的方案罢了。
...
<font color=LightSeaGreen>执行 fetch 就可以取得远程数据库的最新历史记录</font>。<font color=FF0000>**取得的提交 会导入到没有名字的分支，这个分支名为 FETCH_HEAD**</font>。
...
避免了在拉取远端代码需要先 stash 再拉取代码，只需加上 `--autostash` 即可自动实现

学习自：https://x.com/cosmtrek/status/1831965619706872051
...
假设 dev 分支上的提交 76d12 为 index.js 文件添加了一项修改，而我们希望将其整合到 master 分支中。<font color=LightSeaGreen>我们并不想要整个 dev 分支，而只需要这个提交</font>！
