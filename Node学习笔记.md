...
> 💡 25/07/27，重读上面的内容，感觉没说清楚，这里以  [对等依赖报错问题【渡一教育】](https://www.bilibili.com/video/BV1wb421J7CL) 中说明的为例：
> 
> 在编写 Vue 插件时，插件本身不一定要用到 Vue。同时，因为 Vue 分为 Vue2 和 Vue3 两种版本，两个版本插件写法是不一样的（ Vue2 是 `install(Vue, options) {}`，Vue3 是 `install(app)` ），但是作为插件作者还是要告知用户（或者是包管理工具 ）使用该插件时应该使用哪个 Vue 版本。如之前所说：该插件本地并没有使用 Vue ，没法通过 `dependencies` 和 `devDependencies` 来说明，这时候就应该作为对等依赖，在 `peerDependencies` 中说明
...
学习自：[package.json中版本号 `~1.2.3` 和 `^1.2.3的区别`?【渡一教育】](https://www.bilibili.com/video/BV1HzkyYVE8N)

###### 先行版本

先行版本号可以被标注在修订版之后，<font color=lightSeaGreen>先加上一个连接号 `-` 再加上一连串以句点分隔的标识符来修饰</font>。<font color=red>标识符必须由 ASCII 字母数字和连接号组成，且禁止留白</font>。数字型的标识符禁止在前方补零，举个例子：1.0.0-alpha.6
...
<font color=dodgerBlue>`prepatch` 和 `prerelase` 的区别在于</font>，`prepatch` 是增加 `patch` 号，先行版本号置为 0，`prerelase` 则是如果没有先行版本号，则跟 prepatch 一样，如果有的话，则会在先行版本号上加 1，举个例子，当你的版本号为 `1.0.0-0`时，
...
摘自：[冴羽的 JavaScript 博客 -   聊聊 npm 的语义化版本（Semver）](https://www.yuque.com/yayu/blog/tphsm7)
...
<font color=FF0000 size=4>**npx 的原理很简单，就是运行的时候，会到 `node_modules/.bin` 路径和环境变量 `$PATH` 里面，检查命令是否存在**</font>。
