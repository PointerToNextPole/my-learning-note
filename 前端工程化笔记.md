...
> 👀 由于 Next.js 自动集成了 Tailwind CSS 的各种配置，开箱即用，甚至提供了模版 [with-tailwindcss](https://github.com/vercel/next.js/tree/canary/examples/with-tailwindcss) ( 通过运行 `npx create-next-app --tailwind projName` )，不能进行从零到一的配置。所以，使用 Vite 了 react-ts 模版实现，其他的脚手架感觉也类似。
