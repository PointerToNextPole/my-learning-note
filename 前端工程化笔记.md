...
>
> > 该选项允许你创建自定义的公共 chunk。当值为对象形式时，<font color=red>每个属性代表一个 chunk</font>，其中包含列出的模块及其所有依赖，除非他们已经在其他 chunk 中，否则将会是模块图 ( module graph ) 的一部分。chunk 的名称由对象属性的键决定。
> >
> > 
...
根据上面的表述相当于：决定了某几个依赖放在同一个 chunk 中，而不是让构建工具自己决定。另外，这里的内容也可以看下 [[前端面试点总结#局部导入的优势]] 中的内容，感觉存在一定关联
