...
>
> <img src="https://files.seeusercontent.com/2026/06/10/oJ9b/sU1pQTHG5qWSzDh.png" alt="image-20230529181659686" style="zoom:50%;" />
...
类似作用的命令还有 `npm view <repo>` ，它也包含一些子命令，如 `npm view <repo> repository` 

<img src="https://files.seeusercontent.com/2026/06/25/h5Lo/image-20260625164308470.png" alt="image-20260625164308470" style="zoom:50%;" />
...
`npm ci` 是 npm 提供的“干净、可复现安装”命令，主要用于 CI/CD、自动化构建和部署环境。<font color=red>`ci` 指 **Clean Install**</font>。

它会：

- 根据 `package-lock.json` 或 `npm-shrinkwrap.json` 安装精确版本。
- <font color=red>安装前自动删除现有的 `node_modules`</font>。
- <font color=red>如果 `package.json` 与锁文件不一致，**直接报错**，不会自动修复锁文件</font>。
- 不修改 `package.json` 和锁文件。
- 只能安装整个项目，不能像 `npm install lodash` 那样添加单个依赖。

###### 与 `npm install` 的主要区别

| 行为                    | `npm ci`             | `npm install`            |
| ----------------------- | -------------------- | ------------------------ |
| 要求锁文件              | 是                   | 否                       |
| 自动删除 `node_modules` | 是                   | 否                       |
| 锁文件不一致            | 报错                 | 通常更新锁文件           |
| 修改锁文件              | 否                   | 可能                     |
| 适用场景                | CI、部署、可复现构建 | 本地开发、添加或更新依赖 |
...
需要注意，`npm ci` 默认仍会执行依赖包的生命周期脚本。若依赖来源不可信或希望禁用脚本：

```sh
npm ci --ignore-scripts
```

简而言之：日常修改依赖时使用 `npm install`，根据已经提交的锁文件进行稳定构建时使用 `npm ci`。

摘自：codex://threads/019f9936-697a-75b3-8cab-13de1b971309
...
Tailwind CSS v4 使用 CSS 变量作为 Design Token。`--spacing` 是间距工具类使用的基础单位，`p-4`、`mt-6`、`gap-2` 等 class 会根据它计算最终值：

```css
@theme {
  --spacing: 0.25rem; /* 默认值；根字号为 16px 时等于 4px */
}
```
...
因此，修改 `--spacing` 会整体调整使用 spacing scale 的工具类。例如将 `--spacing` 改为 `1px` 后，`p-4` 变为 `4px`；`pl-4.5` 仍然跟随这个 scale，而 `pl-[18px]` 是固定的任意值，不会随 `--spacing` 改变。


##### Arbitrary values：`[]` 和 `_`

`[]` 表示 arbitrary value（任意值），用于在工具类中直接写入不在默认设计 token 中的 CSS 值：

```html
<div class="pl-[18px]">content</div>
```

在 `[]` 内部，`_` 通常表示一个空格。这样可以把包含多个 CSS 值的内容保持为一个 class token：

```html
<div class="shadow-[0_4px_12px_rgba(0,0,0,0.2)]">content</div>
```

HTML 的 `class` 属性是由空白字符分隔的 token 列表；如果直接写成 `rgba(0, 0, 0, 0.2)`，字面空格会把它拆成多个 class，Tailwind 也就无法按预期识别完整的任意值。若任意值本身需要字面下划线，可以使用转义形式 `\_`。


##### `calc()` 中的空白

CSS `calc()` 的加减运算符两侧必须保留空白，例如：

```css
width: calc(100% - 24px);
```

但 class token 不能包含字面空格，因此要用 `_` 编码空白：

```html
<div class="w-[calc(100%_-_24px)]">content</div>
```

Tailwind 生成 CSS 时会把 `_` 还原为空格；`w-[calc(100%-24px)]` 则可能生成无效的 `calc()` 值。


##### `group`：父子状态联动

`group` 用在父元素上，表示“后代可以根据这个元素的状态应用样式”。真正写样式的是后代的 `group-hover:*`、`group-focus:*` 等 variant：

```html
<article class="group p-4">
  <h3 class="text-gray-700 group-hover:text-red-500">title</h3>
  <button class="opacity-0 group-hover:opacity-100">详情</button>
</article>
```

鼠标悬停在 `article` 的任意区域时，`h3` 和 `button` 的样式会变化；`group` 本身只是状态来源标记，不会因为写了 `group` 自动获得样式。多个 variant 可以串联，例如 `md:group-hover:shadow-lg` 表示“视口达到 `md` 断点，并且父元素处于 hover 状态”时才应用阴影。


参考会话：`019f8331-fe34-70c2-93ee-4ae2d09204e0`
...
CSS 压缩本质上是在<font color=red>不改变最终渲染结果</font>的前提下，让生产环境 CSS 产物体积更小。它和 “删除未使用 CSS” 不是一回事：

- **CSS 压缩**：把已经存在的 CSS 写得更短，比如删空格、缩短值、合并声明。
- **CSS 清理 / Tree Shaking**：删除页面没有用到的样式规则，比如 Tailwind 根据 `content` 扫描结果移除未使用工具类。

<font color=dodgerBlue>现代 CSS 压缩工具一般不是简单正则替换</font>，而是先理解 CSS 语法，再基于 AST 做优化，流程大致如下：
...
之所以需要 AST，是因为 CSS 有层叠顺序、选择器优先级、浏览器兼容、`url()`、字符串、`calc()`、CSS hack 等细节；简单正则很容易把合法 CSS 压坏。

<font color=dodgerBlue>常见的压缩优化包括：</font>

- 删除注释、换行、缩进、多余空格、最后一个分号等无用字符。
- 缩短等价值，比如 `#ffffff` -> `#fff`、`0px` -> `0`。
- 在安全情况下合并相同 selector 或相邻规则。
- 在不改变语义的前提下合并 longhand，如 `margin-top/right/bottom/left` -> `margin`。
- 删除无效、重复或确定会被覆盖的声明。
- 简化 `calc()`、颜色、`transform`、`grid-template-areas` 等写法。
- 根据目标浏览器添加或移除不必要的 vendor prefix。

常见工具包括：

- [`cssnano`](https://github.com/cssnano/cssnano)：PostCSS 生态常用 CSS 压缩器。
- [`Lightning CSS`](https://lightningcss.dev/)：Rust 实现的 CSS parser、transformer 和 minifier，压缩速度快，并且理解 CSS 语义。
- `esbuild`：内置 CSS minify 能力。
- `webpack`：常配合 `css-minimizer-webpack-plugin`。
- `Vite` / `Rollup` / `Parcel` 等构建工具：生产构建阶段通常会集成 CSS 压缩能力。

在实际项目里，通常不需要自己实现 CSS 压缩器，而是在构建工具的生产构建阶段接入成熟的 minifier。需要注意：<font color=red>CSS minify 只是压缩最终产物，不等于完整的 CSS transform 流程</font>；例如 PostCSS 插件、CSS Modules、浏览器前缀、现代 CSS 语法降级等能力，可能由同一个工具完成，也可能由不同工具分别负责。

<font color=dodgerBlue>Lightning CSS 的优势可以概括为：</font>

1. **速度快**：Rust 实现，比许多 JS 生态 CSS 压缩工具更快。
2. **更懂 CSS 语义**：它会解析 rule、property、value，而不只是把 CSS 当 token 流处理，因此能做更深入的安全优化。
3. **更注重行为安全**：不会做无法证明安全的优化，例如规则合并会谨慎处理相邻关系，避免改变 CSS 层叠顺序。
4. **支持目标浏览器处理**：可以基于 `targets` / Browserslist 做语法降级和前缀处理。
5. **压缩质量较好**：能处理 longhand 合并、颜色缩短、`calc()` 简化、前缀清理等场景，通常输出体积也比较理想。
6. **还支持 CSS Modules**：可以处理 class、id、keyframes、CSS variables 等局部作用域映射。

参考：

- [Lightning CSS Minification](https://lightningcss.dev/minification.html)
- [Lightning CSS Transpilation](https://lightningcss.dev/transpilation.html)
- Codex session：`019efdd4-4c88-7323-81ad-b0d2eb6e6493`
...
##### 如何在 Vite 实现类似于 webpack external 的功能

- 使用 [vite-plugin-externals](https://github.com/crcong/vite-plugin-externals) ，最接近 Webpack 体验

- 仅配置 Rollup 参数（无插件，仅生产环境生效）
...
- 使用 Import Maps 结合 CDN (面向未来的 ESM 原生方案)

  Vite 是基于原生 ES 模块（ESM）的。比起把第三方库挂载到 window 上，Vite 更推崇浏览器原生的 ES Module 解析方案：**Import Maps**。
  ...
  export default defineConfig({
    optimizeDeps: {
      // 告诉 Vite 开发服务器，不要对这几个依赖进行预构建
      exclude: ['react', 'react-dom']
    },
    build: {
      rollupOptions: {
        // 告诉 Vite 生产构建时，不打包这几个依赖，保留 import 语句
        external: ['react', 'react-dom']
      }
    }
  })
...
摘自：[AI Studio - Vite 实现外部依赖配置](https://aistudio.google.com/app/prompts?state=%7B%22ids%22:%5B%221w51f9ll7smDUpn7tKGxY2GsbPpwmO-0s%22%5D,%22action%22:%22open%22,%22userId%22:%22113986502377656987379%22,%22resourceKeys%22:%7B%7D%7D&usp=sharing)
...
> [!NOTE]
> 这里我主要看的是 [vitest](https://github.com/vitest-dev/vitest) 。另外，也看了 [HDAlex_John 的 Vitest 教程](https://space.bilibili.com/337242418/lists/6783066) ，包含 Vue 和 React 两种生态，将一些核心概念和工具快速过一下还是挺好的
...
- You might test the _happy path_ (successful function calls) in its own `describe()` block, and the _unhappy path_ (such as when errors are thrown) in another.
  ...
  describe('getBlogPosts()', () => {
    describe('when posts exist', () => {
      test('it gets blog posts', () => {
        expect(service.getBlogPosts()).toHaveLength(1);
      });
    });

    describe('when no posts exist', () => {
      test('returns empty array', () => {
        expect(service.getBlogPosts()).toHaveLength(0);
      });
    });
  });
});
...
摘自：[HowToTestFrontEnd - Organise your tests with describe() blocks](https://howtotestfrontend.com/courses/jest-vitest-fundamentals/test-structure-describe)

##### 测试生命周期钩子

Jest 和 Vitest 都提供了 `beforeAll`、`beforeEach`、`afterEach` 和 `afterAll` 四个常用的生命周期钩子。它们的核心区别在于：<font color=red>是在整组测试的开始或结束时执行一次，还是在每个测试的开始或结束时都执行一次</font>。

| 钩子 | 执行时机 | 执行次数 | 常见用途 |
| --- | --- | --- | --- |
| `beforeAll` | 当前作用域内的所有测试开始前 | 一次 | 建立数据库连接、启动测试服务器、加载昂贵的共享资源 |
| `beforeEach` | 每个测试开始前 | 每个测试一次 | 创建全新的测试对象、准备独立测试数据、重置 mock |
| `afterEach` | 每个测试结束后 | 每个测试一次 | 清理 DOM、恢复 mock、回滚当前测试产生的数据 |
| `afterAll` | 当前作用域内的所有测试结束后 | 一次 | 关闭数据库连接、停止测试服务器、释放共享资源 |
...
这样每个测试都会获得一个全新的 `user`。如果改用 `beforeAll`，所有测试将共享同一个对象，前一个测试对对象的修改可能会影响后一个测试，使测试依赖执行顺序。

> [!TIP]
> 需要隔离的状态优先使用 `beforeEach` / `afterEach`；创建成本较高且可以安全共享的资源使用 `beforeAll` / `afterAll`。

这些钩子会受到所在作用域的限制。写在某个 `describe` 中时，只作用于该 `describe` 内部的测试；写在测试文件顶层时，则作用于该文件中相应作用域内的测试。
...
<font color=red>You can think of the `toBe` matcher as `===`</font>.

The `toBe()` matcher is used to test for <font color=red>strict equality</font> between two values.

It checks if the expected value and the actual value are the same using the strict equality operator (`===`). <font color=red>This matcher is particularly useful when working with primitive data types like numbers, strings, and booleans</font>.
...
> [!TIP]
>
> unplugin-auto-import 是用来自动引入 API 的，而 unplugin-vue-components 是用来自动引入 Vue 组建的
>
> > Auto import <font color=red>APIs</font> on-demand for Vite, Webpack and Rollup
> >
> > 摘自：[GitHub - unplugin-auto-import - readme](https://github.com/unplugin/unplugin-auto-import)
>
> > On-demand <font color=red>components</font> auto importing for Vue
> >
> > 摘自：[GitHub - unplugin-vue-components - readme](https://github.com/unplugin/unplugin-vue-components)
...
export default defineConfig({
  plugins: [vue(), AutoImport({
    imports: ['vue', 'vue-router', 'pinia'], // 需要导入哪些包中的模块
    dirs: ['src/composables/**', 'src/utils/**', 'src/stores/**'], // 需要导入的本地模块
...
其中，有相当重要的配置 `resolvers` ，它是自定义的解析器，一般配置自动导入第三方组件库时要用到（比如 element-plus）。另外，还一些有点意思的配置，比如：`vueTemplate` ，作用是在 Vue 模版内部自动导入；`vueDirectives` ，作用是在 Vue 模版内部自动导入指令。

##### unplugin-vue-components

除了用于注册成熟的第三方组件库外，也可以用于自定义组件。示例如下：
...
如果有重名组件，建议开启：`collapseSamePrefixes: true`，这样 `src/components/user/Card.vue` 可以按命名空间变成 `<UserCard />`，避免多个 `Card.vue` 冲突。
...
3. 全局安装 [npm-check-updates](https://github.com/raineorshine/npm-check-updates) ，并运行 `ncu -u` 将 `package.json` 中所有的依赖版本号改写为最新的稳定版
   > [!NOTE]
   >
   > 也可以考虑 antfu 的 [taze](https://github.com/antfu-collective/taze) ，尤其是对于 monorepo 等现代前端工程。不过 npm-check-updates 在 **通用性、生态成熟度、复杂策略/CI 自动化** 要更强一些
   >
   > 以下说法来自 GPT5.5 Extra High：
   >
   > > **现代前端/Node monorepo：优先选 taze。**
   > >
   > > **通用性、生态成熟度、复杂策略/CI 自动化：优先选 npm-check-updates / ncu。**
   > >
   > > | 维度        | npm-check-updates                                            | taze                                                         |
   > > | ----------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
   > > | 默认策略    | 默认升到 latest，可能跨 major；可用 `--target` semver/minor/patch 收敛 | 默认只更新 `package.json` 允许范围内的版本，更保守           |
   > > | monorepo    | 支持 `--deep`、`--workspace`、`--workspaces`                 | README 明确把 monorepo 做成一等能力，`taze -r` 会递归扫描并处理本地私有包 |
   > > | 现代包管理  | 支持 npm/yarn/pnpm/deno/bun；v21+ 是现代 ESM/CJS 双构建      | 更贴近 pnpm/yarn/bun 生态，近期有 `package.yaml`、Yarn catalogs、bun catalog、pnpm/yarn maturity 配置相关更新 |
   > > | 安全冷却期  | 有 `--cooldown`，还能显示被跳过包、写函数配置                | 有 `--maturity-period`，并可从 pnpm/yarn 配置推断 exclude    |
   > > | 成熟度/生态 | 更老牌：10.3k stars、365 forks、最新 v22.2.2，2026-06-03     | 更年轻但活跃：4.2k stars、143 forks、最新 v19.14.1，2026-05-25 |
   > > | 高级能力    | 更强：programmatic API、JSON 输出、复杂 filter/reject 函数、peer 检查、doctor 跑测试定位破坏性升级 | 更轻量，面向日常升级和 monorepo 体验                         |
   > >
   > > 为什么我会偏向 **taze 用于现代 monorepo**：它的 README 直接写了 “Built-in support for monorepos”，`taze -r` 会递归扫描 `package.json` 并自动处理本地私有包；默认也更安全，只在现有 semver 范围内升级。它还在最近版本里持续补 pnpm/yarn/bun 相关能力，比如 `package.yaml`、Yarn catalogs、bun catalog、maturity-period 推断。对 pnpm workspace、catalogs、现代多包仓库来说，它更顺手。
   > >
   > > 为什么 **ncu 仍然很强**：它成熟、选项极多、生态大得多，支持 `--deep`、`--workspaces`、多包管理器、`cooldown`、`doctor`、peer 检查、JS 配置函数和模块化调用。尤其在 CI、脚本化批量升级、复杂规则过滤、或者非 monorepo 的普通项目里，ncu 仍然是更稳的“瑞士军刀”。
   > >
   > > **我的建议**：
   > >
   > > 如果你们是 pnpm/yarn/bun workspace、catalogs、多 package 的现代仓库，用 **taze** 作为默认依赖更新工具。
   > >
   > > 如果你们需要跨很多老项目统一脚本、依赖复杂筛选、JSON/程序化输出、doctor 跑测试排雷，用 **npm-check-updates**。
   > >
   > > 大型团队里也可以组合：日常 `taze -r`，季度/专项大版本升级时用 `ncu --deep --target major --cooldown 7` 做更强的审计和控制。
5. 依赖安装 install
