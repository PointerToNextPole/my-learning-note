...
##### MacBook 搬运和更换网络时，如何尽量无损地恢复 Codex 长任务

> [!NOTE]
> 整理自 [Codex thread：确认熄屏时 Codex 任务执行](codex://threads/019fd0aa-4ea4-72f2-adf7-93a643c31eec)。这个会话先讨论了熄屏和系统睡眠的区别，最后重点讨论了：Codex 正在 MBP 上执行长任务时，如何在搬运电脑、更换网络的情况下尽量减少中断损失。

首先要区分三种情况：

| Mac 状态 | 本地 Codex 任务 | 云端任务 |
| :--- | :--- | :--- |
| 仅熄屏或锁屏，系统仍清醒 | 继续执行 | 继续执行 |
| 系统睡眠或合盖睡眠 | 通常暂停，唤醒后才可能恢复 | 继续执行 |
| 关闭 Codex 应用 | 不能保证继续执行 | 继续执行 |

<font color=red>熄屏不会让任务变快；真正影响本地任务的是 Mac 是否进入系统睡眠</font>。网络仍连接也不代表本地进程仍在执行。若只是暂时离开电脑，可以接通电源、不要合盖，并运行：

```bash
caffeinate -i
```

它会阻止由空闲触发的系统睡眠，同时允许屏幕正常熄灭；任务结束后按 `Control+C` 停止。

###### 搬运电脑时的核心原则

同一台 MBP 已经写入磁盘的文件、Git 改动和 Codex thread 记录通常不会丢失。真正容易中断的是：<font color=red>正在进行的模型请求、联网命令、下载、SSH 连接，以及尚未完成的文件写入</font>。

因此，不要追求搬运过程中保持本地任务不断线，而应使用 <font color=fuchsia>**“建立检查点 → 合盖搬运 → 恢复核验”**</font> 的流程。
...
```text
我即将合盖并更换网络。请不要开始新的长步骤。

完成当前可以安全完成的原子步骤；如果当前步骤不能安全完成，请在合适位置停止。
然后记录一个恢复检查点，包括：
1. 已完成和未完成的工作
2. 当前 Git 状态和主要文件改动
3. 最后一条命令及其是否确定成功
...
将检查点写入当前任务回复；必要时同时写入 work/CODEX_CHECKPOINT.md。
完成后停止并等待，不要继续执行新步骤。
```

等 Codex 明确回复已经暂停并等待继续后再合盖。如果正在执行数据库迁移、部署、远程写入或其他不可安全重复的外部操作，应先等它结束或安全回滚，不要直接强制中断。
...
> [!warning]
> 不要在支付、删除、部署、数据库结构变更等具有外部副作用的操作进行到一半时直接合盖。

###### 到达后：先核验，再继续

连接稳定网络、接通电源，打开同一个 Codex thread，然后发送：

```text
已经到达新地点。请读取之前的检查点，并先执行恢复核验：

1. 检查 git status 和 git diff
2. 检查最后一步的输出文件或副作用
3. 检查原有进程是否仍在运行
4. 除非有明确证据，否则把中断时的最后一条命令视为“完成状态未知”
5. 从第一个未完成且可以安全重复的步骤继续

不要盲目重复可能产生外部副作用的操作。
```

恢复时的关键不是“从刚才那条命令继续”，而是<font color=red>先确认中断前最后一步到底有没有成功</font>，再从第一个未完成且可安全重复的步骤继续。

###### 不同任务的恢复方式

- **文件编辑、Git 改动**：通常完整保留。
- **本地编译或测试**：可能从睡眠处恢复，但重新运行更容易确认结果。
- **下载、API 请求、浏览器操作**：切换网络后通常需要重试。
- **SSH 命令**：连接可能断开；以后可在远端使用 `tmux` 或 `screen` 保留进程。
- **数据库迁移、部署、外部写操作**：必须先核验实际状态，不能直接重复。
- **Codex 云端任务**：Mac 睡眠或关闭应用后仍可继续，但本地任务不能把正在运行的本机进程无缝迁移到云端。

> [!warning]
> 不要为了让任务继续运行，把保持清醒且已经合盖的 MacBook 放进包里，这会带来明显的发热和耗电风险。需要在两个场所间反复移动时，每次出发前都建立检查点，比尝试维持网络连接可靠。
